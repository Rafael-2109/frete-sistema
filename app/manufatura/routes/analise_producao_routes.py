...
def register_analise_producao_routes(bp):
    ...
    def analise_producao():
        """Tela principal de Análise de Produção"""
        # Parâmetros de filtro
        data_inicio = request.args.get('data_inicio', '')
        data_fim = request.args.get('data_fim', '')
        cod_produto = request.args.get('cod_produto', '')
        operacao_id = request.args.get('operacao_id', '')
        page = request.args.get('page', 1, type=int)
        per_page = 50

        # Query base: tipo_movimentacao = 'PRODUÇÃO'
        ...
        if operacao_id:
            query = query.filter(MovimentacaoEstoque.operacao_producao_id.ilike(f'%{operacao_id}%'))

        # Ordenar por data desc
        query = query.order_by(MovimentacaoEstoque.data_movimentacao.desc(), MovimentacaoEstoque.id.desc())

        # Paginação
        producoes = query.paginate(page=page, per_page=per_page, error_out=False)
