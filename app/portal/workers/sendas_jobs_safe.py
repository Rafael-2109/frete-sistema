"""
Jobs assíncronos para o portal Sendas - Versão Otimizada para Render
...
def processar_agendamento_sendas(integracao_id, lista_cnpjs_agendamento, usuario_nome=None):
    ...
    with app.app_context():
        try:
            ...
            for item in lista_cnpjs_agendamento:
                ...
                if isinstance(data_entrega, str) and data_entrega:
                    try:
                        ...
                        logger.warning(f"[Worker Sendas] Não foi possível converter data_entrega: {data_entrega}")
            ...
            if not integracao:
                logger.error(f"[Worker Sendas] Integração {integracao_id} não encontrada")
                return {
                    'success': False,
                    'message': f'Integração {integracao_id} não encontrada'
                }

            logger.info(f"[Worker Sendas] Integração encontrada: {integracao.nome_portal}")
            ...
            # Processar agendamento usando a função do módulo sendas
            ...
            logger.info(f"[Worker Sendas] Processamento finalizado. Sucesso: {resultado.get('success', False)}")
            ...
            logger.error(f"[Worker Sendas] Erro ao processar agendamento: {str(e)}")
            logger.error(f"[Worker Sendas] Traceback: {traceback.format_exc()}")
            ...
            try:
                ...
                logger.error(f"[Worker Sendas] Erro ao atualizar banco: {str(db_error)}")
            ...
            try:
                ...
                logger.error(f"[Worker Sendas] Erro ao registrar log: {str(log_error)}")
