...
| Geracao da fatura | Operador ou automatica (XML CTe) | `POST /emitir_fatura_freteiro/<transportadora_id>` (`routes.py:5286`) — agrupa fretes/despesas selecionados + lado CarVia |
| Excel de fechamento | — | `GET /faturas/exportar-fechamento-freteiros` — **3 abas** (2026-06-24): **Detalhamento** (Nacom + CarVia na MESMA aba, diferenciado pela coluna `Operação`=NACOM/OP.ASSAÍ/CARVIA; inclui fretes + DespesaExtra Nacom + `CarviaCustoEntrega` via `todos_custos_entrega()`) + **Resumo Nacom** (renomeada de "Resumo por Transportadora") + **Resumo Carvia** (com dados bancarios). CarVia só entra sem filtro de origem; falha CarVia NÃO é mais silenciosa (flash + redirect, era `except` mudo). Restrito a admin/financeiro/logistica. Teste: `tests/fretes/test_export_fechamento_freteiros_carvia.py` |
