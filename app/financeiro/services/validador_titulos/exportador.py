...
def gerar_excel(rv) -> BytesIO:
    ...
    for chave, rotulo in rotulos.items():
        if chave in rv.resumo:
            ws.append([rotulo, rv.resumo[chave]])
    ...
    for d in rv.resultado.duplicados:
        ...
        ws.append([d["nf_parc"], ", ".join(d["bancos"]), d["qtd_bancos"], flag])
    ...
    for b in rv.resultado.boleto_sem_nota:
        ws.append([b["nf_parc"], ", ".join(b["bancos"])])
    ...
    for n in rv.resultado.nao_identificados:
        ws.append([n.get("banco"), n.get("original"), n.get("valor"),
