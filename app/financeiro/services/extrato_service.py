...
class ExtratoService:
    ...
    def importar_extrato(
        ...
        for linha in linhas:
            try:
                item = self._processar_linha(lote.id, linha, journal_code, journal_name)
                if item:
    ...
    def _processar_linha(
        ...
        # Buscar linha de crédito (para conciliação futura)
    ...
    def listar_journals_disponiveis(self) -> List[Dict]:
        ...
        resultado = []
        for j in journals:
            # Contar linhas não conciliadas
            pendentes = self.connection.execute_kw(
            ...
            resultado.append({
                ...
                'recebimentos_pendentes': pendentes
            })
    ...
    def listar_statements_disponiveis(self, journal_code: Optional[str] = None) -> List[Dict]:
        ...
        for st in statements:
            ...
            # Contar linhas não conciliadas (recebimentos) deste statement
            pendentes = self.connection.execute_kw(
                'account.bank.statement.line', 'search_count',
                [[
                    ['statement_id', '=', st['id']],
            ...
            lote_existente = ExtratoLote.query.filter_by(statement_id=st['id']).first()
