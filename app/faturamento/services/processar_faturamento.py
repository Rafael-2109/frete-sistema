...
class ProcessadorFaturamento:
    ...
    def processar_nfs_importadas(self, usuario: str = 'Importação Odoo', limpar_inconsistencias: bool = True) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for idx, nf in enumerate(nfs_pendentes):
                try:
                    logger.info(f"🔄 [{idx+1}/{len(nfs_pendentes)}] Processando NF {nf.numero_nf} - Pedido: {nf.origem}")
    ...
    def _buscar_nfs_pendentes(self) -> List[RelatorioFaturamentoImportado]:
        ...
        return RelatorioFaturamentoImportado.query.filter(
    ...
    def _buscar_embarque_ativo_por_pedido(self, num_pedido: str) -> Optional[EmbarqueItem]:
        ...
        """
        return EmbarqueItem.query.join(
    ...
    def _avaliar_score_e_gerar_inconsistencia(self, nf: RelatorioFaturamentoImportado, 
    ...
    def _avaliar_score_completo(self, nf: RelatorioFaturamentoImportado, usuario: str) -> bool:
        ...
        embarques_pedido = EmbarqueItem.query.filter_by(
            pedido=nf.origem
        ).all()
    ...
    def _calcular_divergencia_lote(self, nf: RelatorioFaturamentoImportado, 
                                   separacoes: List[Separacao]) -> bool:
        """
        ...
        for prod_nf in produtos_nf:
            sep_correspondente = next(
                (sep for sep in separacoes if str(sep.cod_produto).strip() == str(prod_nf.cod_produto).strip()), 
                None
            )
    ...
    def _avaliar_melhor_lote_por_score(self, nf: RelatorioFaturamentoImportado, 
                                      embarques_pedido: List[EmbarqueItem], 
                                      usuario: str) -> bool:
        """
        ...
        for lote_id in lotes_unicos:
            ...
            for prod_nf in produtos_nf:
                sep_correspondente = next(
                    (sep for sep in separacoes_lote if str(sep.cod_produto).strip() == str(prod_nf.cod_produto).strip()), 
                    None
                )
    ...
    def _criar_movimentacao_com_lote(self, nf: RelatorioFaturamentoImportado, 
                                    lote_id: str, usuario: str):
        """
    ...
    def _criar_justificativa_divergencia(self, nf: RelatorioFaturamentoImportado,
                                       lote_id: str, usuario: str):
        """
        ...
        for sep in separacoes:
            prod_corresp = next(
                (p for p in produtos if str(p.cod_produto).strip() == str(sep.cod_produto).strip()),
                None
            )
    ...
    def _gerar_inconsistencia_divergencia_embarque(self, nf: RelatorioFaturamentoImportado,
