...
|-------------|------|-----------------|----------|---------|----------|
| **V32** | 2026-05-18 (codigo) | — | — | TBD | V32 — fix nomes totalizadoras I050 (resolve TODO `V2` legado em `data.py:368` `nome = f'GRUPO {prefix}'`). `_gerar_hierarquia_sintetica` agora consulta `plano_analiticas` para cada `prefix` intermediario: se conta com aquele code esta cadastrada no Odoo CIEL IT, usa `name`/`account_type`/`create_date` reais; senao, fallback generico atual. Tambem deduplica (code emitido como sintetica nao repete como analitica). Telemetria adicionada: log conta quantas sinteticas tem nome do Odoo vs fallback. Impacto IMEDIATO: zero se Odoo NACOM nao tem totalizadoras cadastradas (caso atual provavel — V31 mostra 100% "GRUPO X" para codes 2-9 digitos). Impacto FUTURO: ao cadastrar totalizadoras no Odoo, I050 ja passa a usar nomes reais sem nova alteracao de codigo. |
