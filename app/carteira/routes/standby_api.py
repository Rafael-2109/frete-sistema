...
def criar_standby():
    ...
    try:
        ...
        standby_existente = SaldoStandby.query.filter_by(
            num_pedido=num_pedido,
            status_standby__in=['ATIVO', 'BLOQ. COML.', 'SALDO']
        ).first()
        ...
        for item in itens_carteira:
            ...
            registros_criados.append({
                'cod_produto': item.cod_produto,
                'qtd': float(item.qtd_saldo_produto_pedido)
            })
...
def verificar_status_standby(num_pedido):
    ...
    try:
        standby = SaldoStandby.query.filter_by(
            num_pedido=num_pedido
        ).filter(
            SaldoStandby.status_standby.in_(['ATIVO', 'BLOQ. COML.', 'SALDO', 'CONFIRMADO'])
        ).first()
...
def atualizar_status_standby():
    ...
    try:
        ...
        itens_standby = SaldoStandby.query.filter_by(
            num_pedido=num_pedido
        ).all()
        ...
        for item in itens_standby:
            ...
            if novo_status == 'CONFIRMADO':
                ...
                item.resolvido_por = current_user.username if hasattr(current_user, 'username') else 'Sistema'
...
def listar_standby():
    ...
    try:
        ...
        for pedido in pedidos_standby:
            ...
            produtos = SaldoStandby.query.filter_by(
