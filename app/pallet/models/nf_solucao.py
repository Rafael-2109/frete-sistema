...
devolução, retorno ou cancelamento. Pertence ao Domínio B (Tratativa das NFs).
...
class PalletNFSolucao(db.Model):
    ...
    - RETORNO: NF de retorno emitida pelo cliente (1:1 com remessa)
    - CANCELAMENTO: NF foi cancelada (registro para auditoria)

    Vinculação pode ser:
    - AUTOMATICO: Sistema encontrou match automático
    ...
    # Informações complementares (para match automático de RETORNO)
    ...
    @staticmethod
    def criar_retorno(nf_remessa_id: int, quantidade: int, numero_nf: str,
                      data_nf, cnpj_emitente: str, nome_emitente: str,
                      usuario: str, vinculacao: str = 'MANUAL',
                      info_complementar: str = None, chave_nfe: str = None,
                      serie: str = None, observacao: str = None):
        """
        Factory method para criar solução tipo RETORNO.

        Args:
            nf_remessa_id: ID da NF de remessa
            quantidade: Quantidade retornada
            numero_nf: Número da NF de retorno
            data_nf: Data da NF
            cnpj_emitente: CNPJ do emitente
            nome_emitente: Nome do emitente
            usuario: Usuário que registrou
            vinculacao: Tipo de vinculação
            info_complementar: Informações complementares para match
            chave_nfe: Chave da NF-e (opcional)
            serie: Série da NF (opcional)
            observacao: Observações (opcional)
