...
def simulate_mcp_response(query):
    ...
    if "status" in query_lower or "sistema" in query_lower:
        return (
            """🟢 **SISTEMA DE FRETES ONLINE**

📊 **ESTATÍSTICAS ATUAIS:**
- Sistema operacional desde: """
...
def test_mcp_directly():
    ...
    try:
        ...
        if tool_name == "status_sistema":
            ...
            return jsonify({"success": False, "error": f"Ferramenta não suportada: {tool_name}"}), 400
...
def redis_status():
    ...
    if not REDIS_DISPONIVEL:
        return jsonify({"disponivel": False, "erro": "Redis não está instalado ou configurado", "status": "❌ Offline"})
    ...
    try:
        ...
        return jsonify({"disponivel": False, "erro": str(e), "status": "❌ Erro"})
...
def context_status():
    ...
    try:
        ...
        if not context_manager:
            return jsonify({"success": False, "error": "Sistema de contexto não disponível"})

        user_id = str(current_user.id)
        summary = context_manager.get_context_summary(user_id)

        return jsonify({"success": True, "context_summary": summary, "user_id": user_id})
...
def api_query():
    ...
    try:
        ...
        # ✅ VALIDAR ENTRADA
        valid, error_msg, validated_data = InputValidator.validate_json_request(data, required_fields=["query"])
        if not valid:
...
def get_suggestions():
    ...
    try:
        ...
        try:
            ...
            return jsonify({"success": False, "error": "Sistema de sugestões não disponível", "suggestions": []})
        ...
        if not suggestion_engine:
            return jsonify({"success": False, "error": "Engine de sugestões não inicializado", "suggestions": []})
        ...
        suggestions = suggestion_engine.get_intelligent_suggestions(user_context, conversation_context)
...
def suggestion_feedback():
    ...
    try:
        ...
        if not suggestion_text:
            return jsonify({"success": False, "error": "Texto da sugestão é obrigatório"})
        ...
        try:
            ...
            if suggestion_engine:
                ...
                if hasattr(suggestion_engine, "learn_from_interaction"):
                    suggestion_engine.learn_from_interaction(user_context, suggestion_text, was_helpful)
                ...
                return jsonify({"success": False, "error": "Engine de sugestões não disponível"})

        except ImportError:
            return jsonify({"success": False, "error": "Sistema de sugestões não disponível"})
...
def suggestions_dashboard():
    ...
    try:
        ...
        stats = {"total_suggestions": 0, "categories": {}, "user_interactions": 0, "feedback_positive": 0}
        ...
        return render_template("claude_ai/suggestions_dashboard.html", stats=stats, user=current_user)
...
def dashboard_executivo():
    ...
    return render_template(
        "claude_ai/dashboard_executivo.html", momento_atual=datetime.now().strftime("%d/%m/%Y %H:%M:%S")
    )
...
def api_dashboard_kpis():
    ...
    try:
        ...
        entregas_hoje = (
            ...
            .filter(db.func.date(EntregaMonitorada.data_hora_entrega_realizada) == hoje)
            .count()
        )
        ...
        entregas_atrasadas = (
            db.session.query(EntregaMonitorada)
            .filter(EntregaMonitorada.data_entrega_prevista < hoje, EntregaMonitorada.entregue == False)
            .count()
        )

        pendencias_financeiras = (
            db.session.query(EntregaMonitorada).filter(EntregaMonitorada.pendencia_financeira == True).count()
        )
        ...
        total_entregas_30d = (
            db.session.query(EntregaMonitorada).filter(EntregaMonitorada.data_embarque >= data_limite_30d).count()
        )

        entregas_no_prazo_30d = (
            db.session.query(EntregaMonitorada)
            .filter(
        ...
        performance_geral = (
            round((entregas_no_prazo_30d / total_entregas_30d * 100), 1) if total_entregas_30d > 0 else 0
        )
...
def api_dashboard_graficos():
    ...
    try:
        ...
        for i in range(14, -1, -1):
            ...
            entregas_dia = (
                db.session.query(EntregaMonitorada)
                .filter(db.func.date(EntregaMonitorada.data_hora_entrega_realizada) == data_analise)
                .count()
            )
        ...
        top_clientes = (
            db.session.query(EntregaMonitorada.cliente, db.func.count(EntregaMonitorada.id).label("total_entregas"))
            .filter(EntregaMonitorada.data_embarque >= data_limite)
            .group_by(EntregaMonitorada.cliente)
            .order_by(db.func.count(EntregaMonitorada.id).desc())
            .limit(5)
            .all()
        )

        top_clientes_data = [{"nome": cliente.cliente, "entregas": cliente.total_entregas} for cliente in top_clientes]

        return jsonify({"labels": labels, "entregas": dados_grafico, "top_clientes": top_clientes_data})
...
def api_relatorio_automatizado():
    ...
    try:
        ...
        return jsonify({"success": True, "relatorio": relatorio, "gerado_em": datetime.now().isoformat()})
...
def processar_comando_excel():
    ...
    try:
        ...
        # ✅ VALIDAR ENTRADA
        valid, error_msg, validated_data = InputValidator.validate_json_request(data, required_fields=["command"])
        if not valid:
            return jsonify({"success": False, "error": error_msg}), 400
        ...
        if client:
            ...
            if not valid:
                return jsonify({"success": False, "error": f"Cliente inválido: {msg}"}), 400
        ...
        if any(
            palavra in command for palavra in ["entregas pendentes", "pendente", "não entregue", "aguardando entrega"]
        ):
        ...
        if resultado and resultado.get("success"):
            ...
            is_cliente = any(
                cliente in filename.lower() for cliente in ["assai", "atacadao", "carrefour", "tenda", "mateus", "fort"]
            )
...
- Análise categórica por status de entrega"""
            ...
            return jsonify({"success": True, "resposta_formatada": resposta_claude, "arquivo_info": resultado})
        else:
            return jsonify(
                {
                    "success": False,
        ...
        return (
            jsonify(
                {"success": False, "error": str(e), "resposta_formatada": f"❌ Erro interno ao gerar Excel: {str(e)}"}
            ),
            500,
        )
...
def api_advanced_query():
    ...
    try:
                ...
                if loop.is_running():
                    ...
                    with concurrent.futures.ThreadPoolExecutor() as executor:
                        future = executor.submit(
                            asyncio.run, advanced_ai.process_advanced_query(consulta, user_context)
                        )
                ...
                jsonify(
                    {
                        "success": False,
                        "error": (
                            result.get("error", "Erro no processamento avançado")
        ...
        return jsonify({"success": False, "error": f"Erro interno: {str(e)}", "fallback_available": True}), 500
...
def api_advanced_feedback():
    ...
    try:
        ...
        if not session_id or not feedback_text:
            return jsonify({"success": False, "error": "session_id e feedback são obrigatórios"}), 400
        ...
        try:
            if asyncio.iscoroutinefunction(advanced_ai.capture_advanced_feedback):
                feedback_result = asyncio.run(
                    advanced_ai.capture_advanced_feedback(session_id, query, response, feedback_text, feedback_type)
                )
            else:
        ...
        try:
            ...
            analytics["database_metrics"] = {
                ...
                "feedback_distribution": [
                    {"type": row[0], "severity": row[1], "count": row[2], "avg_rating": float(row[3]) if row[3] else 0}
...
def advanced_dashboard():
    ...
    return render_template("claude_ai/advanced_dashboard.html", user=current_user, titulo="Dashboard Avançado de IA")
...
def advanced_feedback_interface():
    ...
    return render_template("claude_ai/advanced_feedback.html", user=current_user, titulo="Feedback Avançado")
    ...
    try:
        ...
        if critical_count > 0:
            ...
            health_status["recommendations"].append(
                f"{critical_count} componente(s) crítico(s) requer(em) atenção imediata"
            )
        elif degraded_count > 0:
            health_status["overall_status"] = "degraded"
            health_status["recommendations"].append(
                f"{degraded_count} componente(s) degradado(s) - funcionalidade reduzida"
            )
            ...
            jsonify(
                {
                    ...
                    "health_status": {"overall_status": "critical", "error": str(e)},
                }
            ),
            500,
        )
...
def ler_arquivo():
    ...
    try:
        ...
        return jsonify({"status": "success", "arquivo": caminho_arquivo, "conteudo": conteudo, "info": info_arquivo})
        ...
        for item in target_dir.iterdir():
            if item.is_dir() and not item.name.startswith("."):
                ...
                conteudo["arquivos"].append(
                    {"nome": item.name, "tamanho_kb": round(item.stat().st_size / 1024, 2), "extensao": item.suffix}
                )

        return jsonify({"status": "success", "conteudo": conteudo})
...
def inspecionar_banco():
    ...
    try:
        ...
        esquema_completo = {
            "info_banco": {"dialeto": str(db.engine.dialect.name), "driver": str(db.engine.driver)},
            "tabelas": {},
        }
...
def aprovar_acao_seguranca(action_id):
    ...
    try:
        ...
        success, message = security_guard.approve_action(action_id, aprovado, current_user.nome, motivo)
...
def listar_acoes_pendentes():
    ...
    try:
        ...
        return jsonify({"status": "success", "actions": actions, "count": len(actions)})
...
def lockdown_emergencia():
    ...
    try:
        ...
        return jsonify({"status": "success", "message": f"🚨 Lockdown de emergência ativado por {current_user.nome}"})
...
def dev_ai_analyze_project():
    ...
    try:
        ...
        return jsonify({"status": "success", "analysis": analysis, "message": "Análise completa do projeto finalizada"})
...
def dev_ai_analyze_file_v2():
    ...
    try:
        ...
        return jsonify(
            {"status": "success", "analysis": analysis, "message": f"Análise do arquivo {file_path} finalizada"}
        )
...
def dev_ai_modify_file_v2():
    ...
    try:
        ...
        if not file_path or not modification_type:
            return (
                jsonify({"status": "error", "message": "Caminho do arquivo e tipo de modificação são obrigatórios"}),
                400,
            )
...
def dev_ai_analyze_and_suggest():
    ...
    try:
        ...
        return jsonify({"status": "success", "result": result, "message": "Análise e sugestão finalizada"})
...
def dev_ai_generate_documentation():
    ...
    try:
        ...
        return jsonify({"status": "success", "documentation": result, "message": f"Documentação gerada para: {target}"})
...
def dev_ai_capabilities_v2():
    ...
    try:
        ...
        return jsonify(
            {"status": "success", "capabilities": capabilities, "message": "Capacidades do Claude Development AI"}
        )
...
def admin_free_mode_status():
    ...
    try:
        ...
        return jsonify({"success": True, "dashboard_data": dashboard_data, "timestamp": datetime.now().isoformat()})
...
def admin_free_mode_dashboard():
    ...
    try:
        ...
        return render_template("claude_ai/admin_free_mode_dashboard.html", dashboard_data=dashboard_data)
...
def claude_real_free_mode():
    ...
    try:
        ...
        free_mode.log_admin_action(
            "claude_real_free_query",
            {
...
def true_autonomy_status():
    ...
    try:
        ...
        return jsonify({"success": True, "autonomous_data": dashboard_data, "timestamp": datetime.now().isoformat()})
...
def true_autonomy_dashboard():
    ...
    try:
        ...
        return render_template("claude_ai/true_autonomy_dashboard.html", dashboard_data=dashboard_data)
...
def diagnostico_sistema_novo():
    ...
    try:
        ...
        return render_template("claude_ai/diagnostico_sistema_novo.html", diagnostico=diagnostico, user=current_user)

    except Exception as e:
        logger.error(f"❌ Erro no diagnóstico: {e}")
        return jsonify({"erro": str(e), "status": "erro_critico"}), 500
...
def api_forcar_sistema_novo():
    ...
    try:
        ...
        return jsonify({"success": True, "sistema_ativo": transition.sistema_ativo, "diagnostico": diagnostico})
...
def api_claude_ai_novo_metrics():
    ...
    try:
        ...
        interface_metrics = {
            ...
            "ai_system": {
                "modules_loaded": comprehensive_metrics.get("ai_technical", {})
                .get("ai_status", {})
                .get("total_modules_loaded", 0),
                ...
                "api_key_configured": comprehensive_metrics.get("ai_technical", {})
                .get("ai_status", {})
                .get("api_key_configured", False),
                "unlimited_mode": comprehensive_metrics.get("ai_technical", {})
                .get("ai_status", {})
                .get("unlimited_mode", False),
                "learning_enabled": comprehensive_metrics.get("ai_technical", {})
                .get("ai_status", {})
        ...
        return jsonify({"success": True, "metrics": interface_metrics, "source": "claude_ai_novo"})

    except Exception as e:
        logger.error(f"❌ Erro ao gerar métricas do Claude AI Novo: {e}")
        return jsonify({"success": False, "error": f"Erro ao coletar métricas: {str(e)}"}), 500
