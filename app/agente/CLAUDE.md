...
| **Historico do SDK** — features por versao, breaking changes, bug fixes (0.1.49 → 0.2.110) | `SDK_CHANGELOG.md` |
...
├── SDK_CHANGELOG.md                 # Historico SDK 0.1.49 -> 0.2.110 (features, breaking, fixes) — inclui revisao retroativa do 0.2.82 (2 breakings omitidas)
...
- **claude-agent-sdk**: `0.2.110` | **CLI bundled**: 2.1.191 (suporta `claude-fable-5`/`claude-sonnet-5`) | **anthropic**: `0.115.0` | **mcp**: `>=1.26.0,<2.0.0` (teto `<2.0.0` exigido pelo claude-agent-sdk 0.2.96+; floor herda fix `CallToolResult` perdido em 0.1.70; SDK 0.2.110 exige `mcp>=1.23.0`). Bump 0.2.101→0.2.110 (2026-06-30): CLI bundled 2.1.177→2.1.191, SO bumps de CLI, ZERO breaking de API pública. anthropic 0.109.1→0.115.0: suporte tipado a `claude-sonnet-5` (0.114); sem breaking nos padrões usados (`Anthropic()`/`messages.create`/`.parse`/`APIStatusError`/`APIError`).
- **Modelo default (web)**: `claude-opus-4-8` ($5/$25 per MTok, adaptive thinking, 1M context); configurável pelo usuário e **estável dentro da sessão** — o modelo é decidido 1x por sessão (routing só na 1ª msg) e só troca por escolha EXPLÍCITA do usuário no seletor. Bug 2026-06-15: o smart routing rebaixava Opus→Sonnet mid-sessão (em follow-ups curtos), matando o cache MODEL-SCOPED e confundindo o modelo rebaixado; fix via `pick_warm_model`/`should_switch_model` (`sdk/model_router.py`) + `PooledClient.model`. **Teams roda Sonnet FIXO + `TEAMS_EFFORT_LEVEL=high`** — modelo migrado p/ `claude-sonnet-5` no default do código (2026-06-30); ⚠️ a env var `TEAMS_DEFAULT_MODEL` no Render **sobrepõe** o default e precisa ser atualizada p/ efetivar (ver `app/teams/CLAUDE.md`). **Rollback web**: `AGENT_MODEL=claude-opus-4-7`.
...
> **Historico completo (adocoes, breaking changes, bug fixes, features NAO adotadas — 0.1.49 → 0.2.110 + anthropic 0.85 → 0.115.0, com fluxos e arquitetura)**: `SDK_CHANGELOG.md`. Versoes tambem em `.claude/references/BEST_PRACTICES_2026.md` e `MCP_CAPABILITIES_2026.md` (manter os 3 sincronizados ao bumpar).
