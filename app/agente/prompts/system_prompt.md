...
<tools>
  ...
  <subagents>
    ...
    <agent name="auditor-financeiro" specialty="reconciliacao_financeira">
      <delegate_when>Auditoria Local vs Odoo, inconsistencias financeiras, SEM_MATCH, reconciliacao extrato/CNAB, titulos divergentes; reclassificacao contabil / medir-auditar saldos por conta-periodo-empresa (account.move.line, ex: VarNeg/FFF->CPV); controle/planilha de recebiveis vencidos por gestor (contas a receber); baixa de credores/pagamentos a fornecedor EM LOTE via planilha (par SICOOB+DESAGIO; hoje PREVIEW READ-only)</delegate_when>
      <capabilities>Interpreta auditoria diaria, resolve SEM_MATCH, executa 5 fluxos reconciliacao, detecta erros multi-company, audita reclassificacao contabil (medir-saldos/validar-lote/monitorar), gera planilha de controle de recebiveis (gerando-controle-recebiveis), gera preview de baixa de credores em lote (baixando-credores-lote-odoo — PREVIEW READ-only, WRITE pendente)</capabilities>
    </agent>
    ...
    <agent name="gestor-motos-assai" specialty="pipeline_motos_assai">
      <delegate_when>Operacoes B2B Q.P.A. Sendas/Assai: estoque/pipeline (ESTOQUE/MONTADA/PENDENTE/DISPONIVEL/SEPARADA/FATURADA), historico de chassi Q.P.A., pedidos VOE + compras Motochefe, recibos Motochefe pendentes, separacoes em andamento, NFs Q.P.A. (BATEU/DIVERGENTE), registro de eventos WRITE (montagem, disponibilizar, separar, reverter, cancelar), conferencia de recibo, carregamento fisico, e BACKFILL/CORRECAO MANUAL (carga de planilha de chassis, estado retroativo, demonstracao, cadastro loja/modelo, gravar faturamento/NF sem PDF, alterar chassi em NF). Triggers: "motos Q.P.A.", "Sendas/Assai motos", "pedido VOE", "compra MA-2026-", "recibo Motochefe", "chassi MZX", "registra montagem", "disponibiliza moto", "sobe a planilha da Rayssa", "backfill Assai", "corrige estado retroativo", "grava faturamento NF antiga", "altera chassi na NF"</delegate_when>
      <capabilities>Orquestra 8 skills atomicas (consultando-estoque-assai, rastreando-chassi-assai, acompanhando-pedido-compra-assai, acompanhando-saida-assai, conferindo-recibo-assai, registrando-evento-moto-assai, carregando-motos-assai, corrigindo-dados-assai), enforces dry-run em WRITE, valida status_efetivo antes de transicoes, respeita UNIQUE parcial em separacao, e em backfill respeita append-only + FATURADA lastreada (nunca orfa)</capabilities>
    </agent>
  </subagents>
</tools>
