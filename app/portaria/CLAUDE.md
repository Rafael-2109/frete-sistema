...
**Companheiro — `embarques_com_saida_pendente_query(local_cd=None)`**: mesma raiz de bug, superficie DIFERENTE (a LISTAGEM de embarques, `embarques.listar_embarques`, nao a portaria fisica). Mais permissivo que o helper acima: alem do criterio por portaria, mantem o embarque pendente quando `data_embarque IS NULL` (legado-safe — embarque single-CD com `data_embarque` setado SEM ControlePortaria, ex. saida manual, NAO reaparece; so embarque MISTO em saida parcial e resgatado). `local_cd=None` = pendente de QUALQUER CD. Bug corrigido 2026-06-23: embarque misto sumia da lista de pendentes (e do filtro por CD) apos a saida de 1 CD. **Gotcha SQLAlchemy (CADA subquery com EmbarqueItem no FROM precisa de `.correlate()` EXPLICITO)**: (a) a subquery saida-por-item esta tripla-aninhada → `.correlate(Embarque, EmbarqueItem)`, senao a auto-correlacao re-introduz `embarques` no FROM (produto cartesiano) e decorrelaciona o resultado; (b) as subqueries `item_cd_pendente` e `tem_item_do_local` (com EmbarqueItem no proprio FROM) exigem `.correlate(Embarque)` porque o consumidor `embarques.listar_embarques` faz `.outerjoin(EmbarqueItem)` — sem o correlate, a auto-correlacao arrasta o EmbarqueItem dessas subqueries para o FROM externo, deixando-as "sem FROM" e estourando `InvalidRequestError` no `.paginate()` (regressao 500 do mesmo dia 2026-06-23 — o `.count()` nao reproduzia porque so o `paginate`/`outerjoin` da rota dispara). Regressao coberta por `tests/embarques/test_listar_pendentes_correlate.py`.
