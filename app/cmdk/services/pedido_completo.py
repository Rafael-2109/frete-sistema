...
def montar_contexto(num_pedido: str) -> Optional[dict]:
    ...
    # ----------------------------------------------------------- itens & totais
    itens, totais = _montar_itens(itens_orm)
...
def _montar_itens(itens_orm) -> tuple[list[dict], dict]:
    """Monta lista de itens com totais agregados."""
    itens: list[dict] = []
    total_valor = 0.0
    ...
    for item in itens_orm:
        ...
        valor_saldo = qtd_saldo * preco

        try:
            peso, pallet = calcular_peso_pallet_produto(item.cod_produto, qtd_saldo)
        except Exception:
            peso, pallet = 0.0, 0.0
...
def _carregar_separacoes(num_pedido: str) -> list[dict]:
    ...
    LOTE_ORFAO_<id> e logadas como warning — nunca sao silenciosamente perdidas
    (R8: visibilidade de dados).
    ...
    for sep in seps:
        ...
        if not lote_id:
            # Fallback para separacao orfa — nao perder dado, log warn
        ...
        lote = lotes[lote_id]
        lote['qtd_itens'] += 1
        # H2 fix: valor_saldo ja eh o valor TOTAL da linha (qtd * preco), nao unitario
        try:
            lote['valor_total'] += float(sep.valor_saldo or 0)
        except (TypeError, ValueError):
            pass
        try:
            lote['peso_total'] += float(getattr(sep, 'peso', 0) or 0)
        except (TypeError, ValueError):
            pass
