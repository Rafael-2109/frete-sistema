...
`GET|POST /embarques/<id>` e o hub (alvo de redirects de cotacao/portaria/save). Logo no inicio (GET **e** POST) chama `sincronizar_totais_embarque(embarque)`, que faz **`commit()`** — ou seja, **abrir a tela reescreve dados no banco**. Nunca tratar como leitura pura. A EDICAO real do embarque acontece no POST `action='save'` DESTE endpoint — NAO em `editar_embarque` (cujo POST so faz `flash('Erro inesperado')`, nome enganoso). `listar_embarques` tambem tem efeito destrutivo: DELETA rascunhos `status='draft'` sem itens e commita. **Pendencia 2CD-aware**: o pre-filtro padrao (pendentes, `mostrar_todos=false`) NAO usa mais `data_embarque IS NULL` — delega a `ControlePortaria.embarques_com_saida_pendente_query(local_cd)` (FONTE), que mantem o embarque MISTO na lista enquanto QUALQUER CD nao deu saida, mesmo apos a 1a saida carimbar o cabecalho agregado. O filtro `?local_cd=` ja vem embutido nesse helper no modo pendentes; so e reaplicado a parte em `mostrar_todos=true` (bug corrigido 2026-06-23).
