...
class DatabaseScanner:
    ...
    def _get_postgresql_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            table_sizes = [{"table": row[0], "size_pretty": row[1], "size_bytes": row[2]} for row in result]

            # Estatísticas de índices
            index_stats = self.db.session.execute(
                text(
                    """
                SELECT 
                    schemaname,
            ...
            index_usage = [
                {"table": row[1], "index": row[2], "reads": row[3], "fetches": row[4]} for row in index_stats
            ]
    ...
    def _get_mysql_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            table_sizes = [{"table": row[0], "size_mb": row[1], "rows": row[2]} for row in result]

            return {"dialect": "mysql", "table_sizes": table_sizes, "total_tables": len(table_sizes)}
    ...
    def _get_sqlite_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            tables_info = [{"table": row[0], "create_sql": row[1]} for row in result]

            return {"dialect": "sqlite", "tables_info": tables_info, "total_tables": len(tables_info)}
    ...
    def _analyze_relationships(self, tables: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis["orphaned_tables"] = [table for table, count in connection_count.items() if count == 0]

            # Identificar tabelas altamente conectadas
            analysis["highly_connected_tables"] = [
                {"table": table, "connections": count} for table, count in connection_count.items() if count > 3
            ]

            analysis["connection_summary"] = {
                "total_connections": len(analysis["relationships_map"]),
                "avg_connections_per_table": (
                    round(sum(connection_count.values()) / len(connection_count), 2) if connection_count else 0
                ),
            }
    ...
    def _get_mysql_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"threads_connected": result[1] if result else 0}
    ...
    def obter_estatisticas_gerais(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"error": str(e), "total_tabelas": 0, "tabelas": []}
