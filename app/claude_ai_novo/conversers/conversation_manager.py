...
class ConversationManager:
    ...
    def set_memorizer(self, memorizer):
        ...
        try:
            ...
            if not hasattr(memorizer, "context_memory") and not hasattr(memorizer, "conversation_memory"):
                self.logger.warning(
                    "⚠️ Memorizer não tem context_memory ou conversation_memory, mantendo configuração padrão"
                )
    ...
    def start_conversation(
        self, user_id: str, initial_message: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None
    ) -> str:
        """
    ...
    def manage_conversation(
        ...
        """
        Gerencia uma conversa completa.
        ...
        Returns:
            Resultado da gestão da conversa
        """
        try:
            # Verificar se conversa existe
            if session_id not in self.active_conversations:
                # Iniciar nova conversa
                self.start_conversation(session_id, initial_message=query)

            # Adicionar mensagem do usuário
            self.add_user_message(session_id, query, context)

            # Processar conversa (simulado)
            response = f"Processando conversa para sessão {session_id}: {query[:50]}..."
            ...
            return {
                ...
                "session_id": session_id,
                "response": response,
                "message_count": self.active_conversations.get(session_id, {}).get("message_count", 0),
            }

        except Exception as e:
            self.logger.error(f"❌ Erro ao gerenciar conversa: {e}")
