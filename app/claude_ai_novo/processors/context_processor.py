...
class ContextProcessor(ProcessorBase):
    ...
    def _build_contexto_por_intencao(self, intencoes_scores: Dict[str, float], analise: Dict[str, Any]) -> str:
        """
        ...
        try:
            ...
            # Se confiança baixa, usar contexto genérico mas informar limitação
            if score_principal < 0.6:  # Aumentar threshold para ser mais rigoroso
                result = (
                    f"⚠️ Confiança baixa ({score_principal:.1%}) - dados limitados disponíveis. "
                    + self._descrever_contexto_carregado(analise)
                )
...
Módulos: pedidos, fretes, embarques, monitoramento, separacao, carteira, etc."""
                ...
                registros = (
                    self._ultimo_contexto_carregado.get("registros_carregados", 0)
                    if hasattr(self, "_ultimo_contexto_carregado")
                    else 0
                )
