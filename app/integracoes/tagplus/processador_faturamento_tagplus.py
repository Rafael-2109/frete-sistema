...
class ProcessadorFaturamentoTagPlus:
        ...
        try:
            ...
            if erros_etapa:
                self.inconsistencias.append({
                    'nf': faturamento_produto.numero_nf,
                    'erros_parciais': erros_etapa,
                    'processada': True
                })
    ...
    def _encontrar_separacao_por_score(self, faturamento):
        ...
        try:
            ...
            embarque_items = EmbarqueItem.query.join(
                ...
                CarteiraCopia.cnpj_cpf.contains(faturamento.cnpj_cliente.replace('.', '').replace('-', '').replace('/', '')),
            ...
            if not embarque_items:
                logger.info(f"Nenhum EmbarqueItem ativo com erro_validacao encontrado para CNPJ {faturamento.cnpj_cliente}")
                return None
            
            logger.info(f"Encontrados {len(embarque_items)} EmbarqueItems candidatos para CNPJ {faturamento.cnpj_cliente}")
            ...
            if melhor_score > 0 and melhor_match:
                logger.info(f"Melhor match encontrado: separacao_lote_id={melhor_match.separacao_lote_id}, score={melhor_score}")
                return melhor_match
            else:
    ...
    def _sincronizar_carteira_principal(self, num_pedido, cod_produto):
        ...
        try:
            # Busca CarteiraCopia
            carteira_copia = CarteiraCopia.query.filter_by(
    ...
    def processar_lote_nfs(self):
        ...
        try:
            ...
            nfs_pendentes = FaturamentoProduto.query.filter(
    ...
    def processar_nf_completo(self, faturamento_produto):
        ...
        """
        try:
            logger.info(f"🚀 Processamento completo item TagPlus: NF {faturamento_produto.numero_nf} - Produto {faturamento_produto.cod_produto}")
            ...
            if faturamento_produto.numero_nf not in self._nfs_sincronizacoes_completas:
                ...
                try:
                    ...
                    sucesso, resultado = processar_lancamento_automatico_fretes(
                        cnpj_cliente=faturamento_produto.cnpj_cliente,
    ...
    def processar_lote_completo(self, nfs_produtos=None):
        ...
        try:
            ...
            if nfs_produtos is None:
                nfs_produtos = FaturamentoProduto.query.filter(
                    FaturamentoProduto.created_by == 'ImportTagPlus',
                    FaturamentoProduto.status_nf == 'Lançado'
                ).filter(
                    ~FaturamentoProduto.numero_nf.in_(
