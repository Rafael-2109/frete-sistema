...
class AtacadaoPedidoExtractor(PDFExtractor):
    ...
    def _coletar_identificadores(self, filiais: List[str]):
        ...
        for filial_text in filiais:
            # CNPJ: formato "Local de Entrega: 93209765/0599-44"
            cnpj_match = re.search(
                r'Local\s+de\s+Entrega:\s*(\d{8}/\d{4}-\d{2})',
                filial_text, re.IGNORECASE
            )
            if cnpj_match:
                cnpj_formatado = self._format_cnpj(cnpj_match.group(1))
                cnpjs.add(cnpj_formatado)

            # Códigos de produto: formato "XXXXX/YYY CXA"
            for m in re.finditer(r'(\d{5,6})/\d+\s+CXA', filial_text):
    ...
    def _preload_depara(self, codigos: List[str]):
        ...
        try:
            ...
            # Inclui versão normalizada (sem zeros à esquerda) para cobrir ambos formatos
