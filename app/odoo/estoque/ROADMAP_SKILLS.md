...
## SKILL 6 — `planejando-pre-etapa-odoo`  🟡 (mín viável + 19 pytest verdes + 3 smokes CLI)
...
- **Checkpoints:** C1 ✅ (3 scripts-fonte + service + 13 testes lidos integral) · C2 ✅ ([`scripts/pre_etapa.py`](scripts/pre_etapa.py) capinado + estendido; **19 testes pytest verdes** — 13 originais preservados via shim + 6 novos cobrindo helpers I/O) · C3 ✅ (contrato 4 modos em SKILL.md) · C4 ✅ ([`SKILL.md`](../../.claude/skills/planejando-pre-etapa-odoo/SKILL.md)) · C5 ✅ ([`planejar_pre_etapa.py`](../../.claude/skills/planejando-pre-etapa-odoo/scripts/planejar_pre_etapa.py) — CLI 4 modos, --dry-run default em writes, exit codes 0/1/2/4) · **C6 ✅** (3 smokes CLI: FALHA_INPUT_AUSENTE exit 1, FALHA_USO exit 2, DRY_RUN_OK inputs vazios exit 4; log `/tmp/log_skill6_C6_validacao_dry_run.json`; limitações documentadas: listar-onda PG + batch real Odoo = sessão futura) · C7 ✅ (subagente + ROUTING_SKILLS 47→48 + 15→16 Skills Odoo + tool_skill_mapper + CLAUDE.md módulo) · C8 ✅ ([fluxo 4.1](fluxos/4.1-pre-etapa-cd-d007.md) com 4 sub-casos a/b/c/d) · C9 ✅ (2 scripts SUPERADOS em `_validados/planejando-pre-etapa-odoo/`: 03b + 04b + VALIDACAO.md; 09b permanece VIVO) · C10 ✅ (MAPA_SCRIPTS + este ROADMAP atualizados).
- **Status global da skill 6: 🟡 mín viável** (2026-05-24 v6 — 4 modos prontos; 0 execuções `--confirmar` em PROD nesta sessão; pattern original ja rodou em PROD via 03b/04b em sessões anteriores). [VALIDACAO.md](../../scripts/inventario_2026_05/_validados/planejando-pre-etapa-odoo/VALIDACAO.md).
