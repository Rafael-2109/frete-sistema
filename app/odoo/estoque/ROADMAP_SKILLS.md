...
| **Categoria INFRA-ODOO `provisioning/` (NOVO 2026-06-15 — fora do sistema de skills, §1.1+CLAUDE.md:410-412)** | 🟡 **framework testado + bodies versionados** — `sa_retorno_industrializacao` (provisionador idempotente SA+cron por `name` + `verificar` READ; bodies G1 montagem [genealogia safe_eval ✅ **VALIDADA** canary `s71` vs oráculo] / G2 transmissão [fiel s54 provado]) + `monitor_sa_industrializacao` (D8 grava flag + hook SessionStart lê). `dry_run` default; crons gated por canary. 16 pytest. **Canary G1 estágios 0+1 ✅ (15/06, SA efêmera, ZERO NF, 2 bugs fix); estágio 2 montagem→draft = go fresco.** | `provisioning/` + `.claude/hooks/sessionstart_industrializacao_sa.py` + `scripts/s71_canary_g1_genealogia.py` |
