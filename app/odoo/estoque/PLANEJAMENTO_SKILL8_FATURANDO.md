...
| Campo | Valor |
|-------|-------|
| **Status global** | 🟡 ORCHESTRATOR BASE PROD-GRADE v15c (C6/C7/C8/F5c + F1 idempotencia + 15 fixes 4 reviewers paralelos; ETAPAS C/D/E/F stubs v16/v17 com refs explicitas) |
| **Sessao atual** | v15c (2026-05-25) — **15 FIXES PROD-GRADE APLICADOS apos 4 code-reviewers paralelos (Conformidade Arquitetural + Aproveitamento de Recursos + Fluxos+Gotchas + Robustez Operacional)**: F1 idempotencia F5a no atomo Skill 5 (CR-CRIT 95 anti-duplicacao SEFAZ — pattern espelha `criar_picking_entrada_destino_manual` v15a) + F2 abort chunk se commit_resilient falha + F3 G014 docstring warning + F4 D11 barreira MACRO explicita em executar_pipeline_bulk + F5 ETAPA A raise NotImplementedError em real-run sem flag explicita + F6 safe_session_get re-fetch apos commit_resilient (anti-DetachedInstance) + F7 D10 dispose macro stubs + F8 ACAO_PARA_DIRECAO + ACAO_PARA_CFOP_ENTRADA + ACOES_ENTRADA_FB consolidadas em `operacoes_fiscais.py` + F9 `app/odoo/estoque/scripts/_commit_helpers.py` consolidado (SSL match tightened: lista especifica em vez de BROAD 'connection') + F10 4 constantes ETAPA F importadas + ref stub F + F11 PAYMENT_PROVIDER ref stub C + F12 external_id_operacao populado em F5a/F5b/F5c + F14 contadores estruturados (proxy Skill 6 v9) + F15 EXECUTADO_AUTO_CORRIGIDO distinction + F16 Semaphore intra-picking analise (atomo Skill 5 NAO tem — TODO v16). **472 pytest verdes** (+7 v15c). Smoke dry-run PROD re-validado (cod 210639522 DRY_RUN_OK em 2119ms). MIN seguranca SEFAZ alcancado (F1+F2 garantem zero NF duplicada em re-execucao apos SSL drop). |
| **Sessoes estimadas** | 5-6 sessoes restantes (v16 → v20+) — F5d/F5e/E/F + recovery + canary + bulk PROD |
| **Baseline pytest atual** | 472 verdes (tests/odoo/ — v15c em 15.27s; 435 baseline v15a + 30 v15b + 7 v15c) |
