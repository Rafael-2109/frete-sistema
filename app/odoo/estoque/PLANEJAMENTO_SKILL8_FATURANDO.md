...
| **Sessao atual** | v22+ (2026-05-27 commit `7f6eb196`) — **G-AUDIT-3 fix Skill 5 + Sub-skill C5 G038 (l10n_br_origem) + Caminho B FLUXO L3 1.2.x VALIDADO PARCIAL PROD pela 1a vez**. SEFAZ autorizou chave `35260561724241000178550010000945661007164482` (invoice 716448 RPI/2026/00238). DFe 43533 criado no LF via criar_dfe_a_partir_do_invoice_saida + PO C2619591 (id=42419) com order_line OK + workaround team 143 destravou state='purchase' + button_approve gerou picking 321617. **Descobertas v22+**: G039 (purchase.team gatekeeper LF — PO LF cai com team_id=41 Edilane → to_approve permanente via XML-RPC; solucao: criar purchase.team com user_id=user-execucao); G-PERM-1 (ir.rule record-level em dfe.line bloqueia Rafael apesar de ter grupos ir.model.access). Passo 9 (action_create_invoice) bloqueado por G-PERM-1 — pendencia v23+. **580 pytest verdes** (576 baseline v21+ + 4 net v22+ = 2 G-AUDIT-3 + 2 G038). Tasks v23+ documentadas (13-16). **Sessao anterior v19+ (2026-05-26)** — Skill 7 ABRANGENTE LIVE + Fluxos L3 1.2.1/1.2.2 + executar_fluxo_l3_1_2_x no orchestrator. AP1+AP3+AP4+AP5 ✅; AP2 RESOLVIDO PARCIAL v22+ (caminho B validado); AP6 pendente v23+. Sessao anterior v18 (2026-05-26) — RECOVERY + SKILL.md Skill 8 + G037 NOVO + 4 antipadroes documentados. | — **Skill 7 ABRANGENTE LIVE + Fluxos L3 1.2.1/1.2.2 + executar_fluxo_l3_1_2_x no orchestrator**. AP1+AP3+AP4+AP5 ✅; AP2 reclassificado com causa real; AP6 NOVO v20+. 554 pytest verdes. **Sessao anterior v18 (2026-05-26)** — RECOVERY + SKILL.md Skill 8 + G037 NOVO + 4 antipadroes documentados. C14: `executar_pipeline_resume(*, ciclo, apenas_etapa, max_iter, timeout_iter_s, detector_stagnation, ...)` em orchestrator (~280 LOC novas) — loop de `executar_pipeline_bulk(etapas=(apenas_etapa,))` ate' TUDO_OK / STAGNATION / MAX_ITER / EXCECAO. Substitui `fat_lf_resume.sh` + `fat_lf_resume_entrada.sh`. Constants: RESUME_MAX_ITER_DEFAULT=18, RESUME_TIMEOUT_ITER_S_DEFAULT=900, RESUME_ETAPAS_VALIDAS=('B','C','D','E','F'), FASES_TERMINAIS_B. `_contar_pendentes_por_etapa(etapa, ciclo, ...)` filtros por etapa cobre B/C/D/E/F + cross-ref RecebimentoLf para E. CLI estendido: `--modo {bulk,pre-flight,resume}` + `--apenas-etapa` + `--max-iter` + `--timeout-iter` + `--sem-stagnation`. C15: SKILL.md `.claude/skills/faturando-odoo/SKILL.md` (~430 LOC) com 4 receitas + TRADE-OFFS + secao ANTIPADROES DETECTADOS V17.5 — REFATOR V19+ + Checklist Expansao. G037 NOVO em `docs/inventario-2026-05/02-gotchas/G037-operacao-nao-cadastrada-exige-cfop-explicito.md` (G036 ja' ocupado por lote virgula). Referencia rapida em `.claude/references/odoo/GOTCHAS.md` nova secao tabela G011-G037. CLAUDE.md estoque §6.5 NOVO "ANTIPADROES DETECTADOS V17.5 — REFATOR V19+" (4 antipadroes + gotcha G037). 8 PYTEST novos em test_faturamento_pipeline_orchestrator.py cobrindo recovery. 1 code-reviewer paralelo (feature-dev) entregou 4 findings TODOS aplicados (F1 CRIT contador inclui F5x_FALHA, F3 HIGH lying parameter clarificado, F2+F4 HIGH doc TRADE-OFFS). **521 pytest verdes** (513 baseline + 8 net v18). Smokes dry-run PROD: resume B/D/E/F + FALHA_USO em `/tmp/log_skill8_smokes_v18_*.json`. |
| **Sessoes estimadas** | 1-2 sessoes (v20+) — canary REAL FLUXO L3 1.2.x + refator nomenclatura AP6 + remocao tampoes deprecated (criar_picking_entrada_destino_manual + V1 STRICT wrapper) |
| **Baseline pytest atual** | 555 verdes (tests/odoo/ — v19+ em 15.96s; 521 baseline v18 + 34 net v19+ = 22 Skill 7 ABRANGENTE + 7 Skill 5 S2 + 4 dispatch fluxo L3 + 1 timeout CR-v19+-MED-2) |
| **Baseline pytest pos-v22+ esperado** | ≥530 verdes (+~9 testes restantes FLUXOS L3 + canary REAL evidences) |
...
| **Destino do orchestrator** | `app/odoo/estoque/orchestrators/faturamento_pipeline.py` (LIVE v17.5) |
...
| **Skill 7 service (v17.5)** | `app/odoo/estoque/scripts/escrituracao.py` (~500 LOC) — atomo C3 macro `criar_recebimento_orchestrado(invoice_id, ajustes)` que encapsula RecebimentoLf + agg + svc externo. SKILL.md em `.claude/skills/escriturando-odoo/SKILL.md`. 10 pytest. |
| **Decisoes ABERTAS** | 0 (TODAS resolvidas v13-v18) — v18 Q1 recovery design = compor `executar_pipeline_bulk` em loop; v18 Q2 receitas SKILL.md = 4 (canary+bulk+canary F, resume D, resume E+F, pre-flight); v18 Q3 status reporting = rico; v18 Q4 G037 (renumerado de G036 ocupado). |
| **Checkpoints concluidos** | **18 de 24** (C1-C17 + C24 + **C18 v19+ folhas L3 1.2.1+1.2.2 + dispatch executar_fluxo_l3_1_2_x**). Pendentes: C19 cross-refs final, C20 canary REAL PROD FLUXO L3, C21 bulk REAL PROD, C22 code-review final v19+, C23 commit + arquivar `criar_picking_entrada_destino_manual` + ETAPAS E/F legacy. |
...
| **C11** | Capinar F5e (transmitir SEFAZ) + G016 SSL | metodo `executar_etapa_d` Playwright serial + idempotencia TRIPLA D8 + HARD_FAIL_CONFIG D7 + SNAPSHOT meta D5 + commit_resilient G016 + safe_session_get D9 + D18 2 niveis + MED C-1/C-2 | 5 pytest verdes (mockando Playwright); dry-run sem confirmar-sefaz OK | v17 | ✅ | **COMPLETO v17 2026-05-25** — substitui stub NOT_IMPLEMENTED_v15b; +4 fixes pos code-review (CRITICAL-1 commit pos-Playwright falha NAO conta sucesso + CRITICAL-2 guard permissivo + HIGH-1 WARN inconsistente + HIGH-2 docstring) |
| **C12** | Capinar etapa E (RecebimentoLf X→FB) | metodo `executar_etapa_e` + idempotencia G-RECLF-3 via UK odoo_lf_invoice_id + G-RECLF-2 parcial + status='processando' retoma + svc dentro do loop + produto_tracking real via fetch | 4 pytest verdes; dry-run OK + smoke PROD identifica invoice 629364 | v17 | ✅ | **COMPLETO v17 2026-05-25** — SEQUENCIAL (decisao 10.7 Rafael); +4 fixes pos code-review (CRITICAL-3 UK migration aplicada PROD + HIGH-3/4/5) |
| **C13** | Capinar etapa F (entrada manual FB→{LF,CD}) — INVOCA atomo Skill 5 NOVO `criar_picking_entrada_destino_manual` (G023 fix codificado intra-atomo, alinha "Fluxo>>Skills") | metodo `executar_etapa_f` DELEGA atomo Skill 5 + V1 STRICT INDUSTRIALIZACAO_FB_LF + origin idempotente + pre-check state='posted' + situacao_nf='autorizado' (CRITICAL-4) + HOJE dentro loop + auditoria agg_vazio | 7 pytest verdes; dry-run OK | v17 | ✅ | **COMPLETO v17 2026-05-25** — DELEGADO 100% atomo Skill 5 v15a; +3 fixes pos code-review (CRITICAL-4 + HIGH-6/7) |
...
| **C24** | **NOVO v17.5 — Criar Skill 7 `escriturando-odoo`** (REVERT ETAPA E inline → atomo C3 dedicado) | `app/odoo/estoque/scripts/escrituracao.py` (EscrituracaoLfService ~500 LOC) + `.claude/skills/escriturando-odoo/SKILL.md` (3 receitas) + `tests/odoo/services/test_escrituracao_lf_service.py` (10 pytest) + REVERT `executar_etapa_e` delegando atomo + ETAPA F expansion canary DEV_FB_LF + TRANSFERIR_FB_CD | 512 pytest verdes; smoke PROD ETAPA E detecta invoice 629364 via atomo Skill 7 em 765ms | v17.5 | ✅ | **CONCLUIDO v17.5 2026-05-26** — V1 STRICT LF→FB; G-RECLF-2/3 + HIGH-3/4/5 + D17 + D9 encapsulados; Skill 8 ETAPA E reduzida de ~420 LOC para ~180 LOC delegando |
...
### Sessao v18 (2026-05-26 — C14 recovery + C15 SKILL.md + G037)
- ✅ Setup worktree + venv + ENV; main NAO avancou desde v17.5 (apenas 4 commits docs PROMPT v18 e8bfea73→c5ac0607); pytest baseline 513 verdes.
- ✅ Leituras: PLANEJAMENTO §0 + `faturamento_pipeline.py` (executar_pipeline_bulk L3492-3764 + main L3813-3884) + `escrituracao.py` + SKILL.md Skill 7 (modelo) + `fat_lf_resume.sh` + `fat_lf_resume_entrada.sh` + estrutura `fluxos/` (10 folhas L3 ja' presentes) + `orchestrators/pre_etapa_executor.py` (pattern de C3 macro).
- ✅ AskUserQuestion v18 — Rafael:
  - **Recovery design**: pergunta arquitetural ("`executar_pipeline_bulk` eh fluxo?"). Esclarecido: NAO eh fluxo L3 (em `fluxos/`); eh metodo de C3 macro orchestrator (`orchestrators/faturamento_pipeline.py`). `executar_pipeline_resume` adicionado ao MESMO orchestrator (C3 macro), compondo `executar_pipeline_bulk(etapas=(apenas_etapa,))` em loop.
  ...
  - **G036 vs G037**: G036 ja' ocupado (lote virgula literal — `docs/inventario-2026-05/02-gotchas/G036-lote-virgula-literal-e-duplicado-operador-igual.md`); usado G037 para "operacao nao cadastrada exige CFOP explicito".
- ✅ **C14 RECOVERY** (`executar_pipeline_resume`) em `app/odoo/estoque/orchestrators/faturamento_pipeline.py` (~280 LOC novas):
  ...
  - `_contar_pendentes_por_etapa(etapa, ciclo, company_origem_id, cod_produto)` com filtros B/C/D/E/F (ETAPA E cross-ref com RecebimentoLf.status='processado' — espelha `fat_lf_resume_entrada.sh:contar_e`).
  - `executar_pipeline_resume(*, ciclo, apenas_etapa, max_iter, timeout_iter_s, detector_stagnation, ...)` — loop principal: TUDO_OK_INICIAL → loop max_iter → TUDO_OK / STAGNATION / MAX_ITER / EXCECAO.
  - CLI estendido: `--modo {bulk,pre-flight,resume}` + `--apenas-etapa` + `--max-iter` + `--timeout-iter` + `--sem-stagnation`.
  - `main()` modo='resume' branch com exit codes coerentes (0/1/2/4).
- ✅ **8 PYTEST NOVOS** em `tests/odoo/services/test_faturamento_pipeline_orchestrator.py`:
  - 1) apenas_etapa invalida (A) -> FALHA_USO
  - 2) ETAPA D real sem confirmar_sefaz bloqueia
  - 3) TUDO_OK_INICIAL sem pendentes (bulk nao invocado)
  - 4) TUDO_OK apos 2 iter (mock 5->2->0)
  - 5) STAGNATION detector para na 1a iter (mock 5->5)
  - 6) MAX_ITER atingido (mock 10->7->5 com max_iter=2)
  - 7) EXCECAO no bulk -> motivo_parada=EXCECAO
  - 8) sem_stagnation continua ate' max_iter (mock 5->5->5 com flag)
- ✅ **C15 SKILL.md** em `.claude/skills/faturando-odoo/SKILL.md` (~430 LOC):
  - Frontmatter completo com NAO USAR PARA + V1 LIVE descricao
  - REGRAS CRITICAS (9 itens)
  - Contrato `executar_pipeline_bulk` (entry-point principal) + `executar_pipeline_resume`
  - 4 RECEITAS concretas com bash exemplos
  - TRADE-OFFS V1 ACEITOS (estendido pos CR v18 F2/F3/F4): timeout-iter NAO ENFORCADO + stdout mistura logs+JSON + CR-H4 nao se aplica em resume isolado
  - Secao ANTIPADROES DETECTADOS V17.5 — REFATOR V19+ (4 antipadroes detalhados)
  - Cross-refs (constituicao, planejamento, ROADMAP, subagente, ROUTING_SKILLS, tool_skill_mapper, Skill 7 par, sub-skill C5, Skill 5 atomos, Skill 2 v2, service-fonte legado, scripts shell substituidos, gotchas, IDs fixos)
  - CHECKLIST DE EXPANSAO V19+ (10 itens documentados)
- ✅ **G037 NOVO** em `docs/inventario-2026-05/02-gotchas/G037-operacao-nao-cadastrada-exige-cfop-explicito.md`: sintoma + causa raiz + solucao V1 paliativa (cfop_esperado da MATRIZ_INTERCOMPANY) + solucao V2 correta (refator v19+ via Skill 7 ABRANGENTE + `l10n_br_tipo_pedido`).
- ✅ **Referencia rapida G037 em `.claude/references/odoo/GOTCHAS.md`**: nova secao "Gotchas Inter-Company Fiscal (Inventario 2026-05)" com tabela completa G011-G037.
- ✅ **CLAUDE.md estoque §6.5 NOVO** "ANTIPADROES DETECTADOS V17.5 — REFATOR V19+" (4 antipadroes + gotcha G037 detalhados).
- ✅ **1 code-reviewer paralelo** (feature-dev:code-reviewer) — 4 findings (1 CRIT + 3 HIGH) TODOS aplicados:
  - **F1 CRITICAL conf 90**: `_contar_pendentes_por_etapa` ETAPA F nao contava `F5f_FALHA` → resume parava prematuramente em retries. Fix: incluir `F5f_FALHA` + `F5e_FALHA` + `F5d_TIMEOUT` nas etapas relevantes (C/D/F). STAGNATION garante alerta operador para retries de FALHA (intervencao manual: revert fase_pipeline OU investigar SEFAZ cstat).
  - **F3 HIGH conf 95**: `timeout_iter_s` aceito mas nao propagado para `executar_pipeline_bulk` (lying parameter). Fix: clarificar docstring + CLI help text + SKILL.md TRADE-OFFS (NAO ENFORCADO em v18; operador deve usar `timeout NNN python -m ...` shell para enforcement real).
  - **F2 HIGH conf 82**: CR-H4 guard de bulk nao se aplica em resume isolado (B nao no caminho). Fix: documentado no SKILL.md TRADE-OFFS — operador deve rodar `--modo bulk --etapas A,B,C,D` antes de tentar `--modo resume --apenas-etapa D` para ter B/C garantidos.
  - **F4 HIGH conf 80**: stdout mistura logs Flask + JSON. Fix: documentado no SKILL.md TRADE-OFFS — operador `2>/dev/null | tail | jq`; subprocess wrapper pendente v19+.
- ✅ **BASELINE PYTEST ODOO**: 513 → **521 verdes em 14.76s** (+8 net v18). Sem regressao apos aplicar F1+F3 fixes (re-rodado pos-fixes).
- ✅ **SMOKES DRY-RUN PROD v18** em `/tmp/log_skill8_smokes_v18_*.json`:
  - resume B cod 105000007 → DRY_RUN_OK TUDO_OK_INICIAL 788ms (sem pendentes)
  - resume D cod 105000007 → DRY_RUN_OK TUDO_OK_INICIAL 814ms (sem pendentes)
  - resume E cod 104000003 → DRY_RUN_OK TUDO_OK_INICIAL 799ms (sem pendentes)
  - resume F cod 210030007 → DRY_RUN_OK TUDO_OK_INICIAL 757ms (sem pendentes; cod ja' em F5f_OK conforme v17)
  - resume sem --apenas-etapa → exit 2 + stderr "ERRO USO: --modo resume exige --apenas-etapa" (FALHA_USO confirmado)
  - Resumo consolidado em `/tmp/log_skill8_smokes_v18_resumo.json`
- ✅ Cross-refs aplicados:
  - `app/odoo/estoque/CLAUDE.md` (§6 status global + §6.5 antipadroes + tabela §6 catalogo Skill 8 v18)
  - `app/odoo/estoque/ROADMAP_SKILLS.md` HANDOFF v18 (entry NOVA acima da v17.5)
  - `app/odoo/estoque/PLANEJAMENTO_SKILL8_FATURANDO.md` §0 + §12 trilha v18 (esta entry)
  - `.claude/references/odoo/GOTCHAS.md` (nova secao tabela G011-G037)
  - `docs/inventario-2026-05/02-gotchas/G037-*.md` (NOVO arquivo)
  - `.claude/skills/faturando-odoo/SKILL.md` (NOVO arquivo)
- 🟢 **Scripts shell SUPERADOS** (NAO removidos ate' v22+ pos-canary REAL):
  - `scripts/inventario_2026_05/fat_lf_resume.sh` → substituido por `--modo resume --apenas-etapa D/C`
  - `scripts/inventario_2026_05/fat_lf_resume_entrada.sh` → substituido por `--modo resume --apenas-etapa E/F`
  - Workflows operacionais migrados para SKILL.md Receitas 2 e 3
- 🟢 **NAO MEXEU** em `RecebimentoLfOdooService` (regra v14a-fix) nem em `09_executar_onda1_bulk.py` (regra v14a-ops) nem em `escriturar_dfe_lf.py` (referencia v19+).

**Status global após v18:**
- Skill 8 `faturando-odoo` 🟡 **PIPELINE A-F + RECOVERY + SKILL.md LIVE** — C6-C13 + C24 (v17.5) + **C14 + C15 + C16 + C17** (v18); **17 checkpoints ✅** de 24.
- Pendentes apenas: C18 folhas fluxos L3 (1.1*, 1.3 — junto com refator Skill 7 v19+), C19 cross-refs final, C20 canary REAL PROD, C21 bulk REAL PROD, C22 code-review final, C23 commit + arquivar 09_*.
- Próximo passo (v19+): **REFATOR Skill 7 ABRANGENTE** + **FLUXO L3 1.2.1 escriturar-dfe-industrializacao** + **extrair ETAPA F do orchestrator para FLUXO L3** (escopo MUITO ALTO cross-modulo — antipadroes 1+2+3 documentados em CLAUDE.md §6.5 + SKILL.md). Risco MEDIO em v18 (escopo conservador); v19+ Risco MUITO ALTO.
...
### Sessao v17 (2026-05-25) — C11 ETAPA D F5e SEFAZ + C12 ETAPA E RecLF + C13 ETAPA F atomo Skill 5 (PIPELINE COMPLETO A-F LIVE)

- ✅ Setup worktree + venv + ENV; rebase de main (2 commits — 48b0dfa6 recebimento-lf gravar NF + 9906e70b agente CLI fix) sem conflito (zero overlap com `app/odoo/estoque/`); pytest baseline pos-rebase: **483 verdes em 15.03s**.
- ✅ Leituras obrigatorias (regra inviolavel 0):
  - PLANEJAMENTO §0 + §5 SSL/timeout + §7.2 D7-D9 service legado F5e L1116-1346 + §7.3 D17 mapa CFOP + §7.4 G-RECLF-1..11 (decisao 10.7 paralelismo) + §10.3 etapa-barreira + §10.6 Skill 5 atomos + §10.7 paralelismo + §12 trilha v16 INTEIRA.
  - Service legado: `app/odoo/services/inventario_pipeline_service.py:1116-1346` (`f5e_transmitir_sefaz` — idempotencia tripla, HARD_FAIL, MED C-1/C-2).
  - Script 09 ETAPA E: `scripts/inventario_2026_05/09_executar_onda1_bulk.py:1239-1421` (`etapa_e_entrada_fb` — agg por (pid, lote, cfop), ACAO_PARA_CFOP_ENTRADA, idempotencia via UK).
  - Script 09 ETAPA F: `09_executar_onda1_bulk.py:1428-1688` (`etapa_f_entrada_destino_manual` + `_f_criar_entrada_destino_para_invoice` — origin idempotente, MIGRAÇÃO->INV-{cod}-{HOJE}).
  - Atomo Skill 5 v15a: `app/odoo/estoque/scripts/picking.py:1174-1428` (`criar_picking_entrada_destino_manual` — G023+G011+G019/G020+idempotencia origin).
  - Interface RecebimentoLfOdooService: L1-280, L370-528 (`processar_recebimento`, `_safe_update`, `_checkpoint`, `_recover_state_from_odoo`).
  - Playwright: `app/recebimento/services/playwright_nfe_transmissao.py:41-200` (`transmitir_nfe_via_playwright(invoice_id, odoo, logger, max_tentativas, intervalo_retry)`).
- ✅ AskUserQuestion v17:
  - **Escopo**: Rafael deixou avaliar — escolhi **C11+C12+C13 completo** (~500 LOC + 16 pytest mocks, viavel em 1 sessao).
  ...
  - **ETAPA F V1 STRICT**: APENAS INDUSTRIALIZACAO_FB_LF (Rafael: "estudar scripts ja realizados" — confirmado APENAS LF=19 em PROD via pickings 317306, 317316). DEV_FB_LF/TRANSFERIR_FB_CD commented out em `ACOES_ENTRADA_DESTINO_MANUAL`.
- ✅ **C11 ETAPA D real** em orchestrator (~370 LOC novas substituindo stub):
  - Playwright serial 1 browser via `transmitir_nfe_via_playwright(invoice_id, self.odoo, logger, max_tentativas=15, intervalo_retry=120)`.
  - Idempotencia TRIPLA (D8): por ajuste (sem invoice_id), por invoice no batch (1 inv = 1 SEFAZ via `invoices_processadas` dict), por persistencia (`fase_pipeline=F5e_SEFAZ_OK` OR `status='EXECUTADO'`).
  - HARD_FAIL_CONFIG_ERRORS (D7): frozenset {'playwright_indisponivel', 'odoo_password_ausente', 'odoo_username_ausente'} + `tentativas == 0` ABORTA batch via return early `status='FALHA_CONFIG'` (NAO RuntimeError — HIGH-2 docstring fix).
  - SNAPSHOT meta (D5) antes do loop.
  - commit_resilient (G016) antes do loop + pre-NF + pos-NF.
  - safe_session_get (D9 + F6 v15c) apos Playwright (sessao pode expirar 5-10min).
  - D18 2 niveis: dry-run NAO bloqueia (planejamento); real-run `BLOQUEADO_SEM_CONFIRMAR_SEFAZ` se faltar `--confirmar-sefaz`.
  - MED C-1: `situacao_nf != 'autorizado'` mas `sucesso=True` (excecao_autorizado) registra em `aj.erro_msg` para audit fiscal.
  - MED C-2: persistir `cstat`+`xmotivo` de `ultimo_estado` em falha SEFAZ.
- ✅ **C12 ETAPA E real** (~270 LOC novas substituindo stub):
  - SEQUENCIAL (decisao 10.7 v17 — Rafael) invocando `RecebimentoLfOdooService.processar_recebimento(rec.id, usuario_nome=usuario)` (NAO MEXER no service externo 4562 LOC).
  - **HIGH-4** v17: `svc = RecebimentoLfOdooService()` INSTANCIADO DENTRO do loop (alinha script base — evita estado interno `self._recebimento_id` vazando entre invoices).
  - Idempotencia G-RECLF-3 via `RecebimentoLf.odoo_lf_invoice_id` UK + check `status='processado'` (skip).
  - **HIGH-3** v17: `status='processando'` (crash recovery) **RETOMA** via service (etapa_atual > 0) em vez de criar duplicado.
  - **HIGH-5** v17: `produto_tracking` REAL via batch fetch `product.product.tracking` (anti-D-OPS-5 — `'lot'` hardcoded quebra `_step_10_preencher_lotes` para `tracking='none'`); fallback `'lot'`.
  - G-RECLF-2: aceita `transfer_status='erro'` como sucesso parcial (`contador parcial_fb_ok_transfer_erro`).
  - D17 `ACAO_PARA_CFOP_ENTRADA` mapeia 5xxx->1xxx (FB so' tem fiscal_position para entrada).
  - Agg por (pid, lote_dest, cfop) -> qty (lote MIGRAÇÃO default).
- ✅ **C13 ETAPA F real** (~250 LOC novas substituindo stub):
  - DELEGA atomo Skill 5 v15a `picking_svc.criar_picking_entrada_destino_manual(company_destino_id, location_origem_id, location_destino_id, moves_data, picking_type_id, origin)` — principio Fluxo>>Skills (decisao 10.6 Rafael v13).
  - V1 STRICT: filtra `acao_decidida in ACOES_ENTRADA_DESTINO_MANUAL` (= `{'INDUSTRIALIZACAO_FB_LF'}` frozenset). DEV_FB_LF/TRANSFERIR_FB_CD nem entram (commented out na constant).
  - Defensive check `not picking_type_id or not location_dest` (incrementa `not_implemented_direcao` se mapeamento ausente).
  - Origin idempotente `INV-{ciclo}-ENTRADA-{label}-NF{invoice_id}` (consistente com 317306, 317316 PROD).
  - Lote MIGRAÇÃO/vazio -> `INV-{cod}-{YYYYMMDD}` (consistente G014 v16).
  - **CRITICAL-4** v17: pre-check `invoice.state='posted'` + `l10n_br_situacao_nf='autorizado'` (anti-saldo-fantasma se NF cancelada externamente).
  - **HIGH-6** v17: `HOJE` calculado DENTRO do loop (anti-lote-errado se run cruzar meia-noite).
  - **HIGH-7** v17: `agg_vazio` registra auditoria por ajuste (anti-falha-sistematica silenciosa).
  - F5f_OK marca fase apos picking done (CRIADO ou IDEMPOTENT_DONE skip).
- ✅ **Migration v17** PROD aplicada: `scripts/migrations/2026_05_25_v17_uk_recebimento_lf_invoice_id.{py,sql}` adiciona `UniqueConstraint uq_recebimento_lf_invoice_id` (CRITICAL-3 Reviewer 2 — anti-RecLf duplicado quando `processar_recebimento` falha mid-process e re-run cria 2o).
- ✅ **3 code-reviewers paralelos** (Playwright SEFAZ + RecLF integration + Skill 5 atomo) — 11 findings (4 CRITICAL + 7 HIGH) TODOS aplicados:
  - **CRITICAL-1** (R1 conf 95): commit POS-Playwright falha NAO conta como sucesso (anti-dupla-SEFAZ em re-run). Fix: `FALHA_COMMIT_POS_SEFAZ_OK` em `invoices_falha` + log CRITICAL para operador.
  - **CRITICAL-2** (R1 conf 90): guard permissivo D8.3 — `ja_processados` nao-vazio (1+ ajuste F5e_OK) = invoice ja transmitida (anti-dupla-SEFAZ em crash mid-loop).
  - **CRITICAL-3** (R2 conf 95): UK `recebimento_lf.odoo_lf_invoice_id` criada via migration v17 (anti-RecLf duplicado).
  - **CRITICAL-4** (R3 conf 92): situacao_nf!=autorizado em ETAPA F pula (anti-saldo-fantasma).
  - **HIGH-1** (R1 conf 85): WARN para estado inconsistente status=EXECUTADO+fase!=F5e_OK (anti-corrupcao silenciosa).
  - **HIGH-2** (R1 conf 82): docstring removeu `Raises RuntimeError` (codigo retorna early com status, NAO lanca).
  - **HIGH-3** (R2 conf 88): status='processando' RETOMA via service (anti-RecLf orfao).
  - **HIGH-4** (R2 conf 82): svc instanciado DENTRO do loop (anti-estado-vazando entre invoices).
  - **HIGH-5** (R2 conf 80): produto_tracking real via batch fetch (anti-D-OPS-5 quebra tracking='none').
  - **HIGH-6** (R3 conf 80): HOJE dentro do loop (anti-lote-errado cross-midnight em run longa).
  - **HIGH-7** (R3 conf 82): auditoria SKIP_AGG_VAZIO (anti-falha-sistematica silenciosa).
- ✅ **19 PYTEST NOVOS VERDES** em `tests/odoo/services/test_faturamento_pipeline_orchestrator.py`:
  - **5 ETAPA D** (real_run_sem_confirmar_sefaz_bloqueado, dry_run_sem_ajustes_skip, dry_run_com_ajustes_planeja, real_run_sucesso_sefaz mock Playwright, hard_fail_config_aborta, idempotencia_persistente_skip, falha_sefaz_com_cstat MED C-2)
  - **4 ETAPA E** (dry_run_skip, dry_run_com_ajustes_planeja_filtra_FB_X, real_run_sucesso_reclf mock, idempotencia_processado, sucesso_parcial_transfer_erro G-RECLF-2)
  - **7 ETAPA F** (dry_run_skip, dry_run_com_ajustes_planeja, real_run_sucesso_atomo verifica delegacao, v1_strict_DEV_FB_LF_skip, idempotente_done_skip, idempotent_other_falha_investigacao, invoice_nao_posted_pula)
  - **3 pos-fixes** (CRITICAL-1 commit_pos_falha contador correto, CRITICAL-4 situacao_nf_nao_autorizado_pula, HIGH-3 status_processando_retoma_nao_duplica)
- ✅ **BASELINE PYTEST ODOO**: 483 → **502 verdes em 14.51s** (+19 v17 = 16 base + 3 pos-fixes).
- ✅ **SMOKES DRY-RUN PROD** (INVENTARIO_2026_05):
  - ETAPA D dry-run cod 104000003 (PERDA_LF_FB): SKIP_NENHUM_AJUSTE em 746ms (esperado — cod em F5e_SEFAZ_OK, nao F5d_INVOICE_GERADA).
  - ETAPA E dry-run cod 104000003: DRY_RUN_OK_ETAPA_E em 742ms (1 invoice 629364 PERDA_LF_FB detectada para entrada FB).
  - ETAPA F dry-run cod 210030007 (INDUSTRIALIZACAO_FB_LF ja em F5f_OK): SKIP_NENHUM_AJUSTE em 743ms (idempotencia funcional).
  - Pipeline completo A-F cod 105000007: DRY_RUN_OK em 746ms — status agregado coerente.
- ✅ Cross-refs aplicados: CLAUDE.md estoque (status v17) + ROADMAP HANDOFF v17 + PLANEJAMENTO §0 + §7 (C11/C12/C13 ✅) + §12 (esta trilha).
- 🟢 **NAO MEXEU** no script 09 (regra v14a-ops) NEM RecebimentoLfOdooService 4562 LOC (regra v14a-fix) NEM InventarioPipelineService legacy.
- 🟢 **Pipeline A-F COMPLETO** — destrava v18 (recovery `--resume` + SKILL.md + smokes adicionais) e v19-20 (canary REAL PROD + bulk REAL PROD).
