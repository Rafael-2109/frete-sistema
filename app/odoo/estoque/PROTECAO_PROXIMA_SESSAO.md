...
| N23 | Idempotência reaproveitar registro state=cancel — incidente G-AUDIT-3 v21+ (✅ RESOLVIDO v22+ 2026-05-27) | Skill 5 `criar_picking_inter_company` faz idempotência por origin match. Se picking existente está em state=cancel (do retry anterior), reaproveita erroneamente → `action_assign` falha (`<Fault 2: 'Nada para verificar a disponibilidade.'>`). Estados válidos para reaproveitar: `draft/confirmed/assigned/done`. State=cancel exige criar NOVO. **FIX v22+**: `picking.py:944-1006` segrega cancelados; se TODOS são cancel, prossegue para create; se mistura, prefere o vivo e loga skip dos cancelados. 2 pytest novos (`test_..._g_audit_3_pula_pickings_cancelados` + `test_..._g_audit_3_prefere_vivo_sobre_cancel`). LIÇÃO ATEMPORAL: ao implementar idempotência por chave externa (origin, external_id, etc.), SEMPRE filtrar `state=cancel` (ou equivalente "registro morto") da busca, OU segregar e logar. Idempotência "reaproveita o que existir" sem filtrar dead-states gera deadlock retry. | CLAUDE.md §14 D-V21-7 + D-V22-1 |
| N24 | Criar PO no LF (FLUXO L3 1.2.x caminho A ou B) sem garantir `purchase.team` com user_id permitido — G039 (✅ CODIFICADO v23+ 2026-05-27) | PO criada cai em `team_id` default ('Aprovação LF - JOSEFA' user_id=78 Edilane). Se PO precisa aprovação dupla por valor/regra CIEL IT custom, `button_confirm` retorna True mas state fica 'to approve' permanente. **FIX v23+**: átomo `escrituracao.garantir_purchase_team(user_id, company_id)` busca por (user_id, company_id, active=True); cria se não existe. Hook `_resolver_team_g039` no orchestrator com cache local substitui `team_id` STATIC pelo team correto do user de execução. Fallback silencioso (warning + STATIC) se hook falhar. 14 pytest novos. Validado E2E PROD: PO 42419 team 41→143 + FLUXO L3 1.2.x passo 7→10 OK invoice ENTIN/2026/05/0055 posted. | CLAUDE.md §14 D-V22-3 + D-V23-1 |
| N25 | Skill 7 `criar_dfe_a_partir_do_invoice_saida` cria dfe.lines com `company_id` herdado do XML da SAÍDA — B-V23-1 (✅ CODIFICADO v23.5+ 2026-05-27) | Quando XML da saída é FB→LF, `action_processar_arquivo_manual` cria DFe no LF (company_id=5) mas as dfe.lines herdam company_id=1 (FB). Sintoma: passo 9 `action_create_invoice` falha com `<Fault 4: leitura dfe.line>`. Causa: CIEL IT faz `with_company(dfe.company_id=5)` reduzindo `allowed_company_ids=[5]`; lines company=1 não passam pela ir.rule 353 nesse contexto. **FIX v23.5+**: átomo `criar_dfe_a_partir_do_invoice_saida` codifica fix pós-poll: search dfe.lines por dfe_id + read company_id + batch write `company_id=company_destino` se divergente. Idempotente (skip se já alinhado). Não-fatal: log warning se write falhar. 3 pytest novos. Workaround manual v23+ permanece em PROD para o caso 43533 (museum vivo). | CLAUDE.md §14 D-V23-2 + VALIDACAO B-V23-1 |
| N26 | Skill 7 `gerar_po_from_dfe`/`preencher_po` deixa `PO.line.account_id` apontando para account da company FONTE — B-V23-2 (✅ CODIFICADO v23.5+ 2026-05-27) | `action_gerar_po_dfe` cria PO.lines no LF (company_id=5) mas `account_id` é resolvido para `account.account` da FB (ex: id=22611 '3202010001') em vez do equivalente LF (id=26459). Cada code de conta existe em todas 4 companies. Sintoma após fix N25: passo 9 falha com `"Empresas incompatíveis"`. **FIX v23.5+**: novo átomo `resolver_account_id_por_company(account_id_fonte, company_destino)` que search por (code, company_id=destino). Hook em `gerar_po_from_dfe` após status=CRIADO: itera PO.lines + para cada line resolve account equivalente da line.company_id + batch write se divergente. Idempotente (JA_NA_DESTINO=skip). Não-fatal (warning log). Account inexistente na destino: warning + line preserva account divergente (caller detecta no passo 9 com diag claro). 9 pytest novos (5 átomo + 4 hook). Workaround manual v23+ permanece em PROD para PO 42419. | CLAUDE.md §14 D-V23-3 + VALIDACAO B-V23-2 |
| N27 | Orchestrator caminho novo L3 v19+ chama `executar_fluxo_l3_1_2_x` SEM `lotes_data` resolvido — **F1 v25+** (✅ CODIFICADO 2026-05-27) | `_executar_etapa_f_via_fluxo_l3` deixava `lote_default='MIGRAÇÃO'` literal aplicado a TODOS MLs → saldo final em LF/Estoque/MIGRAÇÃO em vez do lote do XML SEFAZ. Legacy v17.5 da ETAPA F (`executar_etapa_f` linhas 3998-4018) JÁ FAZIA certo (lia `AjusteEstoqueInventario.lote_destino`, transformava vazio/'MIGRAÇÃO' → `INV-{cod}-{HOJE}`); caminho novo regrediu. **FIX v25+**: `_executar_etapa_f_via_fluxo_l3` resolve `lotes_data` por invoice via `_resolver_pids_em_batch` + agrega `(product_id, lote_destino)` com `abs(qtd_ajuste)` + transforma MIGRAÇÃO → INV-{cod}-{HOJE}; F1b muda default `lote_default` 'MIGRAÇÃO' → None (forca caller). | CLAUDE.md §14 D-V25-1 + CIRURGIA_AVULSO_FRASCO_2026_05_27.md |
| N28 | Caminho A do FLUXO L3 1.2.x (DFe via SEFAZ) sem fix B-V23-1 (`dfe.line.company_id=destino`) — **F2a v25+** (✅ CODIFICADO 2026-05-27) | B-V23-1 estava codificado APENAS em `criar_dfe_a_partir_do_invoice_saida` (caminho B). Caminho A (`buscar_dfe` retorna `encontrado=True`) NÃO tinha fix equivalente. Em INDUSTRIALIZACAO_FB_LF os DFes vêm via SEFAZ (canary v20+ provou 4 de 4 — `fluxos/1.2.2-criar-dfe-manual-transferencia.md:24`). Sintoma: passo 9 `action_create_invoice` falha com `<Fault 4: leitura dfe.line>`. **FIX v25+**: novo átomo público `EscrituracaoLfService.alinhar_dfe_lines_company(dfe_id, company_destino)` (escrituracao.py:835, ~120 LOC) generaliza B-V23-1 + invocado no passo 1.5 do `executar_fluxo_l3_1_2_x` quando caminho A. | CLAUDE.md §14 D-V25-1 |
| N29 | Picking nativo (gerado via `action_gerar_po_dfe`) sem G023 force `company_id` em picking + moves — **F2b v25+** (✅ CODIFICADO 2026-05-27) | XML-RPC não herda `company_id` automaticamente em alguns cenários. Átomo legacy `criar_picking_entrada_destino_manual` já codificava G023 (`picking.py:1391-1399`) mas o caminho novo L3 v19+ regrediu. Sintoma: picking sai de `location_id=4 Parceiros/Fornecedores` (virtual fornecedor) em vez do estoque destino correto. **FIX v25+**: passo 6.5 do `executar_fluxo_l3_1_2_x` força `stock.picking.company_id` + `stock.move.company_id = company_destino` após localizar picking ativo. Não-fatal: erro vira log warning. | CLAUDE.md §14 D-V25-1 |
| N30 | ~~`escriturar_dfe` deve escrever `dfe='compra'` no DFe de INDUSTRIALIZACAO_FB_LF~~ — **F3 v25+ ❌ CONCLUSÃO REVERTIDA por C6 (canary D-V30-1, 2026-06-02)** | **VERDADE ATUAL**: INDUSTRIALIZACAO_FB_LF usa `dfe='serv-industrializacao'` (= `po`). `escriturar_dfe` **REJEITA** `'compra'` (whitelist `escrituracao.py:1390-1394` não inclui 'compra') e o canary REAL (744869→745414, CFOP 1901) escriturou com `serv-industrializacao`. Código já corrigido: `inventario_pipeline.py:3316-3319` = `{'dfe': 'serv-industrializacao', 'po': 'serv-industrializacao'}`. A tentativa F3a v25+ (`dfe='compra'`) baseou-se na cirurgia AVULSO_FRASCO (PO 42543 alterada manual via UI) — conclusão errônea para o caminho do pipeline; preservada como histórico. ⚠️ As OUTRAS 7 ações do mapping ainda têm `dfe='compra'` (CANDIDATE) = **BUG LATENTE** (quebraria `escriturar_dfe` Passo 3 via fluxo-L3 opt-in `--usar-fluxo-l3-v19`; dormente no caminho default — ver N33 + follow-up §2). | CLAUDE.md §14 D-V30-1 (revoga D-V25-1 F3) |
| N31 | Override G039 dinâmico para LF quando team operacional é fixo — **F4 v25+** (✅ CODIFICADO 2026-05-27) | G039 hook (v23+ `_resolver_team_g039`) resolvia `team_id` do user de execução. Para Rafael uid=42 retornava team 143 corretamente; para outro user retornaria diferente. Padrão operacional desta skill é team=143 SEMPRE para LF, independente de quem dispara. **FIX v25+**: `CONSTANTS_FLUXO_L3_POR_COMPANY_DESTINO[5]['team_id']` 41 → **143** STATIC FIXO + `_resolver_constants_fluxo_l3` by-passa override G039 quando `company_destino=5`. Demais destinos (FB=1, CD=4) mantêm G039 dinâmico quando forem mapeados v26+. | CLAUDE.md §14 D-V25-1 |
| N32 | ~~Adicionar `_nova_funcao_modulo` em `inventario_pipeline.py` sem atualizar lista explícita do stub `faturamento_pipeline.py:62-72`~~ (✅ STUB REMOVIDO v28+ S6.b 2026-05-28 — N32 OBSOLETO) | Pre-v28+ S6.b: stub re-exportava públicos + lista explícita de privados. Sessão v28+ confirmou zero imports Python ativos via grep + suite pytest passa SEM stub (681 verdes). **Lição ATEMPORAL preservada (vale para QUALQUER stub futuro)**: stubs alias-compat criam contrato implícito; cada `_privado` novo no módulo destino EXIGE re-export explícito ou converter para method de classe. **Pattern de remoção segura**: (1) grep -rn imports ativos retornar zero; (2) pytest verde com stub removido; (3) atualizar cross-refs em docs/CLAUDE.md/PROTECAO; (4) commit isolado para rollback fácil se descobrir caller faltando. | (Stub removido v28+; N32 mantida como lição atemporal sobre stubs futuros) |
| N33 | Usar entrada `CANDIDATE` de `CONSTANTS_FLUXO_L3_POR_COMPANY_DESTINO` ou `L10N_BR_TIPO_PEDIDO_POR_ACAO` em real-run sem canary REAL PROD primeiro (✅ DOCUMENTADO v27+ pós-CR) | Entries v27+ S4 para FB=1 e CD=4 + 7 ações da MATRIZ (PERDA_LF_FB, DEV_*, TRANSFERIR_*) marcadas CANDIDATE — IDs descobertos via XML-RPC `read` mas NÃO validados por execução completa do pipeline. `picking_type_id` errado cria picking em local incorreto silenciosamente; `l10n_br_tipo_pedido` errado deriva CFOP fiscalmente incorreto na NF (SEFAZ irreversível). **Mitigação**: rodar `--dry-run=True` primeiro + inspecionar campos Odoo manualmente; `--confirmar-sefaz` SOMENTE após cross-check com legacy `executar_etapa_f` (que sabe o caminho certo). CANDIDATE = proibido `--confirmar-sefaz` sem canary cross-check. | `inventario_pipeline.py:3201-3315` (CONSTANTS expand v27+ S4) + CR-reviewer holístico v27+ |
| N34 | Criar átomo XML-RPC (ou flag `recompute=True`/`recompute_fiscal`) para montagem cujo recompute fiscal só persiste server-side — *lying parameter* (industrialização SAÍDA 2026-06-15) | `onchange_l10n_br_calcular_imposto[_btn]` retorna o dict via XML-RPC mas o ORM **descarta** (não grava; `s48`/`s49`). Um átomo que anuncia recompute que seu **substrato** (XML-RPC) não entrega viola §3 "auto-seguro" (mesma classe do `--timeout-iter` mentiroso, AR-F3). A montagem da NF-2 de SAÍDA (op **2864** + recompute + remap conta via fp **111** → 1150100012) é **server-side** → mora no **body da server action** (categoria **infra-Odoo** `app/odoo/estoque/provisioning/`, FORA do sistema de skills — §1.1 + CLAUDE.md:410-412), NÃO em átomo. `montar_invoice_entrada_direta` permanece **PURO** p/ a ENTRADA (que ao contrário PROÍBE o recompute, `escrituracao.py:2315-2316`). §4 ("estender antes de criar") pressupõe MESMO substrato; aqui a diferença é substrato (XML-RPC vs server-side), não arg. | CLAUDE.md §3 + §1.1 + `escrituracao.py:2315-2316` + SOT §6.3 (análise eixo 2/4, 2026-06-15) |
...
9. **Folha L3 com snippet bash `--confirmar` ou `--confirmar-sefaz`** (v27+ pós-CR) — adicionar bloco `> **ATENÇÃO — Canary REAL PROD: <STATUS>**` em negrito **ANTES** do snippet (não depois), explicitando se o caminho foi validado em PROD ou ainda é CANDIDATE. Subagente que lê folha linearmente pode disparar `--confirmar-sefaz` antes de chegar ao status no fim do arquivo. Exemplo: 1.1.1-faturamento-saida-pura.md linha 163 tem snippet bash; aviso "canary REAL: pendente" estava na linha 181 (depois). Posicionamento correto: aviso ANTES do snippet.
10. **Refatorar ETAPA do orchestrator que toque SEFAZ/Playwright/irreversível** (v27+ pós-CR) — OBRIGATORIAMENTE via opt-in coexistente: flag nova (`--usar-X-vN`), helper privado `_executar_etapa_X_via_nova_abordagem`, dispatch `if flag: novo else: legacy`, default OFF. Jamais substituir legacy inline sem opt-in. Flip default (remover legacy) só após canary REAL PROD com 2+ casos naturais validados. Pattern provado: `--usar-fluxo-l3-v19` (v20+ S3 — caminho L3 1.2.x) + `--usar-skill8-atomica-v25` (v27+ S1 — Skill 8 ATÔMICA C+D).
11. **Antes de canary SEFAZ inter-company, validar a `l10n_br_operacao_id` que a engine CIEL-IT vai auto-selecionar para o destino** (v29+ D-V29-1) — o CFOP da LINHA vem da Operação (`l10n_br_ciel_it_account.operacao`), auto-selecionada por **partner/destino**, NÃO da `fiscal_position` (que não carrega CFOP). A engine prioriza Operação com `partner_ids` específico sobre a GENÉRICA (`partner_ids=[]`). Destino **FB cai na Operação genérica 2710 → CFOP 5902 (ERRADO p/ produto acabado tipo 4; correto 5949)**; só destinos com Operação específica por partner (ex: CD via 2719 → 5949) geram o CFOP certo. Custou ~10 NFs LF→FB com CFOP errado (SARET/2026/00003-12) antes de detectar. Para destino novo: confirmar que existe Operação específica (partner_ids contendo o destino) com o CFOP correto ANTES de `--confirmar-sefaz`. Correção de cadastro fiscal (criar/editar Operação) é FORA das skills-átomos — NÃO improvisar XML-RPC; é decisão+execução fiscal.
12. **Status agregado de orchestrator deve detectar TODAS as variantes de "parcial"** (v29+ F1 D-V29-1) — usar `'PARCIAL' in s`, não só `startswith('FALHA')` + tuple. Senão `EXECUTADO_PARCIAL`/`*_TIMEOUT`/`*_MISTO` de uma etapa ESCAPAM e o pipeline reporta `EXECUTADO_OK` mascarando pendências (onda mista parece 100% OK). Espelhar o pattern do guard que já existir (ex.: CR-H4 `'PARCIAL' in status_b`).
...
### AR13 — Caminho novo L3 v19+ regrediu 4 hardenings do legacy v17.5 (descoberto v25+ pós-cirurgia AVULSO_FRASCO)

- **O quê**: ao escrever `_executar_etapa_f_via_fluxo_l3` + `executar_fluxo_l3_1_2_x` em v19+, ESQUECI de auditar TODOS os hardenings que o legacy `executar_etapa_f` v17.5 já tinha. Cirurgia AVULSO_FRASCO (37688un FRASCO FB→LF) revelou 4 gaps simultâneos:
  - **Gap-1 (lote)**: caminho novo não passava `lotes_data` → `lote_default='MIGRAÇÃO'` literal aplicado a TODOS MLs. Legacy v17.5 linhas 3998-4018 FAZIA certo.
  - **Gap-2 (company DFe.line)**: B-V23-1 fix só estava no caminho B (`criar_dfe_a_partir_do_invoice_saida`). Caminho A (DFe via SEFAZ — caso mais comum, 4 de 4 do canary v20+) não tinha fix equivalente.
  - **Gap-3 (company picking+moves)**: G023 force company só estava no átomo legacy `criar_picking_entrada_destino_manual` (picking.py:1391-1399). Picking nativo (gerado via `action_gerar_po_dfe`) não tinha.
  - **Gap-4 (tipo DFe vs PO)**: caminho novo escrevia mesmo tipo nos dois lugares. Tipo correto: DFe='compra', PO='serv-industrializacao' (validado empiricamente por Rafael alterando manualmente PO 42543 via UI).
- **Por que aconteceu**: caminho novo v19+ foi escrito mockado/canary com 1 invoice apenas. Hardenings do legacy v17.5 estavam ESPALHADOS pelo código (4 lugares diferentes); nenhum checklist explícito mapeou "tudo que o legacy faz e o novo precisa fazer".
- **Custo**: 1 cirurgia REAL PROD (v24+) + 1 sessão inteira (v25+ S0) para mapear+codificar 4 fixes F1-F4. ~7 testes novos. Workaround manual permaneceu em PROD para PO 42543 antes do fix codificado.
- **Como evitar**: ao criar caminho novo (FLUXO L3 / opt-in / refator) que substitui legacy validado em PROD:
  1. **Auditoria obrigatória** — abrir o legacy + listar EXPLICITAMENTE cada `write`, cada `force`, cada `transform`, cada lookup de campo. Cada item vira pytest no caminho novo OU é replicado.
  2. **Loop "for ajuste in legacy"** — o legacy lê o quê do DB local? Aplica que transform? Força que campos no Odoo? Cada item DEVE ter equivalente no novo OU rationale explícito do porquê foi removido.
  3. **Spawn subagente diff** — `gestor-estoque-odoo` READ-only com prompt "compare X (legacy) com Y (novo); liste cada hardening de X que não está em Y; classifique se é regressão ou removido intencionalmente."
  4. **Canary REAL DIFERENTE** — não só "1 invoice OK", mas também smoke com lote real (não MIGRAÇÃO), company atravessada, edge cases que o legacy já tratava silenciosamente em PROD.
- **Onde**: `CLAUDE.md §14 D-V25-1` + `CIRURGIA_AVULSO_FRASCO_2026_05_27.md` + N27-N31 desta tabela.

### AR14 — Snippet bash irreversível em folha L3 posicionado ANTES do status canary (descoberto v27+ pós-code-review)

- **O quê**: folha L3 `1.1.1-faturamento-saida-pura.md` v27+ S5 colocou exemplo bash `python -m ...inventario_pipeline ... --confirmar --confirmar-sefaz` na linha 163 (seção "Composição via orchestrator C3"). O bloco "Status v27+ S5 (esta folha)" com aviso "Canary REAL PROD desta folha: pendente" ficou na linha 177 (DEPOIS do snippet).
- **Por que aconteceu**: ao escrever folha L3 documentando pattern arquitetural (composição via opt-in), o foco foi explicar o "como" antes de explicitar o "status de validação". Estrutura de exemplo (snippet) vem antes da meta-informação (status canary) no template original 1.2.1.
- **Risco**: subagente `gestor-estoque-odoo` que lê folha linearmente para "executar fluxo sobre caso real" (invariante 7) pode disparar o snippet `--confirmar-sefaz` ANTES de chegar ao aviso de status. SEFAZ é IRREVERSÍVEL — NF autorizada com chave 35 dígitos só cancela via processo formal 24h sem uso + declaração.
- **Custo**: detectado em code-review v27+ pós-S5; não chegou a PROD nesta sessão (canary REAL S2 deferido). Mas o pattern atual estava plantado para repetir em folhas futuras.
- **Como evitar**: SEMPRE FAZER #9 (acima) — bloco AVISO em negrito ANTES do snippet. Para folhas já escritas (1.1.1 v27+ S5 + 1.3 v27+ S5), reorganizar próxima vez que sessão tocar.
- **Onde**: `app/odoo/estoque/fluxos/1.1.1-faturamento-saida-pura.md:163 vs 177` + CR-reviewer holístico v27+.
