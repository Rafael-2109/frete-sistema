...
| N24 | Criar PO no LF (FLUXO L3 1.2.x caminho A ou B) sem garantir `purchase.team` com user_id permitido — descoberta G039 v22+ (PENDENTE v23+) | PO criada cai em `team_id` default ('Aprovação LF - JOSEFA' user_id=78 Edilane). Se PO precisa aprovação dupla por valor/regra CIEL IT custom, `button_confirm` retorna True mas state fica 'to approve' permanente (button_approve via XML-RPC não destrava). Resultado: sem picking auto, FALHA_PASSO_7_SEM_PICKING no FLUXO L3. **Workaround v22+**: criar `purchase.team` para user_id atual (id=143 'Aprovação LF - RAFAEL' criado para uid=42 Rafael company=5) + write PO `team_id=novo`. **Fix raiz v23+**: Skill 7 codificar este passo. | CLAUDE.md §14 D-V22-3 |
