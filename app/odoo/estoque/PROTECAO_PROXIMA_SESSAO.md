...
| N30 | `escriturar_dfe` escreve `l10n_br_tipo_pedido='serv-industrializacao'` no DFe — **F3 v25+** (✅ CODIFICADO 2026-05-27) | DFe deve receber tipo `'compra'` (destrava `action_gerar_po_dfe` no robô CIEL IT); só na PO+Fatura é que vira `'serv-industrializacao'` (resolve journal ENTIN + CFOP 1949). Evidência empírica: Rafael alterou manualmente PO 42543 'compra'→'serv-industrializacao' via UI antes de confirmar (linha 20 do `CIRURGIA_AVULSO_FRASCO_2026_05_27.md`). **FIX v25+**: `L10N_BR_TIPO_PEDIDO_POR_ACAO` refatorado de `Dict[str, str]` para `Dict[str, Dict[str, str]]` com keys `'dfe'`/`'po'`. INDUSTRIALIZACAO_FB_LF → `{'dfe': 'compra', 'po': 'serv-industrializacao'}`. Passo 3 escreve DFe; passo 5 escreve PO via novo parâmetro opcional `preencher_po(l10n_br_tipo_pedido=...)`; fatura herda no passo 9. | CLAUDE.md §14 D-V25-1 |
