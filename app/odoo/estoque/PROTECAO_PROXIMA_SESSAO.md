...
| N32 | Adicionar `_nova_funcao_modulo` em `inventario_pipeline.py` sem atualizar lista explícita do stub `faturamento_pipeline.py:62-72` (✅ DOCUMENTADO v27+ pós-CR) | Stub re-exporta públicos via `from inventario_pipeline import *` + lista explícita de `_<privado>` (8 nomes manuais). Função/método módulo NOVA prefixo `_` no `inventario_pipeline` NÃO aparece automaticamente no stub. Callers legacy via `from app.odoo.estoque.orchestrators.faturamento_pipeline import _nova_func` quebram silenciosamente com `ImportError`. **Mitigação**: grep `_<nova_func>` no stub + scripts ad-hoc + testes ANTES de commitar qualquer função módulo nova; ou converter para method de classe (`FaturamentoPipelineExecutor._method`) que é re-exportada via instância. Métodos de classe (`_resolver_*`, `_executar_etapa_*_via_*`) NÃO precisam constar no stub — só funções módulo nível. | `faturamento_pipeline.py:62-72` (lista explícita) + CR-reviewer holístico v27+ |
