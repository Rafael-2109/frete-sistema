...
| N25 | Skill 7 `criar_dfe_a_partir_do_invoice_saida` cria dfe.lines com `company_id` herdado do XML da SAÍDA (B-V23-1 v23+ PENDENTE v24+) | Quando XML da saída é FB→LF, `action_processar_arquivo_manual` cria DFe no LF (company_id=5) mas as dfe.lines herdam company_id=1 (FB). Sintoma: passo 9 `action_create_invoice` falha com `<Fault 4: Rafael não tem acesso 'leitura' a dfe.line>`. Causa real: método CIEL IT faz `with_company(dfe.company_id=5)` reduzindo `allowed_company_ids=[5]`; dfe.lines company=1 não passam pela ir.rule 353 nesse contexto. **Workaround v23+ aplicado em PROD**: write `dfe.line.company_id=5` manualmente. **Fix raiz v24+**: codificar no átomo `criar_dfe_a_partir_do_invoice_saida`: após criar DFe, read dfe.lines + write company_id=company_destino. Pytest. | CLAUDE.md §14 D-V23-2 + VALIDACAO B-V23-1 |
| N26 | Skill 7 `gerar_po_from_dfe`/`preencher_po` deixa `PO.line.account_id` apontando para account da company FONTE (B-V23-2 v23+ PENDENTE v24+) | `action_gerar_po_dfe` cria PO.lines no LF (company_id=5) mas `account_id` é resolvido para `account.account` da FB (ex: id=22611 '3202010001 CUSTOS') em vez do equivalente LF (id=26459). Cada code de conta existe em todas 4 companies. Sintoma após fix N25: passo 9 falha com `"Empresas incompatíveis: PO line LF vs Account FB"`. **Workaround v23+ aplicado em PROD**: write PO.line.account_id manualmente para id da LF. **Fix raiz v24+**: criar átomo helper `resolver_account_id_por_company(account_id_fonte, company_destino)` que search por `(code, company_id=destino)` + hook nos átomos Skill 7 que tocam PO.lines. Pytest. | CLAUDE.md §14 D-V23-3 + VALIDACAO B-V23-2 |
