...
|--------------|-----------|--------------|--------|------------------|
| `inventario_pipeline` (renomeado de `faturamento_pipeline` em v27+ S3 — stub alias compat preservado; AP6 RESOLVIDO PARCIAL v24+ + S1 opt-in v27+) | Skill 2 v2 (ETAPA A) + Skill 5 átomos inter-company (ETAPA B) + Playwright SEFAZ (ETAPA D) + Skill 7 atomo V1 STRICT (ETAPA E legacy) + Skill 5 v15a deprecated (ETAPA F legacy) + **v19+ `executar_fluxo_l3_1_2_x`** + **F1-F4 v25+** + **v27+ S1 opt-in `--usar-skill8-atomica-v25`** (helpers `_executar_etapa_c_via_skill8_atomica` + `_executar_etapa_d_via_skill8_atomica` delegam aos atomos 3, 4 e 5 da Skill 8 ATOMICA L2 em `scripts/faturamento.py`; default OFF preserva legacy) | [`orchestrators/inventario_pipeline.py`](orchestrators/inventario_pipeline.py) (~5600 LOC) + [`orchestrators/faturamento_pipeline.py`](orchestrators/faturamento_pipeline.py) (STUB alias compat v27+ S3) | 🟡 **PIPELINE A-F + RECOVERY + FLUXO L3 1.2.x LIVE v19+ + F1-F4 v25+ + S1 opt-in v27+** (90 pytest = 84 + 6 net v27+ S1 dispatch mockado; canary REAL PROD do opt-in pendente proxima INDUSTRIALIZACAO_FB_LF natural) | `.claude/skills/faturando-odoo/SKILL.md` (FACHADA atualizada v27+ S3 — paths apontam para inventario_pipeline) |
...
app/odoo/estoque/
  ...
  orchestrators/                       orchestrators C3 macros L1 (pre_etapa_executor ✅, inventario_pipeline ✅ v27+ S3 [renomeado de faturamento_pipeline; stub alias compat preservado] — pipeline A-F + recovery + opt-in skill8 atomica S1)
...
- **Sintoma 4 (F3)**: `escriturar_dfe` escrevia `l10n_br_tipo_pedido='serv-industrializacao'` no DFe via `L10N_BR_TIPO_PEDIDO_POR_ACAO['INDUSTRIALIZACAO_FB_LF']='serv-industrializacao'`. Mas o tipo correto no DFe é `'compra'`; só na PO+Fatura é que vira `'serv-industrializacao'`. Evidência empírica linha 20 do `CIRURGIA_AVULSO_FRASCO_2026_05_27.md`: Rafael alterou manualmente PO 42543 'compra'→'serv-industrializacao' via UI antes de confirmar.
