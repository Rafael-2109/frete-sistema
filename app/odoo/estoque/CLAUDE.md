...
| `operando-mo-odoo` | mrp.production (cancelar V6 + concluir V7 2026-06-12; criar/alterar sem demanda) | [`scripts/mo.py`](scripts/mo.py) (StockMOService — guards G-MO-01 furo contabil + G-MO-05/06 anti-producao-fantasma no concluir) | C2 | 🟡 **57 pytest verdes** (concluir: dry-run PROD validado 2026-06-12; canary `--confirmar` pendente demanda natural) |
...
| `entrada_retorno_industrializacao` (**WIRE do R2 — NOVO 2026-06-15**) | Skill 7 (`resolver_chave_remessa`, `buscar_dfe`, `garantir_purchase_team`, `alinhar_dfe_lines_company`, `escriturar_dfe`, `gerar_po_from_dfe`, `preencher_po`, `confirmar_po`, `criar_invoice_from_po`, `montar_invoice_entrada_direta`, `marcar_vinculo_r3`, `postar_invoice`) + Skill 5 (`criar_picking_entrada_destino_manual` C9) + `RevaloracaoCustoService` + `DescobertaIndustrializacaoService`. Compõe o **FLUXO L3 1.2.4** (entrada de retorno de industrialização FB←LF). NÃO usa `RecebimentoLfOdooService` | [`orchestrators/entrada_retorno_industrializacao.py`](orchestrators/entrada_retorno_industrializacao.py) | 🟡 **CONSTRUÍDO dry-run-first, validado PROD (READ)** — 8 modos CLI (plan→escriturar-nf1-po→escriturar-nf1-invoice→montar-nf2→postar-nf2→revalorar→postar-nf1→medir), cada escrita gated `--confirmar`; 12 pytest composição. **PENDENTE: execução real (go) + D reconciliar 26489** | (sem fachada; CLI direto + FLUXO 1.2.4) |
| `saida_retorno_industrializacao` (**lado nosso da SAÍDA — NOVO 2026-06-15**) | **READ-only** — compõe `DescobertaIndustrializacaoService` (ORÁCULO) + `resolver_chave_remessa`. NÃO executa (a execução é server-side na SA — ver Tabela 6). `planejar`/`validar`/`medir`. FLUXO L3 1.1.4 | [`orchestrators/saida_retorno_industrializacao.py`](orchestrators/saida_retorno_industrializacao.py) | 🟡 **READ oráculo/validar/medir** (DECISÃO `SOT §6.3` Opção B) — 12 pytest. ZERO escrita Odoo | (sem fachada; CLI direto + FLUXO 1.1.4) |
...
### Tabela 6 — INFRA-ODOO (provisionamento de automação — FORA do sistema de skills)

> **NOVA categoria (2026-06-15, DECISÃO `SOT §6.3` Opção B).** Escrever `ir.actions.server`/`ir.cron` é provisionar **config do ERP**, NÃO operar estoque → falha o teste "qual OBJETO Odoo de operação?" (§1.1 + AR5 + CLAUDE.md:410-412 que já classifica cadastro/config fiscal como FORA das skills). Mora em `app/odoo/estoque/provisioning/`, NÃO em `.claude/skills/` nem `orchestrators/` nem `fluxos/`. O **body** da SA é **string versionada = fonte de verdade**; provisionamento idempotente + monitor anti-upgrade (records custom somem em upgrade CIEL IT — precedente DFE NFD).

| Artefato | Objeto Odoo | O quê | Status |
|----------|-------------|-------|--------|
| `provisioning/sa_retorno_industrializacao.py` | `ir.actions.server` + `ir.cron` | Provisionador idempotente (cria/atualiza SA+cron por `name`) + `verificar` (READ, núcleo do monitor). Bodies versionados: **G1** montagem NF-2 (op 2864 + recompute + remap fp 111 + R3 + post; genealogia `safe_eval` 🔴 PENDENTE CANARY) · **G2** transmissão SEFAZ (fiel `s54`). `dry_run` default; crons gated por canary | 🟡 framework testado (10 pytest); NADA escrito no Odoo |
| `provisioning/monitor_sa_industrializacao.py` + `.claude/hooks/sessionstart_industrializacao_sa.py` | (nosso runtime) | Monitor anti-upgrade LEVE: D8 grava flag (`~/.claude/state/...json`) via `verificar` READ; hook `SessionStart` injeta aviso se SA/cron sumiram | 🟡 6 pytest |
