...
|--------------|-----------|--------------|--------|------------------|
| `faturando-odoo` (Skill 8 nomenclatura confusa — ver AP6) | Skill 2 v2 (ETAPA A) + Skill 5 átomos inter-company (ETAPA B) + Playwright SEFAZ (ETAPA D) + Skill 7 atomo V1 STRICT (ETAPA E legacy) + Skill 5 v15a deprecated (ETAPA F legacy) + **NOVO v19+ `executar_fluxo_l3_1_2_x`** (compõe Skill 7 ABRANGENTE 10 átomos + Skill 5 `preencher_lotes_picking` + Skill 5 `validar` via FLUXO L3 1.2.1/1.2.2) + **F1-F4 v25+** (4 fixes pipeline pos-cirurgia AVULSO_FRASCO: F1 resolve `lotes_data` por invoice a partir de `AjusteEstoqueInventario.lote_destino`; F2a invoca novo átomo `alinhar_dfe_lines_company` no passo 1.5 caminho A; F2b force G023 company picking+moves passo 6.5; F3 tipos separados DFe='compra' PO='serv-industrializacao'; F4 team_id=143 STATIC FIXO p/ LF + G039 by-pass) | [`orchestrators/faturamento_pipeline.py`](orchestrators/faturamento_pipeline.py) (~5200 LOC com pipeline A-F + recovery + método v19+ + fixes v25+) | 🟡 **PIPELINE A-F + RECOVERY + FLUXO L3 1.2.x LIVE v19+ + F1-F4 v25+** (84 pytest verdes = 79 + 5 net v25+ ETAPA F com `_resolver_pids_em_batch` mock + 2 G039 reescritos F4; AP2 reclassificado; ETAPAS E+F legacy preservadas até v20+ canary) | `.claude/skills/faturando-odoo/SKILL.md` (FACHADA — 4 receitas v17.5 + roadmap v19+) |
