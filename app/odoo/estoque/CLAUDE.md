...
**Status:** EM CONSTRUÇÃO (ONDA 0 concluída 2026-05-22; ONDA 0.4 ✅ fechada 2026-05-24 v3 — G019/G020 codificadas no service; **Skill 2 `transferindo-interno-odoo` ✅ MATURADA**; **Skill 5 `operando-picking-odoo` 🟡 ESTENDIDA v15a + F1 IDEMPOTENCIA v15c** — 3 atomos inter-company; **Skill 6 `planejando-pre-etapa-odoo` 🟡 mín viável COMPLETA v9**; **Skill 8 `faturando-odoo` 🟡 PIPELINE COMPLETO A-F LIVE v17** — `app/odoo/estoque/orchestrators/faturamento_pipeline.py` compoe Skill 5 + Skill 2 v2 + Playwright SEFAZ + RecebimentoLfOdooService + atomo Skill 5 entrada destino em pipeline A->B->C->D->E->F. v17 implementa: **C11 ETAPA D F5e SEFAZ** (Playwright serial + idempotencia TRIPLA D8 + HARD_FAIL_CONFIG aborta batch D7 + SNAPSHOT meta D5 + commit_resilient antes/apos cada NF + safe_session_get pos-Playwright + D18 2 niveis `--confirmar`+`--confirmar-sefaz` + MED C-1/C-2 cstat+xmotivo) + **C12 ETAPA E RecLF X->FB** (SEQUENCIAL invocando service externo + idempotencia G-RECLF-3 via UK `odoo_lf_invoice_id` + G-RECLF-2 transfer_status='erro' parcial OK + D17 CFOP 5xxx->1xxx + status='processando' RETOMA via etapa_atual>0) + **C13 ETAPA F atomo Skill 5** (DELEGA criar_picking_entrada_destino_manual + V1 STRICT INDUSTRIALIZACAO_FB_LF + origin idempotente + lote MIGRAÇÃO->INV-{cod}-{YYYYMMDD} + pre-check invoice.state='posted' + situacao_nf='autorizado'). **+11 fixes pos-3 reviewers paralelos**: CRITICAL-1 commit pos-Playwright falha NAO conta sucesso (anti-dupla-SEFAZ) + CRITICAL-2 guard permissivo invoice 1+ F5e_OK (anti-dupla-SEFAZ crash mid-loop) + CRITICAL-3 UK `recebimento_lf.odoo_lf_invoice_id` (migration v17 PROD aplicada — anti-RecLf duplicado) + CRITICAL-4 invoice situacao_nf!=autorizado pula picking F (anti-saldo-fantasma) + HIGH-1 WARN estado inconsistente status=EXECUTADO+fase!=F5e_OK + HIGH-2 docstring D removeu RuntimeError + HIGH-3 status='processando' RETOMA (anti-RecLf orfao) + HIGH-4 svc instanciado dentro loop (anti-vazamento estado) + HIGH-5 produto_tracking real via fetch (anti-D-OPS-5 quebra tracking='none') + HIGH-6 HOJE dentro do loop (anti-lote errado cross-midnight) + HIGH-7 auditoria SKIP_AGG_VAZIO. **502 pytest verdes** (483 + 19 v17: 5 ETAPA D + 4 ETAPA E + 7 ETAPA F + 3 pos-fixes CRITICAL-1/CRITICAL-4/HIGH-3). Smoke dry-run PROD: ETAPA E identifica invoice 629364 PERDA_LF_FB; pipeline A-F cod 105000007 em 746ms. | **Atualizado:** 2026-05-25 v17
