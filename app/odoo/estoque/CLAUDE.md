...
**Status:** EM CONSTRUÇÃO (ONDA 0 concluída 2026-05-22; ONDA 0.4 ✅ fechada 2026-05-24 v3 — G019/G020 codificadas no service; **Skill 2 `transferindo-interno-odoo` ✅ MATURADA**; **Skill 5 `operando-picking-odoo` 🟡 ESTENDIDA v15a + F1 IDEMPOTENCIA v15c** — 3 atomos inter-company + idempotencia por origin em `criar_picking_inter_company` (anti-duplicacao SEFAZ Reviewer D R-OPS-1 conf 95); 64 pytest verdes (+3 idempotencia v15c); **Skill 6 `planejando-pre-etapa-odoo` 🟡 mín viável COMPLETA v9**; **Skill 8 `faturando-odoo` 🟡 ORCHESTRATOR BASE LIVE + 15 FIXES PROD-GRADE v15c** — `app/odoo/estoque/orchestrators/faturamento_pipeline.py` (~1700 LOC) compoe Skill 5 atomos v15a em pipeline A->B->C->D->E->F. C6+C7+C8+F5c implementados; ETAPAS C/D/E/F stubs v16/v17 com refs explicitas para guiar implementer. **v15c aplica 15 fixes de 4 code-reviewers paralelos** (F1 idempot/F2 abort/F3 G014 warn/F4 D11 macro barreira/F5 ETAPA A guard/F6 re-fetch DetachedInstance/F7 D10 dispose stubs/F8 ACAO_PARA_DIRECAO consolidado em `operacoes_fiscais.py`/F9 `_commit_helpers.py` consolidado SSL match tightened/F10 4 constantes ETAPA F importadas/F11 PAYMENT_PROVIDER ref/F12 external_id_operacao populado/F14 contadores estruturados/F15 EXECUTADO_AUTO_CORRIGIDO distinction/F16 Semaphore intra-picking analise). **472 pytest verdes** (435 + 30 v15b + 7 v15c). Smoke dry-run PROD validado (cod 210639522 status DRY_RUN_OK). PRE-FLIGHT sub-skill C5 via subprocess. **MIN seguranca SEFAZ ALCANCADO**: F1+F2 atomicos garantem zero risco de NF duplicada em re-execucao apos SSL drop) | **Atualizado:** 2026-05-25 v15c
