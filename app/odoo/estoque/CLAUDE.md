...
**Status:** EM CONSTRUÇÃO (ONDA 0 concluída 2026-05-22; ONDA 0.4 ✅ fechada 2026-05-24 v3 — G019/G020 codificadas no service; **Skill 2 `transferindo-interno-odoo` ✅ MATURADA**; **Skill 5 `operando-picking-odoo` 🟡 ESTENDIDA v15a + F1 IDEMPOTENCIA v15c** — 3 atomos inter-company + idempotencia por origin em `criar_picking_inter_company` (anti-duplicacao SEFAZ Reviewer D R-OPS-1 conf 95); 64 pytest verdes (+3 idempotencia v15c); **Skill 6 `planejando-pre-etapa-odoo` 🟡 mín viável COMPLETA v9**; **Skill 8 `faturando-odoo` 🟡 ORCHESTRATOR BASE + ETAPA A REAL + ETAPA C F5d + G014 (v16)** — `app/odoo/estoque/orchestrators/faturamento_pipeline.py` compoe Skill 5 + Skill 2 v2 em pipeline A->B->C->D->E->F. v16 implementa: **C10 ETAPA C F5d real** (polling 1800s/40s + sub-etapas via `_invoice_helpers.py` com PERFIL `inventario-inter-company` V1) + **C10.2 ETAPA A real** (substitui guard NotImplementedError por Skill 2 v2 `transferir_quantidade_para_lote_v2`, filtra ACOES_LOTE = {RENOMEAR_LOTE, TRANSFERIR_LOTE}) + **C10.3 G014 pre-check** (`_g014_pre_check_lotes_vencidos` migra qty de lote vencido para lote novo `INV-{cod}-{YYYYMMDD}` via Skill 2 v2 ANTES de criar picking). **`_invoice_helpers.py` NOVO arquivo separado** com 3 helpers F5d.5/.6/.7 + perfis multiplos (Rafael v16 — evita inline na classe contaminando logica generica). **+9 fixes pos-2 reviewers paralelos**: R1F1 validar perfil pre-polling (CRIT 95 anti-poison) + R2F1 guard situacao_nf em `garantir_payment_provider` fallback (CRIT 92 anti-invalidar SEFAZ) + R2F2 incluir 'enviado' nos guards (CRIT 88) + R1F4 substituir `datetime.utcnow()` (banido pelo hook) por `agora_utc_naive` + R2F3 guard situacao_nf em `corrigir_price_zero_em_invoice` + R2F4 guard SEFAZ retorna True (nao falha) + R2F5 SKIP_NAO_MAPEADO auditavel (DEV_FB_LF) + R1F2 G014 partial failure marca cod como falha do chunk + R1F3 commit_resilient False -> continue (anti-cascata). **483 pytest verdes** (472 + 11 v16: 5 ETAPA C + 4 ETAPA A + 2 G014 + ajustes). Smoke dry-run PROD validado em cod 105000007 (4 pickings F5c_LIBERADO detectados + ETAPA A planeja Skill 2 v2). **MIN seguranca SEFAZ ALCANCADO**: F1 idempotencia + R2F1/F2/F3 guards situacao_nf garantem que re-execucoes NAO invalidam chaves fiscais ja autorizadas/enviadas. | **Atualizado:** 2026-05-25 v16
...
| `faturando-odoo` | **SÓ SAÍDA**: NF→robô CIEL IT→SEFAZ | [`orchestrators/faturamento_pipeline.py`](orchestrators/faturamento_pipeline.py) (v15b: ~1300 LOC com pipeline A->B->C->D->E->F; C6/C7/C8/F5c live; C/D/E/F stubs v16/v17) | C3 | 🟡 **base v15b** (30 pytest verdes; smoke dry-run PROD OK; PRE-FLIGHT C5 subprocess; invoca atomos Skill 5 v15a) |
