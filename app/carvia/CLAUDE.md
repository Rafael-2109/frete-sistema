...
| 5 | **Custo Entrega / Despesa Extra** | `CarviaCustoEntrega` | `/carvia/custos-entrega` + `/carvia/despesas-extras` | Lista + Criar + Detalhe (com anexos AJAX) + Editar + **xerox DespesaExtra Nacom** (criar via NF + criar via CarviaFrete + vincular fatura + editar documento) |
...
| CarviaCustoEntrega | `carvia_custos_entrega` | Custos que CarVia pagou/incorreu (DEBITO) — **xerox de DespesaExtra Nacom** (fluxo compra). `numero_custo` CE-### (`gerar_numero_custo()`). `TIPOS_CUSTO`: DIARIA, REENTREGA, ARMAZENAGEM, DEVOLUCAO, AVARIA, PEDAGIO_EXTRA, TAXA_DESCARGA, OUTROS. **`STATUS_CHOICES`**: PENDENTE, VINCULADO_FT, PAGO, CANCELADO. **FKs**: `operacao_id` **nullable** (null no fluxo compra, required no fluxo venda para SSW 222), `cte_complementar_id` nullable (se virou CTe Comp para cliente), `frete_id` nullable (populado automatico), **`fatura_transportadora_id` nullable** (FK direta para FT — padrao DespesaExtra.fatura_frete_id). `transportadora_id` nullable (override opcional de transportadora do pagamento — xerox DespesaExtra). `tipo_documento` VARCHAR(20) (CTE/NFS/RECIBO/BOLETO/OUTROS, sentinel `'PENDENTE_DOCUMENTO'`). `numero_documento` VARCHAR(50) (sentinel `'PENDENTE_FATURA'`). `subcontrato_id` ainda existe mas e LEGADO. `fornecedor_nome`/`fornecedor_cnpj` opcionais. **COM integracao financeira**: FluxoCaixa (por `data_vencimento`), Conciliacao (`tipo_doc='custo_entrega'`, DEBITO), ContaMovimentacao (automatico). **Integridade CE↔FT**: se `fatura_transportadora_id IS NOT NULL`, CE fica **bloqueado** para conciliacao direta — sera pago via propagacao automatica da FT (`pago_por='auto:...'`). @properties: `pode_vincular_fatura`, `transportadora_efetiva`, `usa_transportadora_alternativa`, `status_descricao`. Relationship: `fatura_transportadora` + backref `CarviaFaturaTransportadora.custos_entrega` (dynamic), `transportadora` (override), `frete` (backref `CarviaFrete.custos_entrega`). **Rotas xerox Nacom** em `custo_entrega_routes.py`: `/despesas-extras/nova` (busca NF), `/despesas-extras/criar/<frete_id>`, `/fretes/<frete_id>/despesas-extras/nova`, `/despesas-extras/<id>/{vincular-fatura, editar-documento, excluir, vincular-cte-comp, desvincular-cte-comp}`. **UI**: card "Despesas Extras" no CarviaFrete detalhe (inline xerox `visualizar_frete.html:594-819`), card readonly em FaturaTransportadora `visualizar.html`, subcontrato `detalhe.html` e CTe Comp `detalhe.html` (via include `_card_despesas_extras_readonly.html`). Service: `CustoEntregaFaturaService` (vincular/desvincular/faturas_disponiveis). Migration: `scripts/migrations/carvia_ce_xerox_despesa_extra.{py,sql}` |
