...
**Excecao do Simulador 3D** (`routes/simulador_routes.py`): le (LAZY) `app/embarques` (Embarque/EmbarqueItem), `app/separacao` (Separacao), `app/carteira` (RotaSalva) e `app/monitoramento` (EntregaMonitorada) para pre-preencher a simulacao de carga — resolve NFs do embarque / da rota do mapa / nao entregues -> `CarviaNf` -> motos -> `CarviaModeloMoto`. Helper unico `_resolver_motos_de_nfs(numeros_nf, nf_ids, itens_diretos)`. NAO replicar esse cruzamento em services de negocio.
  - **Fonte das motos por NF (ordem, sem dupla contagem)**: (1) `CarviaNfVeiculo` (1 chassi/linha, match textual do modelo) quando existir — contagem exata; (2) senao `CarviaNfItem.modelo_moto_id`+`quantidade` — MESMA fonte do peso cubado (`MotoRecognitionService.calcular_peso_cubado_nf`). Motivo: NFs por `tipo_fonte='PDF_DANFE'` (a maioria) NAO tem chassis em `carvia_nf_veiculos`, mas tem o item com o modelo detectado — contar so por chassi dava "0 motos" (~110 NFs ATIVAS afetadas, ex. NF 5491 = 1 moto MIA so no item). Itens sem `modelo_moto_id` sao ignorados (consistente com o peso cubado).
  - **Rota do mapa (`/simulador-carga/rota`)**: o mapa monta lotes CarVia via cotacao/pedido (NAO via NF — `app/carteira/services/mapa_service.py`), entao `_resolver_prefill_rota` aceita 4 formatos de `separacao_lote_id`: `CARVIA-NF-{id}` (NF direta), `CARVIA-PED-{id}` (CarviaPedido), `CARVIA-{cot_id}` (CarviaCotacao -> pedidos) e lote NACOM (via `Separacao.numero_nf`). Pedido faturado resolve as motos REAIS pela NF (`CarviaPedidoItem.numero_nf` -> chassi); item sem NF cai no fallback `itens_diretos` (`modelo_moto_id`+`quantidade`) p/ abrir preenchido pre-faturamento. O lado mapa usa `_lotesSelecionados()` (selecao viva), nao a var estatica do load.
    - **Chips removiveis de NF (2026-06-18)**: o prefill devolve `nfs` = breakdown POR NF (`{numero_nf, cliente, municipio, uf, motos[]}`) alem das `motos` agregadas; o frontend (`applyPrefill`) cria 1 chip removivel por NF reusando a infra do fluxo manual "NF nao entregue" (`addNfChip`/`removeNf`/`state.nfsAdicionadas`) e abre o `#simulador-nf-box`. Contagem por NF extraida p/ fonte UNICA `_contar_modelos_por_nf` (compartilhada pela agregacao total e pelo breakdown — sem dupla contagem). `motos` no prefill = SO `itens_diretos` (sem NF, linhas livres); motos COM NF vivem nos chips. `_resolver_motos_de_nfs` agora retorna 4-tuple `(motos, peso_total, items_sem_modelo, nfs)`.
