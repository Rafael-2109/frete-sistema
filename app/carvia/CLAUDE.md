...
- [Regras Criticas](#regras-criticas)
  ...
  - [R24: Cotacao Publica /cotacao — blueprint isolado sem login](#r24-cotacao-publica-cotacao-blueprint-isolado-sem-login)
- [Modelos](#modelos)
...
| [FRAGILIDADES_VINCULOS.md](FRAGILIDADES_VINCULOS.md) | Indice consolidado das 11 fragilidades de integridade de vinculo (frete↔operacao↔sub↔custo) com status + file:line; #1 (link frete↔CTe) corrigido 2026-06-22 |
...
**Excecao das telas de NF** (`routes/nf_routes.py`, 2026-06-22; filtros/badge revisados 2026-06-23): `listar_nfs`/`detalhe_nf` leem (LAZY) `app/monitoramento` (`EntregaMonitorada` origem `CARVIA`, match por `numero_nf`) e `app/embarques` (`Embarque`/`EmbarqueItem`) para os badges de rastreamento e **2 filtros default OFF**: **"Apenas NF nao entregues"** (2026-06-23: era default ON, agora **default OFF**; `?nao_entregue=1` liga; `NOT IN` por `numero_nf`) e **"Apenas Emb. Pendente"** (`?emb_pendente=1`): mostra NFs que NAO sairam da portaria (sem `Embarque.data_embarque`) OU sem embarque algum — exclui as que sairam pelas MESMAS 2 vias do badge (evita divergencia badge<->filtro). Badges na **coluna dedicada "Rastreamento"** (listagem, A ESQUERDA da coluna Valor — Valor NAO contem mais badge nem emitente) + bloco Rastreamento (detalhe), empilhados nesta ordem: **Coletado/Recebido - DD/MM** (modelos NATIVOS `CarviaColeta.data_coletada_em` / `CarviaColetaRecebimento.concluido_em` quando `CONCLUIDO`, via `CarviaColetaNf.carvia_nf_id`) -> **Emb. #### - DD/MM** (link `embarques.visualizar_embarque`; badge **VERDE** com a data quando `Embarque.data_embarque` preenchida = saiu da portaria, senao AZUL sem data; **resolvido pela UNIAO de 2 vias** (2026-06-23): (a) `embarque_itens` CARVIA-* ativo por `nota_fiscal` [pega NF em embarque ANTES do CTe/frete — antes 61 de 330 NFs em embarque ficavam SEM badge, ex. NF 2044] + (b) `CarviaOperacaoNf`->`CarviaFrete`->`Embarque`, mas SO para NF **sem** `EmbarqueItem` CARVIA (EI ativo ja vem por (a); **EI cancelado = a NF saiu daquele embarque** — decisao 2026-06-23; via (b) e fallback so p/ frete sem item, ~3 casos); com >1 embarque por NF prioriza o que ja saiu) -> **Entregue - DD/MM** (`nf_entregue` segue o MESMO criterio do filtro = `entregue=True`, com data opcional). A coluna Emitente trunca a razao social em 40 chars (title = nome completo). O mesmo filtro+badge "Entregue" existe no Nacom `lista_pedidos` (`app/pedidos`, param `nao_entregue`, `NOT EXISTS` por `Pedido.nf`). NAO materializar esses cruzamentos em services de negocio.
...
> **Simulador 3D — 3 ajustes (2026-06-20)**:
> 1. **Empacotamento MONOTONO em apoio** (`bin-packer.js`): o slider "Apoio mín. p/ empilhar" (`minSupport`) era NAO-monotono — afrouxar (10%) podia caber MENOS que apertar (15%) porque o Simulated Annealing (`packOptimized`, orcamento fixo) tomava trajetorias diferentes por threshold. Corrigido com `packMonotonic`/`packLadder`: roda a escada dos thresholds do slider (100%→10%, cold-start cada degrau) e devolve o melhor degrau **>= o apoio pedido** (todo layout valido p/ apoio maior tambem vale p/ menor) — `bestOfLadder`. **Monotono por construcao**: afrouxar nunca posiciona menos. A UI (`simulador-ui.js`) computa a escada **1x por assinatura** (carga+bau+amplitudes, SEM o apoio) e cacheia (`state.ladderCache`), entao mexer so no slider de apoio e lookup instantaneo. Budget da escada `{maxIters:100, maxMs:500}` por degrau (em background, apos o preview guloso). Testes: `bin-packer.test.js` (monotonia + `packMonotonic >= packOptimized`).
> 2. **Render por CAIXA** (nao 1 bloco): `palletizacao_service.calcular_lastro` agora expoe `nx/ny/caixa_x/caixa_y` e `Pallet.to_dict` traz `caixa_x/caixa_y/caixa_z/nx/ny/camadas/total_caixas` (a grade). `bin-packer.js:palletCaixas(item)` gera as caixas (ultima camada parcial; `[]` => fallback 1 bloco) e `expandItems` preserva os campos; `carga-renderer.js` desenha caixa a caixa.
> 3. **Pendencias de palletizacao NAO somem em silencio**: produto Nacom sem cadastro completo (dims zeradas — ~66% do `cadastro_palletizacao`) nao vira pallet. `_resolver_dados_embarque` agora propaga `pendencias` ao prefill e `applyPrefill` chama `mostrarPendencias` (aviso "N produtos sem cadastro"). Causa-raiz original (embarque 6009): cod 4149304 com dims 0×0×0.
...
**Frete = fonte canonica do cubado (2026-06-23):** `CarviaFreteService._peso_cubado_resolvido` (item real com NF) e `peso_total_de_nfs` (backfill manual `frete_routes.py`) resolvem o cubado pela FONTE CANONICA `MotoRecognitionService.calcular_peso_cubado_nf`/`_batch` (itens via `modelo_moto_id`) — NAO mais via `calcular_cubado_por_modelos` da cotacao, que zerava o cubado (→ frete gravava o BRUTO) quando a cotacao nao cobria o modelo. Encerra a re-incidencia D8 #32/#33 (ex.: NF 38312 JET, 675,92 bruto → 867,54 cubado). O Ramo 3 (provisorio SEM NF) segue somando a cotacao.

**Distribuicao de peso entre itens e PROPORCIONAL**, NAO exata por unidade. Detalhes: [CONFERENCIA.md](CONFERENCIA.md).

**Rateio DIRETA com peso 0 (fix 2026-06-24):** `_calcular_custo_rateio` rateia o frete do CAMINHÃO por peso (`proporcao = (peso_grupo or 1)/peso_embarque`). Frete com `peso_grupo=0` (NF sem motos reconhecidas → cubado 0) usa **fallback de 1 kg** (fatia mínima, decisão Rafael 2026-06-25) — NUNCA o frete inteiro nem 0. O bug antigo (`else: proporcao=1.0` quando `peso_grupo` era falsy) atribuía o frete do caminhão todo ao item de peso zero (E-VIBE NF 2044 embarque 6008 → R$12.000). NF de valor relevante + peso 0 = sintoma de motos NÃO reconhecidas (corrigir cubagem à parte). Correção de dados já gravados: `scripts/carvia/corrigir_rateio_peso_zero.py`. Teste: `tests/carvia/test_calculo_custo_rateio_diretra.py`.
...
**Consolidacao do pipeline de expedicao (2026-06-23, Fases 0-6)** — SOT: `docs/superpowers/specs/2026-06-23-carvia-consolidacao-pipeline-expedicao-design.md`. Causa-raiz dos bugs recorrentes (local_cd VM-errado, totais stale, frete nao-gerado, badge cego): CarVia reusava `EmbarqueItem`/`Embarque` sem herdar a maquina de consistencia da `Separacao` Nacom; cada porta fazia recompute manual e "esquecer um" virava bug. 3 PONTOS CANONICOS agora:
- **`reconciliar_embarque_carvia(embarque_id, *, usuario, gatilhos, commit)`** (`embarque_carvia_service.py`) — orquestrador idempotente que TODA porta mutante chama (cotacao, pedido, expandir_provisorio, save do embarque, portaria ×2). Ordem: `local_cd → totais → [commit-trava] → frete → entregas`. Frete por ultimo (rollback InFailedSqlTransaction). Retorna relatorio (`erros`, `frete_aguardando_cds`) que alimenta o flash ao operador.
- **`criar_embarque_item_carvia(...)`** — UNICO ponto que instancia `EmbarqueItem` CARVIA-; herda `local_cd` na criacao (`resolver_local_cd_carvia`: NF→cotacao→DEFAULT). Guard test `tests/carvia/test_factory_embarque_item_carvia.py::test_guard_*` falha o build se um criador novo nao setar local_cd.
- **`app/utils/resolver_embarque_nf.py`** — fonte UNICA do vinculo NF↔Embarque (badge/detalhe/filtro `emb_pendente`), 2 vias (EmbarqueItem por `nota_fiscal` + operacao→CarviaFrete→Embarque). Acaba a divergencia detalhe×listagem.

Fork Nacom/CarVia: o listener da `Separacao` (Nacom) fica INTOCADO; as 2 diferencas reais (CarVia nao-Odoo; CarVia cobra frete) sao branches explicitos, nao codigo paralelo. NB: o skip CarVia em `sync_totais_service._sincronizar_item` e CORRETO (o total do header ja soma CarVia) — NAO remover.

**Reconciliacao frete↔operacao (2026-06-22)**: `frete.operacao_id` (vinculo VENDA) so era escrito na criacao do frete (`_vincular_operacao_existente`, escolhe operacao de MAIOR cobertura) ou manualmente. CTe importado DEPOIS do frete deixava a FK NULL — a UI (`_build_cte_por_frete`, `lancar_cte.html`) le "tem CTe" so pela FK direta e mostrava "Sem operacao" com o CTe EXISTINDO via junction `carvia_operacao_nfs` (em prod: 110 de 114 fretes NULL = 33% dos ativos). Agora: (1) a importacao do CTe re-vincula via `importacao_service._revincular_fretes_da_operacao`; (2) helper canonico `CarviaFreteService.revincular_frete_estrito` / `resolver_operacao_unica_por_nfs` em modo **ESTRITO** (so grava se operacao UNICA; NFs espalhadas por CTes diferentes → PULA, nao adivinha); (3) backfill `scripts/migrations/2026_06_22_backfill_carvia_frete_operacao_id.py`; (4) `_build_cte_por_frete` tem fallback de leitura via junction (tipo `OPERACAO_JUNCTION` = vinculo nao materializado). NUNCA usar `numero_nf` como chave unica (R1) — o match exige `status='ATIVA'`.
...
- `fatura-transportadora` (FT) — bloqueia se ha `CarviaConciliacao` **OU subcontrato com CTe REAL** (`cte_chave_acesso` OU `cte_numero` que nao seja `Sub-%` de freteiro) **OU `status_conferencia=='CONFERIDO'`** (2026-06-26: paridade Nacom `fretes.excluir_fatura` + fecha gap A-7 — reabra via `reabrir_fatura_transportadora` antes de excluir). **Exclusao tambem liberada ao operador `sistema_carvia`** (nao so admin) via botao "Excluir" em `faturas_transportadora/visualizar.html` -> rota `excluir_fatura_transportadora` (`fatura_routes.py`), que reusa `AdminService.excluir_fatura_transportadora` (mesmo cascade/auditoria/guards). Paridade Nacom `fretes.excluir_fatura` (2026-06-24, IMP Talita). **Reversao do `CarviaFrete` (2026-06-26, causa-raiz emb 6075)**: excluir/desanexar NAO basta soltar a FK — `CarviaFreteService.reverter_frete_ao_desfazer_fatura` reverte o frete (freteiro→PENDENTE limpa `valor_cte`/conferencia → reaparece no Lançamento Freteiros; demais→`FATURADO`→`CONFERIDO`) + reseta CE desvinculado (`numero_documento`/`tipo_documento`). Chamado em `excluir_fatura_transportadora` e `desanexar_subcontrato_fatura_transportadora`. Backfill: `scripts/carvia/corrigir_fretes_orfaos_faturado.py`. Testes: `tests/carvia/test_reverter_frete_fatura.py`
...
- **CTe Complementar exibe o TOMADOR como "Cliente" (2026-06-22)**: o `nome_cliente`/`cnpj_cliente` herdados da operacao apontam SEMPRE para o EMITENTE — errado quando `cte_tomador=DESTINATARIO`. As telas listar/detalhe + export usam `papeis_frete.tomador_como_cliente(papeis)` (REMETENTE→`emit`, DESTINATARIO→`dest`; outros codigos→`None`), com fallback para o `nome_cliente` herdado. NAO inverte o SOT — so resolve o lado da NF que o `cte_tomador` aponta.
- **Cliente Comercial da fatura — fallback via item (PARTE 3, 2026-06-22)**: `CarviaClienteService.resolver_clientes_por_faturas_cliente` resolve por `CarviaOperacao`/`CarviaCteComplementar.fatura_cliente_id` (partes 1/2). Faturas geradas por PDF de um CTe Complementar NUNCA registrado como `CarviaCteComplementar` (item com `operacao_id`/`cte_complementar_id` NULL, so `nf_id` resolvido) nao eram alcancadas → PARTE 3 resolve via `CarviaFaturaClienteItem.nf_id -> cnpj_destinatario da NF -> CarviaClienteEndereco(DESTINO)`. Mesma regra (cliente = destinatario), so muda a porta de entrada.
- **Auto-vinculo de CTe Complementar tardio + amarracao documental (2026-06-22)**: quando o XML do CTe Comp e importado DEPOIS da fatura (item ja existe com `cte_numero`, `cte_complementar_id` NULL), `LinkingService.fechar_vinculo_cte_comp_fatura` amarra o item (sem duplicar) e marca o CTe FATURADO. Agora **flag `CARVIA_FEATURE_AUTO_VINCULAR_CTE_COMP` ON por padrao** (`config.py`; era OFF/rollout). O gate `pode_editar()` foi afrouxado para AMARRACAO DOCUMENTAL: numa fatura paga/conferida o vinculo e permitido **se o `valor_total` nao muda** (preencher FK ≠ editar financeiro); so bloqueia (`SKIP_FATURA_BLOQUEADA`) quando o valor mudaria. Acao manual equivalente na UI: botao **"Vincular a fatura"** no detalhe do CTe Comp (`vincular_fatura_cte_complementar`), exibido so quando ha fatura candidata. Passivo historico (CTes importados antes da flag ON): `scripts/migrations/2026_06_22_vincular_cte_comp_fatura_pre_existente.py`.
- **Telas CTe Comp + Custos de Entrega — NF, Tomador, GNRE (2026-06-22)**: listagem de CTe Comp e de Custos de Entrega passam a **exibir a NF** (via operacao->`carvia_operacao_nfs`->`carvia_nfs`, batch `papeis_frete.batch_nfs_por_operacao`) e **filtrar por NF** (busca em CTe Comp; filtro dedicado `nf` em Custos + export). Em Custos de Entrega a coluna "Emitente" virou **"Tomador"** (`batch_papeis_por_operacao` + `tomador_como_cliente`). **GNRE_ICMS**: nao exibe descricao e nao mostra badge "Pendente" de CTe Comp (guia de ICMS nao gera CTe Complementar). Status `VINCULADO_FT` eliminado (migration `2026_06_22_carvia_custo_entrega_remover_vinculado_ft.{sql,py}`).
...
### R24: Cotacao Publica `/cotacao` — blueprint isolado sem login (2026-06-26)

Blueprint `cotacao_publica_bp` em `app/carvia/cotacao_publica.py`, registrado em `init_app`, exposto SEM login. Reusa `CotacaoRapidaService` + parser LLM (`cotacao_rapida_llm`).

- **Rotas**: `/cotacao` (form), `/cotacao/calcular` (persiste + retorna resultado), `/cotacao/upload` (PDF/imagem), `/cotacao/pdf` (download), `/cotacao/cep/<cep>` (lookup CEP), `/cotacao/cidades/<uf>` (autocomplete publico — `/localidades/ajax/...` exige login).
- **Rate-limit por IP**: `app/carvia/utils/rate_limit.py` (`permitir(acao, ip, *, limite, janela_seg)` via Redis). IP via `X-Forwarded-For` first-hop (best-effort; app sem ProxyFix). Degrada aberto se Redis indisponivel. Aplicado em upload/calcular/pdf/cep.
- **Tabela** `carvia_cotacoes_rapidas_publicas`: snapshot de lead (nome, CNPJ, opcoes escolhidas) gravado ao calcular. Migration: `scripts/migrations/2026_06_26_criar_carvia_cotacoes_rapidas_publicas.{sql,py}`.
- **JS compartilhado** `app/static/js/carvia/cotacao_rapida.js`: parametrizado por `#cr-app data-*` + `data-endpoint-localidades`. Usado pela tela com login (`cotacao_rapida_routes.py`) E pela publica (sem duplicar logica).
- PDF e tela **sem** `tipo_carga`/`modalidade` (badges removidos do render em ambas as telas).
- Ultimas 20 cotacoes publicas aparecem no rodape da Cotacao Rapida com login.
- **Prefixo `/cotacao`**: compartilhado com o modulo Nacom `cotacao` (motor PRE-embarque); sem colisao de path atual. A raiz `/cotacao` agora serve a tela publica CarVia.
...
| `carvia_cotacoes_rapidas_publicas` | Snapshot de lead da cotacao publica (nome/CNPJ/opcoes). Sem FK p/ `Usuario` — IP gravado como texto. Ver R24 |
