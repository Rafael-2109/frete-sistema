...
**Excecao das telas de NF** (`routes/nf_routes.py`, 2026-06-22): `listar_nfs`/`detalhe_nf` leem (LAZY) `app/monitoramento` (`EntregaMonitorada` origem `CARVIA`, match por `numero_nf`) e `app/embarques` (`Embarque` via `CarviaFrete.operacao_id`/`embarque_id`) para os badges de rastreamento e o filtro **"Apenas NF nao entregues"** (default ON; `?nao_entregue=0` desliga; `NOT IN` por `numero_nf`). Badges na pilha da coluna Valor (listagem) + bloco Rastreamento (detalhe): **Coletado/Recebido** (modelos NATIVOS `CarviaColeta.data_coletada_em` / `CarviaColetaRecebimento.concluido_em` quando `CONCLUIDO`, via `CarviaColetaNf.carvia_nf_id`) -> **Emb. ####** (link `embarques.visualizar_embarque`) -> **Entregue** (`nf_entregue` segue o MESMO criterio do filtro = `entregue=True`, com data opcional). O mesmo filtro+badge "Entregue" existe no Nacom `lista_pedidos` (`app/pedidos`, param `nao_entregue`, `NOT EXISTS` por `Pedido.nf`). NAO materializar esses cruzamentos em services de negocio.
