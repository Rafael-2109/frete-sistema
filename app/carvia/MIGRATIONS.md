...
- `2026_06_22_backfill_carvia_frete_operacao_id.py` — **DATA FIX (sem DDL)**: preenche `CarviaFrete.operacao_id` NULL via junction `carvia_operacao_nfs` (numeros_nfs → carvia_nfs ATIVA → operacao nao-cancelada). Causa: a importacao do CTe nunca atualiza `carvia_fretes.operacao_id`, entao o CTe importado APOS o frete deixava a FK NULL e a UI mostrava "Sem operacao" (em prod: 110 de 114 fretes NULL eram falso sem-CTe = 33% dos ativos). Modo ESTRITO: grava so quando ha UMA operacao candidata; AMBIGUO (1 NF→N operacoes) e PULADO e listado para revisao manual (regra Rafael). DRY-RUN default; `--confirmar` efetiva. Reusa `CarviaFreteService.revincular_frete_estrito` (mesma logica do hook do import em `importacao_service._revincular_fretes_da_operacao`). Idempotente.
- `2026_06_22_carvia_emissao_cte_comp_valor_base.py` + `.sql` — `carvia_emissao_cte_complementar.valor_base` (NUMERIC(15,2) nullable — valor LIQUIDO a complementar, = valor do CustoEntrega OU valor a cobrar no AVULSO). Destrava a emissao SSW 222 do **CTe complementar AVULSO** (TDE/Diaria sem CustoEntrega): o worker lia o liquido de `emissao.custo_entrega.valor` e crashava (AttributeError) quando nao havia CE. Agora o liquido e persistido na criacao (`CteComplementarService.criar_para_emissao_ssw`) e o worker usa `_resolver_valor_base_ssw` (valor_base > custo_entrega.valor legado > ERRO). O SSW 222 refaz o grossing-up (PIS/COFINS + ICMS ao vivo) sobre esse liquido — NUNCA sobre `valor_calculado` (que ja tem imposto). `ADD COLUMN IF NOT EXISTS` + backfill do liquido das emissoes ja vinculadas a um CE. Idempotente.
