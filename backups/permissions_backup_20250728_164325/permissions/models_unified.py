...
class UserPermission(db.Model):
    """
    Permissões específicas de usuário em qualquer nível da hierarquia
    Suporta herança e sobrescrita
    """
    ...
    # Controle
    custom_override = db.Column(db.Boolean, default=False, nullable=False)  # Sobrescreve herança
    ...
    __table_args__ = (
        ...
        Index('idx_entity_permission', 'entity_type', 'entity_id', 'ativo'),
        CheckConstraint("entity_type IN ('CATEGORY', 'MODULE', 'SUBMODULE')", name='ck_entity_type')
    )
    ...
    def access_level(self):
        """Retorna o nível mais alto de acesso"""
        if self.can_delete:
            return 'delete'
        elif self.can_edit:
            return 'edit'
...
class Vendedor(db.Model):
    ...
    # Relacionamentos
    users = db.relationship('UserVendedor', backref='vendedor_rel', lazy='dynamic')
    permissions = db.relationship('VendedorPermission', backref='vendedor_rel', lazy='dynamic', cascade='all, delete-orphan')
...
class EquipeVendas(db.Model):
    ...
    users = db.relationship('UserEquipe', backref='equipe_rel', lazy='dynamic')
    permissions = db.relationship('EquipePermission', backref='equipe_rel', lazy='dynamic', cascade='all, delete-orphan')
...
class VendedorPermission(db.Model):
    ...
    # Índices
    __table_args__ = (
        UniqueConstraint('vendedor_id', 'entity_type', 'entity_id', name='uq_vendedor_permission'),
...
class EquipePermission(db.Model):
    ...
    # Índices
    __table_args__ = (
        UniqueConstraint('equipe_id', 'entity_type', 'entity_id', name='uq_equipe_permission'),
...
class PermissionTemplate(db.Model):
    ...
    def apply_to_user(self, user_id, applied_by=None):
        ...
        try:
            for entity_type, entities in self.template_data.items():
                for entity_id, perms in entities.items():
                    ...
                    permission.can_export = perms.get('can_export', False)
                    permission.ativo = True
                    permission.reason = f'Template: {self.nome}'
