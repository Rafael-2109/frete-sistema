...
@permissions_bp.route('/unified')
@login_required
@require_permission('admin.permissions')
def unified_admin():
    """Unified permission management interface with cascading checkboxes"""
    try:
        return render_template('permissions/admin_unified.html')
    except Exception as e:
        logger.error(f"Error loading unified admin interface: {e}")
        return render_template('error.html', error="Erro ao carregar interface unificada de permissões"), 500
