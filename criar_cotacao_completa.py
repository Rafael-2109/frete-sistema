#!/usr/bin/env python3
"""
Script FINAL para criar cotação no Odoo com impostos e Incoterm CIF
Data: 2025-01-25
"""
...
def criar_cotacao_completa():
    """
    Cria cotação no Odoo com impostos calculados e Incoterm CIF
    """
    try:
        # Conectar ao Odoo
        ...
        # 3. Criar cotação com INCOTERM
        logger.info("📝 Criando cotação com Incoterm CIF...")
        ...
        cotacao_data = {
            ...
            'note': 'Cotação com impostos e Incoterm CIF'
        }
        ...
        if cotacao:
            ...
            if cotacao[0].get('payment_term_id'):
                logger.info(f"   💳 Pagamento: {cotacao[0]['payment_term_id'][1]}")
            ...
            logger.info(f"\n🎯 RESUMO FINAL:")
            logger.info(f"   ✅ Cotação {cotacao[0]['name']} criada com sucesso")
...
if __name__ == "__main__":
    logger.info("🚀 Criando cotação completa com impostos e Incoterm CIF...")
    logger.info("="*70)
    
    cotacao_id = criar_cotacao_completa()
    
    if cotacao_id:
        logger.info(f"\n✅ SUCESSO TOTAL!")
        logger.info(f"🆔 Cotação ID: {cotacao_id}")
        logger.info("💡 Cotação criada com:")
        logger.info("   • Impostos calculados corretamente")
        logger.info("   • Incoterm CIF aplicado")
        logger.info("   • Empresa NACOM GOYA - CD")
        logger.info("📋 Acesse no Odoo: Vendas > Cotações")