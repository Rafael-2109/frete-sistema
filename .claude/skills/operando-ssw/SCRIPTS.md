...
Cota frete no SSW (opcao 002). Preenche formulario e executa `calcula('S')` para simular proposta.
Retorna resultado agrupado por categoria (transporte, seguros, taxas, impostos) com indicadores extras.
...
| --valor | **Sim** | — | Valor da mercadoria em R$ (ex: 14649.73) |
| --cep-origem | Condicional | CEP CarVia se coletar=N | CEP de coleta/origem (8 digitos). Ver regra abaixo |
...
| --defaults-file | Nao | ../ssw_defaults.json | Caminho para ssw_defaults.json |
| --dry-run | — | — | Preenche formulario sem simular |
| --discover | — | — | Modo exploratorio: lista todos os campos do formulario |
...
**Impacto**: O CEP origem determina a ROTA no SSW. CEP errado = tabela de preco errada.
Exemplo: CEP cliente em Amparo/SP → rota CARR-GIGR (Generica). CEP CarVia em Santana de Parnaiba/SP → rota CARP-GIGR (Promocional).

### Workflow do Agente

O agente DEVE seguir este fluxo ao receber pedido de cotacao SSW:

```
1. PERGUNTAR ao usuario (obrigatorios):
   - CNPJ do pagador (quem paga o frete?)
   - CEP destino (para onde entregar?)
   - Peso em kg
   - Valor da mercadoria em R$

2. PERGUNTAR ao usuario (decisoes de negocio):
   - Coletar? (S=CarVia coleta no remetente / N=remetente entrega na CarVia)
   - Entregar? (S=CarVia entrega no destino / N=destinatario retira)

3. INFORMAR ao usuario o que sera ASSUMIDO (com base em defaults):
   "Vou assumir os seguintes parametros (voce pode alterar):
    - Frete: CIF (emitente paga)
    - Contribuinte: S (destinatario contribuinte ICMS)
    - Cubagem: automatico (SSW calcula pelo peso)
    - CEP origem: [se coletar=N] 06530581 (CEP CarVia, Santana de Parnaiba/SP)"

4. EXECUTAR --dry-run primeiro → mostrar preview
...
O script rastreia quais parametros foram assumidos (nao fornecidos explicitamente pelo usuario).
O campo `parametros_assumidos` no retorno JSON lista cada assuncao com descricao:

```json
{
  "parametros_assumidos": {
    "frete": "CIF (emitente paga o frete)",
    "coletar": "S (CarVia coleta no cliente)",
    "entregar": "S (CarVia entrega no destino)",
    "contribuinte": "S (Destinatario contribuinte ICMS)",
    "cubagem": "automatico (SSW calcula pelo peso)",
    "cep_origem": "06530581 (CEP CarVia — coletar=N, cliente entrega na CarVia)"
  }
}
```

O agente DEVE exibir `parametros_assumidos` ao usuario antes e depois da simulacao para transparencia.
...
Login → abrir 002 → `ajaxEnvia('NEW', 1)` → interceptar → injetar DOM → re-override createNewDoc → preencher campos via `preencher_campo_js` → `calcula('S')` → aguardar resultado → capturar RESULT_FIELDS agrupados

**Formato de valores**: Script converte automaticamente ponto decimal para virgula (1894.405 → "1.894,405").

### Resultado Agrupado

O retorno `proposta` e organizado em 6 categorias:

| Categoria | Campos SSW | O que representa |
|-----------|-----------|------------------|
...
| **impostos** | `impostos` | Imposto repassado na proposta |
...
**Indicadores extras** (calculados pelo script):
- `ad_valorem_pct`: % do Ad Valorem sobre valor NF
- `gris_pct`: % do GRIS sobre valor NF
- `seguro_total` e `seguro_total_pct`: soma Ad Valorem + GRIS (componente de seguro total)

**INFO_FIELDS** (7 campos readOnly informativos):
`cidadeori`, `cidadedest`, `unid_col`, `unid_ent`, `dias_entrega`, `prev_ent`, `pesocalculo`

### Formato de Apresentacao (para o agente)

Ao apresentar o resultado ao usuario, usar esta estrutura:

```
COTACAO SSW — [ORIGEM] → [DESTINO]
Rota: [ROTA] | Tabela: [nome da tabela se disponivel]
...
Prazo: X dias | Previsao entrega: DD/MM/AA
...
1. **`contribuinte` OBRIGATORIO**: Sem ele, `calcula('S')` mostra overlay DOM "Informe se destinatario e contribuinte" e retorna zeros. O overlay NAO e um browser alert — e um `<div>` no DOM.
2. **`tp_frete` numerico**: SSW usa `1`=CIF, `2`=FOB. NAO usar letras. Script converte automaticamente.
3. **`calcula('S')` NAO e ajaxEnvia**: E uma funcao JS que valida campos e chama ajaxEnvia internamente.
4. **`ajaxEnvia('NEW', 1)` para nova cotacao**: A tela inicial mostra listagem. `NEW` abre formulario. NAO usar `INC`.
5. **Re-override createNewDoc**: Necessario apos cada `injetar_html_no_dom` — a injecao reseta funcoes JS.
6. **Rota**: Resultado inclui rota (ex: "CARP-GIGR"). CARP = CarVia Polo (quando coletar=N). CARR = rota generica.
7. **Ad Valorem = Seguro**: O campo `fretevalor` no SSW e o componente de seguro (Ad Valorem sobre valor NF). NAO existe campo "seguro" separado na 002. O seguro total = `fretevalor` + `gris`.
...
```json
{
  "sucesso": true,
  "cotacao": {
    "cnpj_pagador": "33119545000251",
    "origem": "SANTANA DE PARNAIBA/SP",
    "destino": "NOVA IGUACU/RJ",
    "peso_kg": 1894.405,
    "valor_nf": 14649.73,
    "frete": "CIF",
    "coletar": "N",
  ...
  "proposta": {
    "transporte": {"frete_peso": "2.068,01", "despacho": "14,40"},
    "seguros": {"ad_valorem": "73,25", "gris": "38,09", "seguro_fluvial": "0,00"},
    "taxas": {"coleta": "0,00", "entrega_geral": "0,00", "pedagio": "4,44", ...},
    "impostos": {"impostos": "0,00"},
    "totais": {"total_frete": "2.189,19", "tabela_ntc": "...", ...},
    "indicadores": {"rota": "CARP-GIGR", ...}
  },
  "indicadores": {
    "ad_valorem_pct": "0.50%",
    "gris_pct": "0.26%",
    "seguro_total": "111,34",
    "seguro_total_pct": "0.76%"
  },
  "parametros_assumidos": {
    "frete": "CIF (emitente paga o frete)",
    "contribuinte": "S (Destinatario contribuinte ICMS)",
  ...
  "info": {"cidade_origem": "SANTANA DE PARNAIBA/SP", "cidade_destino": "NOVA IGUACU/RJ", "prev_entrega": "20/02/26"},
  "screenshot": "/tmp/ssw_operacoes/002_resultado_CNPJ_TIMESTAMP.png"
}
```
