...
- [2026-05-25-1](atualizacao-2026-05-25-1.md) — 1611/1699 testes (94.82%), 561.63s. 33 falhas + 48 errors + 7 skipped + 1 collection error (`test_todos_parser.py` ImportError `_try_parse_todos`, ignorado). Resultado quase identico ao baseline 2026-05-18: motos_assai (28 FAILED — 22 fixtures PDF/XLSX ausentes + 6 logica) + hora (34 ERROR — `hora_loja_cnpj_key` state pollution) + motos_assai/carregamento_service_crud (14 ERROR — SQLite x ARRAY em `agent_improvement_dialogue.affected_files`) + custeio (2 FAILED — state pollution `TEST_C2_010` + app context margem) + agente/sdk (2 FAILED — race async_event reincidente) + carvia (1 FAILED — `listar_fretes_divergentes` ausente). Sem correlacao D4 (Sentry triagem so modificou `text_to_sql.py` do skill `consultando-sql`, sem testes em `tests/`). +285 testes coletados vs 2026-05-18.
