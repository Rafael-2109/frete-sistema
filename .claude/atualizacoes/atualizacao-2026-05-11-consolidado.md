...
| 5 | Test Runner | PARCIAL | 963/1019 passed (94.50%), 49 falhas em 5 modulos (motos_assai 22, hora 16, custeio 6, carvia 3, agente/sdk 2). 20 nao coletados (conftest pytest_plugins deprecated). |
...
### Tests
- Total: 1019, passed: 963, failed: 49, skipped: 7, nao_coletados: 20
- Taxa: 94.50% (queda de 97.78% em 2026-05-05; +253 testes coletados)
- Tempo: 264.48s
...
49 testes falharam em 5 modulos. Acoes recomendadas:

1. **motos_assai (22 falhas)** — Bloquear modulo via `--ignore` ate criar fixtures PDF/XLSX em `tests/motos_assai/fixtures/`
2. **hora (16 falhas)** — Atualizar fixtures: trocar `modalidade_frete='9'` por `'0'` (CIF) ou `'1'` (FOB)
3. **custeio (6 falhas)** — Aplicar migrations locais (`audit_log_custeio`, `custo_frete.ativo`, constraints unique/check)
4. **carvia (3 falhas)** — 2 reincidentes mock SSW desde 2026-04-27 (test_a3_ctrnc_cte_comp.py); 1 metodo ausente em `ConferenciaService.listar_fretes_divergentes`
5. **agente/sdk (2 falhas)** — Race condition em `async_event.is_set()` apos submit/cancel (test_pending_questions.py)
6. **conftest motos_assai** — Mover `pytest_plugins` para `tests/conftest.py` top-level (pytest 8.4 deprecation)
