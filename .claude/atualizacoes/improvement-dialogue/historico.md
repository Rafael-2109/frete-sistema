...
- **IMP-2026-06-12-003 "embarque DIRETA grava tabela_* em EmbarqueItem"** (warning, gotcha_report): **REJEITADO**. O fluxo de cotacao JA direciona correto (DIRETA→pai `app/cotacao/routes.py:1418,1572`; FRACIONADA→itens `:1429,:2103-2104`). Causa real do embarque 5813 = criacao **manual** fora do fluxo (Talita, SQL cru) — mesma raiz de IMP-2026-06-09-001 (memoria `diagnostico_travamento_agente_falta_ferramenta`). Fix correto = skill que chame o servico (demanda-driven, gate humano).
