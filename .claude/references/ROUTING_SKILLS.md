...
| DEVOLUCOES NACOM (NFD, retornos, descarte) | "devolucoes pendentes", "status NFD", "De-Para confianca", "descarte vs retorno", "produtos devolvidos" | -> Subagente `gestor-devolucoes` (Nacom Goya; **NFd de moto Q.P.A./Assaí -> `corrigindo-dados-assai`**, dominio isolado) |
...
2. FINANCEIRO?
   ...
   |-- Auditar reclassificacao contabil em lote (READ: medir saldos / validar lote-alvo / monitorar andamento) -> Subagente auditor-financeiro (skill auditando-reclassificacao-odoo)
...
| gestor-devolucoes vs corrigindo-dados-assai | **Devolucao Nacom Goya** (alimentos, tabelas `nf_devolucao*`) -> gestor-devolucoes. **Devolucao de moto Q.P.A./Assaí** (NFd, tabelas `assai_devolucao_nfd*`, FATURADA->PENDENTE) -> `corrigindo-dados-assai --registrar-devolucao-nfd`. Sinal: "Q.P.A.", "Assaí", "moto", "Sendas" -> corrigindo-dados-assai |
