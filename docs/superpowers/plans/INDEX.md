...
- [CarVia: Consistência de Comissões — Ajustes (débito/crédito) + data de corte](2026-06-15-carvia-comissao-ajustes.md) — alteração/cancelamento de cte_valor de CTe já comissionado gera CarviaComissaoAjuste (delta) abatido no próximo fechamento do vendedor (FK usuarios); bloqueia total negativo; criação só com data final; 11 tasks TDD (helpers flush-only)
