...
- [x] **Step 0 (recon — FEITO 2026-06-03):** estrutura confirmada (`step_judge.py:_judge_core` + PROD):
  `{"label": "success|partial|failure", "score": 0-100, "evidencia": str, "componente_culpado": "reasoning|odoo|null}`.
  **Mapeamento judge → `AgentEvalCase`:** `agent_name='__online_judge__'` (marcador fixo — o A3/subagentes morreu, sem conflito); `case_id=step_uid`; `case_score=score/100`; `status='pass' if score>=70 else 'fail'` (mantém o domínio pass/fail/error da coluna; label real vai na evidence); `git_sha=NULL` (online judge não é versionado por sha de código); `n_runs=1`, `variance=0`, `invoke_failures=0` (defaults); `evidence=f"label={label} score={score} | {evidencia[:400]}"` + sufixo `" | ⚠ADVERSARIAL REFUTOU: {adv_reason[:300]}"` quando `judge.label='success' ∧ verify.adversarial.refuted=true` (flag de prioridade — Task 3). Confirmação viva do viés de credulidade: step `:6` teve score=85/success com a própria evidência admitindo "não explicitou se os arquivos foram DE FATO exportados" — e o adversarial refutou.
...
- ✅ **T5** UI de spot-check humano. **(model)** `AgentEvalCase.record_human_verdict` (valida agree|disagree, FLUSH, caller commita); **(service)** `insights_service.get_judge_calibration_panel` (`concordance_rate` + `sample_unreviewed('__online_judge__')`, UNION garante prioritários ⚠ADVERSARIAL sempre visíveis — achado Task 3) + `_serialize_calibration_case` puro; **(rotas)** GET `/agente/api/insights/judge-calibration` + POST `…/verdict` (admin-only, gate `USE_AGENT_INSIGHTS`, CSRF ativo); **(UI)** card "Calibracao do Judge" em `insights.html` (KPI concordance + tabela com botões acertou/errou + badge nos prioritários) + JS no `DOMContentLoaded`. Link de menu já existe (`_sidebar.html` → `pagina_insights`; card dentro, padrão O0.3). TDD 8 testes; 231 suíte agente verdes; lint UI=0.
- ✅ **T6 (código+deploy)** code-review adversarial (6 agentes; 1 finding LOW corrigido `19aae4736`) → merge limpo de `origin/main` (interseção zero) → push `origin/main`@`40164221b` → flag `AGENT_CALIBRATION_SAMPLER=true` no web `srv-d13m38vfte5s738t6p60` (deploy `dep-d8g9v3k2m8qs73e1tlg0`, código+flag). Sem DDL. Baseline `agent_eval_case`=0. **A confirmar (operacional):** deploy LIVE + `agent_eval_case` crescer (sampler por-ciclo ~30min). **GATE-1 formal:** + coleta ≥1 semana + rotular ≥10 → concordance≥80% (tempo/humano) — sem código pendente.
