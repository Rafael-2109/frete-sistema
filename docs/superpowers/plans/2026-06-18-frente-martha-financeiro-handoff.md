...
- [ ] **1a. Parser + PREVIEW (READ, dry-run, ZERO escrita):** ler planilha de credores → localizar fatura por `name` (CMPMP, match único `name+company_id+partner_id`) → validar saldo (`abs(residual)`)/company(da fatura)/partner → calcular os pares SICOOB+DESÁGIO por vencimento → relatório + planilha anotada com o plano. **Testável agora, sem risco.**
