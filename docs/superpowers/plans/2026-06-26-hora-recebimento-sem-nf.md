...
- **Migrations duais** (regra `~/.claude/CLAUDE.md`): par `.sql` idempotente + `.py` espelho. Próxima = `hora_54`.
...
- [Task 1: Migration hora_54](#task-1-migration-hora_54)
...
|---|---|---|
| `scripts/migrations/hora_54_recebimento_sem_nf.{sql,py}` | Criar | coluna `tipo` + tabela `hora_recebimento_esperado` |
...
### Task 1: Migration hora_54

**Files:**
- Create: `scripts/migrations/hora_54_recebimento_sem_nf.sql`
- Create: `scripts/migrations/hora_54_recebimento_sem_nf.py`
...
```sql
-- Migration HORA 54: Recebimento por filial sem NF (NF provisória).
...
Uso:
    python scripts/migrations/hora_54_recebimento_sem_nf.py
"""
import logging
import os
import sys
...
def main() -> None:
    ...
    with app.app_context():
        ...
        if not (ok_col and ok_tab):
            print('\nERRO: migration HORA 54 incompleta.')
            sys.exit(1)
...
Run: `source .venv/bin/activate && python scripts/migrations/hora_54_recebimento_sem_nf.py`
Expected: "Migration HORA 54 concluida com sucesso."

- [ ] **Step 4: Commit**

```bash
git add scripts/migrations/hora_54_recebimento_sem_nf.sql scripts/migrations/hora_54_recebimento_sem_nf.py
git commit -m "feat(hora): migration hora_54 — tipo NF provisoria + hora_recebimento_esperado"
```
