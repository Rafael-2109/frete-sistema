...
- **H4 ✅ CONFIRMADO** — branch estava **30 commits behind** origin/main (não 8). `merge-tree` dry-run: **zero conflitos** (interseção de arquivos tocados = só 2 docs). Merge limpo, sem push, HEAD `ddcf830e5`. Re-rodado na base reconstruída: **`tests/agente` 1603 passed / 2 failed / 39 skipped**, **`tests/teams` 76 passed**, smoke 9/9 imports do path handoff/eviction. As **2 falhas são PRÉ-EXISTENTES em origin/main** (commit `042705ddc2`, 16/06 — refactor `os.getenv→_env_bool`; testes frágeis `test_directive_promotion`/`test_gerindo_agente_skill` de OUTRAS features que casam a string literal `os.getenv`), **NÃO regressão** do merge (diff do merge nesses arquivos = vazio) nem do PR #28. Pendente opcional: log prod `"Dead client evicted"` (flag OFF ⇒ ~0 eventos).
