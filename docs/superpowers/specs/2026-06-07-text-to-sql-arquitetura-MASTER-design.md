...
**Gate S2 — qualidade de schema**
- [ ] Descricao curada na fonte (modelo) APARECE no `tables/<t>.json` gerado E em
      `consultar_schema` (teste).
- [ ] Overlay de `business_rules`/`query_hints` aplicado e exibido (teste).
- [ ] Top-40 por uso: descricao em >=90% dos campos + regras/hints presentes.
