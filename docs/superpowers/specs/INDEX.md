...
- [TagPlus → WhatsApp: notificação de Pedido criado e NF emitida (réplica N8N)](2026-06-06-tagplus-notificacao-whatsapp-design.md) — webhook dedicado `pedido_criado`/`nfe_criada` → grupo único de vendas + DM do vendedor (via cadastro usuarios, fallback só-grupo); NF com PDF da DANFE anexado (base64 via gateway OpenClaw); thread async best-effort; reusa send_whatsapp + OAuth notas; pré-req scope read:pedidos
