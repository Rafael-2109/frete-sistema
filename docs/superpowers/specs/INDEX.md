...
- [CarVia — Consistência de comissões: ajustes (débito/crédito) + filtro por data de corte](2026-06-15-carvia-comissao-ajustes-design.md) — alteração/cancelamento de `cte_valor` de CTe já comissionado gera `CarviaComissaoAjuste` (delta) abatido no próximo fechamento do mesmo vendedor; nunca total negativo (bloqueia criação); tela de criação só com data final; aguardando review
