...
| O0.2 | 🟢 **SHADOW** (2026-06-03) — a fila `agent_validation` JÁ estava no `--queues` dos 3 arquivos (`f50df8421`, em PROD); o card+MED-1 estavam obsoletos. O defeito REAL era arquitetural: worker em container Render separado NÃO acessa o transcript em `/tmp` do web → `get_subagent_summary` dava status=error → job abortava (verificado PROD: 0 `[validator]`, `subagent_validations`=0). **FIX**: hook SubagentStop computa o summary no web (FS local) e o passa no payload do job (`summary_dict`); worker usa o payload, não relê FS. TDD +2 (consumidor+produtor), 764 verdes. **Próximo: deploy + confirmar `AGENT_SUBAGENT_VALIDATION` ON em PROD + observar `[validator] concluido` e `subagent_validations`>0 → 🔵** | wiring+bugfix | `AGENT_SUBAGENT_VALIDATION` | 🔵 | `subagent_validations`>0 em PROD pós-deploy + log `[validator] concluido` | — |
