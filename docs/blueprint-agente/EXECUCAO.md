...
| **A3** eval-gate | `eval_gate_service.py` (`run_evals`/`eval_gate`) + `build_subprocess_invoke_fn` + `workers/eval_runner.py` + tabela `agent_eval_scores` + módulo 28 D8 | 🛑 **APOSENTADO — NÃO ATIVAR (2026-06-03, decisão Rafael)**: eval LLM caro VETADO (`[[feedback_evals_llm_caros_preferir_pytest]]`). `AGENT_EVAL_GATE`=false em PROD (web, deploy `dep-d8g865eq1p3s73erk4m0`); fix `_resolve_claude_cli` REVERTIDO (não commitado). **Histórico**: FASE 1 wirada (`9042023bb`, flag-OFF) — `build_subprocess_invoke_fn`=`claude -p --agent <nome>`, job RQ `run_eval_batch` (fila `agent_eval` PESADA), módulo 28 enfileira report-only. **Por que aposentado**: ligar = ~105 invocações Opus/dia (~$20/dia); o sinal de qualidade vem do **judge (E2) + verify (B2)** (Haiku, já ON). GATE-1 NÃO depende mais do A3 → calibração do judge (E3). `eval_gate()` puro permanece (usado pelo A4). Destino do código: **manter OFF+documentado** (decisão Rafael 2026-06-03; sem remoção). |
...
4. **A3-invoke** 🛑 **APOSENTADO (2026-06-03)** — eval LLM caro VETADO; `AGENT_EVAL_GATE`=false em PROD, fix revertido. NÃO está no caminho crítico; GATE-1 depende da calibração do judge (E3). → **A4-batch** já LIVE em PROD (Onda 3).
...
**Próximo passo (ordem; nada ligado — regra de ouro intacta):** (1) **A3 APOSENTADO ✅ EXECUTADO 2026-06-03** (eval LLM caro VETADO — decisão Rafael): fix do `eval_runner` revertido (`git restore`, 34 testes verdes) + `AGENT_EVAL_GATE`=false no web (deploy `dep-d8g865eq1p3s73erk4m0`) + re-framing do GATE-1 (agora depende da calibração do judge **E3**, não do A3) — ver `PROMPT_PROXIMA_SESSAO_A3_DESATIVACAO.md`; (2) **E3 calibração do judge** (`AGENT_EVAL_CALIBRATION` + spot-check 5-10%; reconciliar judge↔adversarial; bug `arithmetic.ok`); (3) decisão de design **B1 combustível** (triage→PlanState vs decompor via Task); (4) coletar ≥1 semana úteis; (5) manter `AGENT_DIRECTIVE_PROMOTION` OFF. **Refino desta sessão (b+a, feitos):** **(b)** fix do `eval_runner` **confirmado** = resolver o CLI bundled do SDK (`claude_agent_sdk/_bundled/claude`, que JÁ existe no worker via `pip`) em `build_subprocess_invoke_fn` (`eval_gate_service.py:224`, único ponto) — opção (b) refinada, ~5 linhas, 1 arquivo, destrava web+worker (detalhe em BLOQUEIOS ATIVOS). **(a)** `gkpj`/30min **descartado como blueprint**: ocorre no INÍCIO do ciclo D8 (antes dos Steps 29-32, que completam OK), pré-existente desde 01/06, sem issue Sentry nova atribuível; causa-raiz exata não-determinada (provável módulo de sync anterior), baixa prioridade.

**DECISÃO (Rafael, 2026-06-03):** o A3 é um eval LLM caro (~105 invocações Opus/dia se ligado) — **VETADO**. NÃO ativar; o fix do `eval_runner` NÃO será commitado (reverter na próxima sessão); GATE-1 re-framing para depender da calibração do judge **E3**, não do A3. Plano de execução: `PROMPT_PROXIMA_SESSAO_A3_DESATIVACAO.md`.
...
**GATE-1** (re-framed 2026-06-03 — **NÃO depende do A3**, que foi aposentado): sinal de qualidade step-level gravado e auditável por ≥1 semana **de dias úteis** + **judge calibrado via E3** — spot-check humano 5-10% em `agent_eval_case` (hoje 0; concordância ≥X% com held-out) + reconciliar judge↔adversarial (discordam **63%**, 127/201) + investigar bug suspeito `verify.arithmetic.ok=false` (42/201 com evidência textual "OK"); E↔D validado (ontologia não piora retrieval medido pelo sinal). **A medição de qualidade é BARATA (Haiku: judge E2 + verify B2, já ON em PROD) — sem evals LLM caros (A3 vetado).**
...
| Item | Descrição | Dep | Flag | Esf | Status |
|------|-----------|-----|------|-----|--------|
| **A3** | Eval runner automatizado + gate no D8 (golden datasets `evals/`). Processo EXTERNO ao agente avaliado, report-only→enforce. | ~~GATE-1~~ | `AGENT_EVAL_GATE` | M | 🛑 **APOSENTADO — NÃO ATIVAR (2026-06-03, decisão Rafael)** — eval LLM caro VETADO (`[[feedback_evals_llm_caros_preferir_pytest]]`). `AGENT_EVAL_GATE`=false em PROD (web); fix `_resolve_claude_cli` revertido (não commitado). Código histórico: `f5883709b` (`eval_gate_service.py` + módulo 28 D8). **GATE-1 NÃO depende mais disto** → judge calibrado (E3). `eval_gate()` puro permanece (usado pelo A4). Destino do código: **manter OFF+documentado** (decisão Rafael 2026-06-03; sem remoção). |
...
- 2026-06-03 (tarde) — **A3 APOSENTADO (eval LLM caro VETADO — decisão Rafael)**, exec via `PROMPT_PROXIMA_SESSAO_A3_DESATIVACAO.md`: (1) fix `_resolve_claude_cli` **REVERTIDO** (`git restore eval_gate_service.py test_build_invoke_fn.py`; working tree limpo; invoke voltou a `'claude'` literal :224; **34 testes verdes** pós-revert). (2) `AGENT_EVAL_GATE`=false no web `srv-d13m38vfte5s738t6p60` (`update_environment_variables` merge — env PERSISTIDA). O deploy que o `update` disparou (`dep-d8g865eq1p3s73erk4m0`) foi **cancelado/superseded** ~2min depois por um auto-deploy do commit `4b17efa6c` do Rafael (merge hora) = **`dep-d8g865eq1p3s73erk4m0`, LIVE 19:47Z**, que reiniciou o web carregando a env nova. Módulo 28 (`sincronizacao_incremental_definitiva.py:109` lê `EVAL_GATE_ENABLED` no import) vira no-op; scheduler roda no WEB (`start_render.sh:349`), por isso desligar no web basta (o gate-lazy `eval_runner.py:619` no worker é redundante). Confirmação observável (tabela não cresce) só na próxima janela D8 (~11h, hour-guard). (3) **re-framing GATE-1** → depende da calibração do judge **E3** (Haiku + spot-check humano `agent_eval_case`), NÃO do A3. `eval_gate()` puro PRESERVADO (usado pelo A4 — log `[eval_gate] Gate OK baseline=0.700`). **Decisões Rafael (2026-06-03):** código A3 = **manter OFF+documentado** (sem remoção — `eval_gate()` fica para o A4); 16 linhas `score=0` = **manter como histórico** (não deletar — inócuas, não lidas pelo A4 nem pelo E3). **Pendência:** commit+push deste doc (`EXECUCAO.md` editado nesta sessão; `main` local == `origin/main`@`4b17efa6c`, push seria fast-forward) — o commit `31abb5fb9` (decisão A3) **JÁ está em `origin/main`** (deployado). **Confirmação "parou de gravar" só na próxima janela D8 (~11h amanhã).** Próximo: E3 (calibração do judge — Haiku + spot-check humano). **Achado E3 (verificado em PROD nesta sessão):** o `verify.arithmetic.ok=false` 42/201 é **bug do parser** `verifiers.py:104-110` — o `_call_sonnet_verifier` devolve RACIOCÍNIO passo-a-passo concluindo com "OK"/✓ (aritmética correta), mas o parser só aceita `resultado.upper()=='OK'` EXATO; raciocínio (len≫5, ≠'OK') → `ok=false` com o próprio texto como 'issue' = falso-positivo. Mix (alguns dos 42 são reais, ex. step 194). Fix candidato (E3, barato, NÃO eval caro): parser por conclusão (termina-com-OK / ✓ sem ✗) OU veredito estruturado (1ª linha `OK`|`ERRO:`) OU JSON. Mesma classe pode afetar o judge↔adversarial 63%.
- 2026-06-03 (noite) — **FRENTE E3/GATE-1 INICIADA** (worktree `feat+gate1-calibracao-judge`; plano `docs/superpowers/plans/2026-06-03-gate1-calibracao-judge-online.md`, 6 tasks). **T1 ✅** ROADMAP reconciliado (O1.4 A3→APOSENTADO; O1.5 E3 re-aponta p/ ONLINE judge `agent_step.outcome_signal.judge`, não `eval_runner`; O1.G sem dep A3). **T2 ✅** fix do parser `verify_arithmetic` (TDD RED→GREEN, 5 testes novos, 11 verifiers + 154 subsistema verdes) — prompt determinístico (`VEREDITO: OK|ERRO`) + `_interpreta_veredito_aritmetico` discrimina pela CONCLUSÃO (não por `=='OK'` exato nem pela palavra ERRO). **T3 ✅ (achado, sem código)** judge↔adversarial 63%: adversarial usa `refuted` (JSON robusto, NÃO é bug de parser); 39/201 refutam `success` com razões SUBSTANTIVAS → o adversarial aponta **viés de CREDULIDADE do judge** (`success` quando tools rodam, sem evidência de conclusão validada — confirmado pelo step `:6`, score=85 com a própria evidência admitindo "não confirmou export"). Decisão: NÃO mexer no `JUDGE_SYSTEM_PROMPT` sem veredito humano; a discordância vira PRIORIDADE de spot-check na T4. **T4 ✅** `calibration_sampler.py` (core `_map_judge_to_case_fields` puro + `populate_calibration_cases` varredura síncrona, 7 testes) + **wiring módulo 33** no `app/scheduler/sincronizacao_incremental_definitiva.py` (por-ciclo, INLINE, gate `AGENT_EVAL_CALIBRATION` OFF=shadow, sem fila RQ nova) — popula `agent_eval_case` do online judge, prioriza discordância judge=success×adversarial. **PENDENTE (próxima continuação):** T5 UI spot-check (`routes/insights.py` + `insights.html`, padrão card O0.3); T6 merge→deploy→ligar `AGENT_EVAL_CALIBRATION` (sem DDL — `agent_eval_case` já existe). Commits worktree (NÃO mergeado): `677cfe4dd`(T1) `b46f3366e`(T2) `f413b7291`(T3) `a97da9284`+`418360ff4`(T4). GATE-1 fecha após T5+T6 + coleta ≥1 semana + concordance≥80%.
- 2026-06-03 (noite, cont.) — **T4.5 ✅ DESACOPLAR a flag do sampler** (pedido Rafael, ANTES do deploy; TDD RED→GREEN, 8 testes do sampler verdes + 26 `test_eval_runner` regressão verdes). Criada flag DEDICADA `USE_AGENT_CALIBRATION_SAMPLER` (env `AGENT_CALIBRATION_SAMPLER`, default OFF) em `feature_flags.py`; `calibration_sampler.py` e o módulo 33 (`sincronizacao_incremental_definitiva.py`, `CALIBRATION_SAMPLER_ENABLED`) passam a ler a flag NOVA. O `eval_runner.py:547` (A3) **fica intacto** em `USE_AGENT_EVAL_CALIBRATION` — assim ligar a calibração do online judge JAMAIS aciona o eval LLM caro (A3), nem num cenário hipotético de A3 religado. Teste `test_desacoplado_da_flag_a3` prova o desacoplamento (flag A3 ON + flag sampler OFF = no-op). **Impacto T6:** ligar `AGENT_CALIBRATION_SAMPLER=true` (NÃO `AGENT_EVAL_CALIBRATION`) no deploy. **PENDENTE:** T5 UI spot-check + T6 merge→deploy.
- 2026-06-03 (noite, cont.) — **T5 ✅ UI de spot-check humano do online judge** (TDD RED→GREEN, 8 testes novos; 231 da suíte agente verdes — regressão zero). 3 camadas: **(model)** `AgentEvalCase.record_human_verdict(case_pk_id, verdict, reviewed_by, note)` — valida `agree|disagree` ANTES do DB, FLUSH (caller commita); **(service)** `insights_service.get_judge_calibration_panel(agent_name, fraction, seed, limit)` (concordance_rate + sample_unreviewed de 5-10%, UNION garante que os prioritários `⚠ADVERSARIAL` SEMPRE apareçam — materializa o achado Task 3) + `_serialize_calibration_case` (puro, deriva `prioritario`); **(rotas)** GET `/agente/api/insights/judge-calibration` + POST `…/verdict` em `routes/insights.py` (admin-only, gate `USE_AGENT_INSIGHTS`, CSRF ativo via meta tag — sem exempt); **(UI)** card "Calibracao do Judge" em `insights.html` (concordance KPI + tabela de casos com botões acertou/errou `btn-success`/`btn-danger`, badge `bg-warning` nos prioritários) + JS `loadJudgeCalibration`/`submitJudgeVerdict` wired no `DOMContentLoaded`. **Link de menu:** já existe (`_sidebar.html:868` → `pagina_insights`); o card vive dentro da página insights (padrão idêntico ao card O0.3). Lint UI `--enforce-new`=0. Testes cobrem model+service (lógica); auth segue padrão IDÊNTICO das 5 rotas insights existentes. **PENDENTE:** T6 code-review→merge→deploy→ligar `AGENT_CALIBRATION_SAMPLER=true`.
- 2026-06-03 (noite, cont.) — **T6 ✅ (código+deploy) — frente E3/GATE-1 mergeada+deployada.** **(1) Code-review adversarial** da branch via workflow (6 agentes, 4 dimensões → verify): 2 findings brutos, **1 confirmado** (LOW, display-only: `submitJudgeVerdict` não decrementava o KPI de prioritários ao revisar um ⚠ADVERSARIAL) → **corrigido** (`19aae4736`); o outro refutado. **(2) Merge** de `origin/main` (avançou `418a717d3`→`86135b7e9`) na branch — **interseção de arquivos ZERO** (P10 gerindo-agente tocou `infra.py`, não meus arquivos) → merge limpo; pós-merge 27 testes + smoke app/flag/rotas/scheduler verdes. **(3) Push** `origin/main`@`40164221b` (FF; bypass da regra de PR pelo acesso do Rafael) → deploy auto. **(4) Flag** `AGENT_CALIBRATION_SAMPLER=true` no web `srv-d13m38vfte5s738t6p60` (`update_environment_variables` merge → deploy `dep-d8g9v3k2m8qs73e1tlg0`, commit `40164221b` = código+flag ON). **Sem DDL** (`agent_eval_case` já existe; `record_human_verdict` usa só colunas existentes). **Baseline pré-flag:** `agent_eval_case`=0 (PROD). **"Fim por código" ATINGIDO.** **A confirmar (operacional):** deploy LIVE + `agent_eval_case` crescer (sampler roda por-ciclo do scheduler ~30min). **GATE-1 formal:** + coleta ≥1 semana + rotular ≥10 → concordance≥80% (tempo/humano). **A3 (verificar 2026-06-04 ~11h):** `agent_eval_scores`=16 linhas `score=0`, último `11:33:17Z` (antes do desligamento 19:47Z) — não cresceu, prova definitiva na próxima janela D8.
