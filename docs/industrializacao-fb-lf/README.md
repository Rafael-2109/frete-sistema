...
**Próximo:** ✅ GATE 0 + ✅ veículo=server action + ⚠️ **GATE 1 executado** (tudo revertido). GATE 1 (13/06) revelou: picking de retorno = **pt66→Clientes(5)** (não pt98/26489); **`create_invoice` exige contexto LF-only `[5]`** (recompute 48s, NF do PA/5124 sai correta); e o **🔴 achado**: a **expansão automática dos 5902 não funciona p/ o shoyu** — investigado e **refutado** (BoM subcontract + rota + supplierinfo testados, não destravam; causa = fiscal profundo CIEL IT, caixa-preta). ⇒ **CONCLUSÃO: a 2ª NF de insumos NÓS derivamos da BoM/remessa (Forma 2 pura)** — temos os 16 componentes + op 2864 + CST 50 + conta + price. ▶️ **Próximo: GATE 1b — montar as 16 linhas 5902 programaticamente** → split → GATE 2 (SEFAZ). Detalhe → `ACHADOS §"GATE 1 EXECUTADO"` + `PROMPT_PROXIMA_SESSAO.md`.
