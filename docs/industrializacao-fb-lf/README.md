...
**Próximo:** ✅ GATE 0 + ✅ veículo=server action + ⚠️ **GATE 1 executado** (tudo revertido). GATE 1 (13/06) revelou: picking de retorno = **pt66→Clientes(5)** (não pt98/26489); **`create_invoice` exige contexto LF-only `[5]`** (recompute 48s, NF do PA/5124 sai correta); e o **🔑🔑 achado DEFINITIVO (GATE 1b)**: a **expansão dos 5902 é MANUAL, não automática** — na NF real do azeite, o robô criou só a linha do PA (5124); as **9 linhas 5902 foram criadas pela OPERADORA Josefa** 12 min depois (mesmo timestamp = um botão). Testado server-side (server action): create_invoice/onchange **não expandem**. ⇒ **NÃO existe expansão automática** (refuta a sessão 7). A busca toda (BoM/rota/compra_indcom/server-side) foi atrás de um mecanismo inexistente. **CONFIRMADO 100% manual** (262 linhas 5902 em 30 NFs, todas por operadoras Josefa/Histaina, zero robô; sem botão CIEL IT — editam o grid à mão). ⇒ **a Forma 2 é o caminho; o R1 pede AUTOMATIZAR o passo manual das operadoras.** ✅ **Decisão (Rafael): plano B via SERVER ACTION** (server-side garante os computes fiscais). ✅ **GATE 1c PROVADO (13/06):** server action (autorizada) **automatiza o passo da Josefa** — explode a BoM subcontract e cria as 9 linhas 5902 (op 2864) server-side; azeite saiu NF de 10 linhas (1×5124+9×5902), CST 51/50, **idêntica à NF real**. ▶️ **Próximo: GATE 1d — split na NF montada** (separar 5902 no RETIND 1083, GATE 0 já provou contábil) → baixa PASSIVA → GATE 2. Detalhe → `ACHADOS §"GATE 1c"` + `PROMPT_PROXIMA_SESSAO.md`. ✅ **(noite 13/06) NF-2 montada (`s24`) + tributação RESOLVIDA (`s27`–`s33`):** o "−278,17" é a **BAIXA da PASSIVA 5101020001**, NÃO imposto (`tax_line_id` vazio, contrapartida `no_payment`); NF-2 fiscalmente PRONTA (tributo real 0, cfop 5902/CST 50/conta 1150100012, vNF SEFAZ=valor cheio, igual à SARET real autorizada). ✅ price=remessa + **BAIXA da PASSIVA provada end-to-end** (`s34`/`s35`: fonte = remessa RPI/2026/00245; post no RETIND 1083 → Δ5101020001=**+279,23**; revertido sem rabo). ✅ **Task #4 (SA une as 2 + R3) + Fase A (post) + Fase B (SEFAZ) CONCLUÍDAS 14/06 — as 2 NFs AUTORIZADAS via server action (`ACHADOS §"FASE B"`).** ▶️ **Próximo: R2 (escriturar os 2 DFes na FB, Etapa 5) + automação dos 2 gatilhos (com descoberta automática da remessa) + validar a SA `s37` atualizada no próximo ciclo real.**
