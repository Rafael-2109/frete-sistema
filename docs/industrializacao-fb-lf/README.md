...
**Próximo:** ✅ GATE 0 + ✅ veículo=server action + ⚠️ **GATE 1 executado** (tudo revertido). GATE 1 (13/06) revelou: picking de retorno = **pt66→Clientes(5)** (não pt98/26489); **`create_invoice` exige contexto LF-only `[5]`** (recompute 48s, NF do PA/5124 sai correta); e o **🔑 achado central (GATE 1b)**: a **expansão dos 5902 é SERVER-SIDE, não do produto** — o **azeite** (que expande no robô) **também não expandiu** pelo nosso `create_invoice` via XML-RPC. O robô faz `create_invoice + onchange + post` server-side; os onchange via XML-RPC não persistem. ⇒ **CONCLUSÃO: a expansão nativa só funciona dentro do Odoo → CONFIRMA o veículo SERVER ACTION** (XML-RPC/Skill 8 não reproduz). A investigação de cadastro (BoM/rota/compra_indcom) foi beco — o problema era o método de execução. ▶️ **Próximo (precisa go): criar+rodar `ir.actions.server` de teste** replicando o robô (foi bloqueado pelo classificador). Detalhe → `ACHADOS §"GATE 1b"` + `PROMPT_PROXIMA_SESSAO.md`.
