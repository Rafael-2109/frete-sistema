...
### PLANO DA PRÓXIMA SESSÃO (4 passos — Rafael, 2026-06-13)
> Pausa após o GATE 1c provado. **Estado limpo** (azeite 4 em 42, shoyu 1 em 31093, RETIND 1083 + op 3252 mantidos; pt98 revertido; 0 SA/NF de teste; 16 pickings done origin=GATE1* = rastro net-zero inofensivo). Scripts: `s11`(picking/revert parametrizável), `s15`(montar 5902 via SA), `s14`(SA teste onchange), `s8d`(split GATE 0), `s10`(config), `s12`(BoM subcontract).

1. **Entender COMO a server action explode a BoM na MESMA NF** (aprofundar o mecanismo do `s15`/GATE 1c, `ACHADOS §"GATE 1c"`): por que `l10n_br_operacao_id=2864` na linha faz o CIEL IT computar cfop 5902 + CST 50 + conta sozinho; papel do `check_move_validity=False` no create; o recompute `onchange_l10n_br_calcular_imposto(_btn)` server-side (não persiste via XML-RPC). Mapear os campos/computes envolvidos.
2. **Copiar a server action explodindo a BoM em OUTRA NF** (não na mesma) — montar a NF-insumos (5902) **SEPARADA** desde o início (vai direto p/ o desenho 2-NF, dispensa o "split" de uma mista). Provavelmente precisa de passos para **criar a 2ª fatura** (account.move do zero, journal RETIND 1083) + explodir a BoM nela + forçar `price_unit` = valores da remessa (invariante 5902=5901). *(Alternativa ainda válida: montar 1 mista e separar = GATE 0; decidir qual.)*
3. **Vínculo determinístico NF-retorno (insumos 5902) → NF-industrialização (serviço 5124)** (R3 da Contadora): investigar/implementar `account.move.referencia_ids` (refNFe; modelo `l10n_br_ciel_it_account.account.move.referencia` = {move_id, chave 44díg., company}) — quando a chave materializa (pré/pós-transmissão SEFAZ) e gravar o vínculo.
4. **Server action de ORQUESTRAÇÃO: toda NF de industrialização (serviço) PUXA o faturamento da NF de retorno (insumos)** automaticamente (R1 + gatilho) — gatilho via cron/`base.automation` quando a NF de serviço é gerada → SA cria/fatura a NF de insumos vinculada (passos 2+3).

**Pendências de fundo:** GATE 2 (SEFAZ, irreversível, go duplo) só depois de 1-4 validados; medir a **baixa da PASSIVA `5101020001`** end-to-end; p/ o **shoyu** (sem BoM subcontract) decidir a fonte das linhas (criar BoM subcontract via `s12` OU explodir BoM normal/remessa); **dívida server action** = script de re-aplicação versionado + monitor anti-upgrade (some em upgrade CIEL IT).
