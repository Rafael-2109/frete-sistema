...
1. **R1 — emitir a 2ª NF (saída LF).** ✅ **DESENHO E2E FECHADO na sessão 9 (12/06): `SOT §6.1 v3.1`** (🟡 proposta, aguarda validação do Rafael) — **wizard nativo `stock.invoice.onshipping` chamado por NÓS** (mesma chamada do robô ⇒ expansão automática da BoM = R1 literal) **+ split em DRAFT** (NF-serviço só-5124 no j847; NF-insumos só-5902 no journal RETIND com `l10n_br_no_payment=True` + `no_payment=26667` + price_unit FORÇADO = remessa) + saga de transmissão (serviço → refNFe → insumos). Histórico: hipótese (i) refutada na sessão 8 (`create_invoice` funde; VIA 1 fechada). Desenho revisado por painel adversarial 3 lentes (30 findings incorporados — CST 50/51 corrigido, pt98 pré-req, ETL pré-condição GATE 2, 3 alavancas anti-robô).
   ...
   - 🔑 **GATE 1b (descoberta que reorienta — `ACHADOS §"GATE 1b"`):** testei com o **azeite (4739099, que expande no robô)** e ele **TAMBÉM não expandiu** pelo nosso `create_invoice` via XML-RPC (1×5124, 0×5902). A VND real do azeite (robô/OdooBot, server-side) tem 9×5902. ⇒ **a expansão é SERVER-SIDE, não do produto.** O robô faz `create_invoice + onchange_l10n_br_calcular_imposto(_btn) + action_post`; os onchange via XML-RPC **retornam None e não persistem**. A investigação de cadastro do shoyu (BoM subcontract/rota/compra_indcom) foi um **beco** — o problema era o **método de execução**. **CONFIRMA o veículo = SERVER ACTION** (XML-RPC/Skill 8 não reproduz a expansão; só server-side).
   - ▶️ **PRÓXIMO = criar+rodar uma `ir.actions.server` (state=code) de TESTE** que replique o robô (create_invoice + onchange + medir) no azeite → confirmar expansão 1→10 linhas → então o split (separar 5902 no RETIND 1083) também via server action → post (sem SEFAZ) → medir baixa PASSIVA → **GATE 2** (SEFAZ). ⚠️ **PRECISA de GO EXPLÍCITO do Rafael** (criar/rodar código no Odoo PROD = escalada; foi bloqueado pelo classificador na sessão). Depois: server action definitiva + script de re-aplicação versionado + monitor anti-upgrade. **Cadastros de teste revertidos** (shoyu+azeite restaurados); **RETIND 1083 mantido**. **Alternativa se não quiser server action p/ a expansão: Forma 2 pura** (criar as 16 linhas 5902 manualmente via XML-RPC — temos op 2864/CST 50/conta 26855/price da remessa; template VND 738097).
