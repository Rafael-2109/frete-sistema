...
> **✅ Execução (2026-06-16) — A1+A4+A2/A3 CONCLUÍDOS e validados (`s83 --validar` = TODOS OS GATES OK).** A migração via picking interno exigiu **3 correções no veículo** (vs o fluxo padrão do `PickingService`): (1) `action_assign` **não reserva** move **pai→filha** (31092⊂42 pós-reparent) → **codificar move.line manual** (`quantity`+`lot`+`picked`); (2) `pt23` é `at_confirm` → **auto-reserva** parcial → **apagar move.lines** antes de codificar (anti-dup 2×); (3) `button_validate` dispara wizard **`expiry.picking.confirmation`** (lotes a vencer) → `finalizar_picking` (s84) processa wizards encadeados. Resultado: **439 quants migrados** (pickings 325573/325575/325576), **0 SVL**, saldo por produto conservado, MO 20797 intacta. **Exceções fechadas (`s86`, 2026-06-16):** açúcar lote 230326 migrado p/ 31092 (picking 325577; saída FB 321794 desreservada→re-reservada, assigned) + 3 sem-lote migrados (picking 325579: PALLET/CORANTE `tracking=none` sem lote, PIMENTAO `tracking=lot` resíduo c/ lote **A-16/06** criado+atribuído). **42 efetivamente vazio** (só 2 resíduos `qty≈1e-6`). **Falta só A5** (repoint contábil L1, Contador). A1 não reverte (é o desenho aprovado).
