...
- **✅ CANARY G1 — estágios 0+1 (2026-06-15, ZERO NF criada): genealogia `safe_eval` VALIDADA contra o oráculo.** SA efêmera log-only (`docs/industrializacao-fb-lf/scripts/s71_canary_g1_genealogia.py`, padrão `s49` — só `ir.actions.server` transitório, create→run→unlink, **nenhum `account.move`**) na NF-1 **791437**: o body `safe_eval` produz **idêntico** ao oráculo (`descobrir_fonte_nf2`) — **ncomp 16 · total 90,88632 · chave remessa 35…6795 · componentes (product_id+qty) idênticos**. O `s71` deriva o corpo do canary do **próprio `SA_BODY_G1`** (truncado antes da montagem) → valida o código REAL, sem cópia/drift.
  ...
  - **PRÓXIMO = estágio 2** (montagem → draft NF-2, **go fresco**): construir `s71 --montagem` (truncar `SA_BODY_G1` antes do `action_post`) → cria draft RETIND 1083 → `saida_retorno_industrializacao validar` → deleta. Runbook completo: `fluxos/1.1.4 §"CANARY — RUNBOOK"`.
