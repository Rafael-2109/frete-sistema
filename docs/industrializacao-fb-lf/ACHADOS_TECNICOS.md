...
- ⏳ **Pergunta operacional p/ o Rafael:** a Josefa usa um **botão/wizard específico** (do CIEL IT) p/ inserir os 9 componentes de uma vez? Se sim, qual? (acharíamos o método nativo p/ chamar; senão = Forma 2 nós montamos as linhas). Não há server action óbvia (`component/industrializ/insumo` no nome) no Odoo.
