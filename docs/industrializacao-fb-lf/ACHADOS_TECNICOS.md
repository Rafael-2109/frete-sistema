...
**Estado:** tudo revertido — shoyu restaurado (route_ids `[134,1]`, sem BoM subcontract, pt98 `False/False`), PA em 31093, NFs draft deletadas. **RETIND 1083 mantido** (cadastro válido). Rastro: 8 pickings done origin=GATE1 (net-zero, não deletáveis). Scripts: `s11_gate1_emissao.py` (--criar-picking/--faturar/--medir/--revert), `s12_bom_subcontract.py` (criar/revert — hipótese refutada, mantido p/ histórico).
