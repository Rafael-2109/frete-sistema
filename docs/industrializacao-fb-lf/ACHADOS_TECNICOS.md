...
- **O azeite TAMBÉM não expandiu pelo nosso `create_invoice` via XML-RPC** (NF 1×5124, 0×5902 — idêntico ao shoyu)! Mas a VND **real** do azeite (738097), criada pelo **robô/OdooBot (server-side)**, tem 1×5124 + **9×5902**. ⇒ **A expansão NÃO depende do produto — depende de rodar SERVER-SIDE.** O robô faz `create_invoice()` **+ `onchange_l10n_br_calcular_imposto()` + `_btn()` + `action_post()`**; nós (XML-RPC) só fazemos `create_invoice` — e os `onchange` via XML-RPC **retornam `None` (erro marshal) e não persistem** (são métodos de UI/recordset in-memory).
- ⇒ **CONCLUSÃO REORIENTADA — confirma o veículo SERVER ACTION:** a expansão nativa (caminho A, o que o Rafael quer) **só funciona dentro do Odoo** (server action reproduzindo o robô). Via XML-RPC puro (pipeline Skill 8) **não dá** — teríamos de criar as 16 linhas 5902 manualmente (Forma 2 pura). **Isto reforça/decide o veículo: server action.** *(A investigação anterior de cadastro — BoM subcontract/rota/compra_indcom — foi um beco: o problema nunca foi o cadastro do shoyu, era o método de execução.)*
- 🔲 **Teste pendente (precisa de AUTORIZAÇÃO explícita do Rafael):** criar+rodar uma `ir.actions.server` (state=code) de teste que replique o robô (create_invoice + onchange server-side) e confirme a expansão 1→10 linhas. **Foi bloqueado pelo classificador** (criar código arbitrário em PROD = escalada de permissão; a msg do Rafael era pergunta, não consentimento). É o próximo passo do GATE 1b.
- **BUG corrigido** no `s11 --revert`: checava estoque em SRC p/ decidir devolver — produtos com saldo pré-existente (azeite tinha 4) enganavam a checagem e deixavam 1 un presa em Clientes. Agora devolve se o picking de saída está `done`. (1 un de azeite foi recuperada manualmente; estoque OK = 4 em 42.)
