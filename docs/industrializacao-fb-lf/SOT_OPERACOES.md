...
- **GATE 1** — ⚠️ **EXECUTADO (2026-06-13)** (`s11_gate1_emissao.py`; tudo revertido, shoyu restaurado, PA em 31093). **3 camadas:** (1) picking de retorno = **pt66 src override 31093→dst 5 Clientes** (pt98/26489 falha a operação fiscal); (2) **🔑 `create_invoice` via XML-RPC exige `allowed_company_ids=[5]` LF-ONLY** (com `[1,5]` pega a conta income da FB → "empresas incompatíveis"); recompute=48s; (3) **🔴 a expansão automática dos 5902 NÃO ocorre p/ o shoyu** (NF só 1×5124) — **investigado/refutado** (BoM subcontract + rota Comprar-FB + supplierinfo testados, não destravam); causa = comportamento fiscal profundo CIEL IT (`compra_indcom` ind×uso, caixa-preta). ⇒ **CONCLUSÃO: não depender da expansão nativa — a 2ª NF (5902) NÓS derivamos da BoM/remessa (Forma 2 pura), temos todos os dados (op 2864, CST 50, conta 26855, price da remessa).** Próximo: **GATE 1b — montar as 16 linhas 5902 programaticamente** + validar. Detalhe: `ACHADOS §"GATE 1 EXECUTADO"`.
