...
- **GATE 1** — ⚠️ **EXECUTADO (2026-06-13)** (`s11_gate1_emissao.py`; tudo revertido, shoyu restaurado, PA em 31093). **3 camadas:** (1) picking de retorno = **pt66 src override 31093→dst 5 Clientes** (pt98/26489 falha a operação fiscal); (2) **🔑 `create_invoice` via XML-RPC exige `allowed_company_ids=[5]` LF-ONLY** (com `[1,5]` pega a conta income da FB → "empresas incompatíveis"); recompute=48s; (3) **🔑🔑 a expansão dos 5902 é MANUAL, não automática** (descoberta definitiva, fecha o mistério): na NF real `738097`, a linha 5124 (PA) foi criada pelo **robô** (17:00:57), mas as **9 linhas 5902 pela OPERADORA Josefa** (uid 38, 17:12:18, todas no mesmo instante = um botão/wizard). Testado server-side (server action c/ sudo+onchange): create_invoice/onchange **não expandem**. ⇒ **NÃO existe expansão automática** — o robô cria só o PA, a operadora adiciona os insumos à mão. **REFUTA a sessão 7 §G.** ⇒ **CONCLUSÃO DEFINITIVA: a Forma 2 é o caminho e o R1 da Contadora pede AUTOMATIZAR o passo manual da Josefa** — nós criamos as 16 linhas 5902 (op 2864, CST 50, conta 26855, price da remessa) via XML-RPC (computes rodam no write); **não precisa server action p/ "reproduzir expansão" (inexistente)**. Toda a investigação anterior (BoM subcontract/rota/compra_indcom/server-side) foi beco — não havia mecanismo automático. **Próximo: confirmar com o Rafael COMO a Josefa insere os componentes** (botão CIEL IT? qual?) → então automatizar. Detalhe: `ACHADOS §"GATE 1b"`.
- **GATE 1c** — ✅ **PROVADO (2026-06-13)** (`s15`): server action (decisão Rafael) **automatiza o passo manual da Josefa** — `create_invoice` (NF do PA) + explode a **BoM subcontract** + cria as N linhas 5902 (`l10n_br_operacao_id=2864`) + recompute server-side. Azeite: NF de 10 linhas (1×5124 + 9×5902), CST 51/50, **idêntica à NF real**. ⇒ Forma 2 via server action VIÁVEL. ▶️ Próximo: **GATE 1d (split na NF montada → baixa PASSIVA)** + GATE 2 (SEFAZ). Detalhe: `ACHADOS §"GATE 1c"`.
...
**Pendências para construir:** (1) cron G1 + SA com a fonte A/B/C (resolver o valor via SVL) **+ descoberta automática da remessa** (no piloto é FIXA); (2) cron G2 + transmissão via SA (já provada `s54`); (3) **R2 — gatilho da entrada FB** (a MECÂNICA já está provada `s67`: NF-1 caminho A + NF-2 direto + revaloração; falta o disparo automático ao chegarem os 2 DFes + reconciliar o trânsito 26489); (4) resiliência (máquina de estados por picking, marcador anti-post, rollback); (5) **ajustar os pontos de código que assumem 1 NF** (`ACHADOS §E`) ANTES de ligar o automático — ✅ **ETL `faturamento_service` FEITO 15/06** (`company_id not in [5]`, commit `43127eb76`, pende deploy; 34 espúrios do piloto limpos via `s68`; histórico LF de 934 linhas a limpar à parte); FALTAM orchestrator/Skill 7/8/`recebimento_lf_odoo_service`. Provas: `ACHADOS §"FASE B"` (saída) + `§"R2.3b"` (entrada). Scripts: `s36`–`s59` (saída), `s60`–`s68` (entrada + limpeza ETL). **POP do usuário: `POP_OPERACIONAL_INDUSTRIALIZACAO.md`.**

### 6.3 ARQUITETURA TRAVADA DA AUTOMAÇÃO (DECISÃO 2026-06-15 — Rafael) — Opção B

> Fecha o "veículo" do §6.2 para a **construção**. Base: análise de 4 eixos (mecanismo `s37`/`s54`, reuso de átomos, padrão SA/cron, placement constitucional) + `ACHADOS §"FASE B"`/`§"R2.3b"`.

- **Veículo:** `ir.cron`-Odoo + **server action PERSISTENTE server-side**, padrão dos 11 robôs CIEL IT. **2 gatilhos:** **G1** (detecta NF-1 serviço em **j847** sem NF-2 vinculada → monta+posta a NF-2 insumos 5902 + R3) · **G2** (detecta NF-1 `l10n_br_cstat_nf=100` com NF-2 não transmitida → transmite a NF-2 ao SEFAZ).
- **Genealogia: 100% server-side (`safe_eval`).** `DescobertaIndustrializacaoService` (`app/odoo/estoque/scripts/descoberta_industrializacao.py`) **deixa de ser executor** (não é importável dentro da SA) e vira **(a) especificação executável + (b) oráculo de validação** — o orchestrator e o monitor comparam a saída da SA contra a nossa Descoberta. Duplicação consciente (o body da SA re-implementa a travessia ORM: explode MO/sub-MO → exclui `consu` → preço pelo SVL-entrada de 31092 → rateio `faturado/produção_real`).
- **Princípio dos átomos (Q2 — RESOLVIDO):** a montagem da saída (NF-1 wizard `stock.invoice.onshipping` + NF-2 op **2864** + **recompute** `onchange_l10n_br_calcular_imposto[_btn]` + remap conta via fp **111** → 1150100012 + R3) é **irredutivelmente server-side** — o recompute fiscal **só persiste dentro de uma SA** (via XML-RPC o ORM descarta; `s48`/`s49`). Logo mora no **body da SA** (categoria infra-Odoo), **NÃO** num átomo XML-RPC. **PROIBIDO** criar átomo/flag `recompute=True` (= *lying parameter*; ver `PROTECAO §N34`). `montar_invoice_entrada_direta` **permanece PURO** para a entrada (que ao contrário *proíbe* o recompute).
- **Atuação humana (G2/SEFAZ): só na NF-1.** O operador cria o picking, valida (→ nasce a NF-1 só com 5124) e **transmite a NF-1** — essa transmissão humana **É a autorização do par**. A **NF-2 é invisível ao operador**: o cron G2 a transmite automaticamente após `cstat=100`. ⚠️ Em build/canary, **cada** escrita SEFAZ exige **go fresco do Rafael** (o automático é o regime de produção, validado pelo piloto).
- **Dívida OBRIGATÓRIA (parte do build):** (a) **provisionamento idempotente versionado** (`app/odoo/estoque/provisioning/` — o body da SA é **string versionada = fonte de verdade**; re-cria SA+cron por `name`); (b) **monitor anti-upgrade LEVE** = checagem READ-only no **D8** (diário) + **hook `SessionStart`** que injeta aviso se a SA/cron sumiram (upgrade CIEL IT apaga records custom — precedente DFE NFD, `incident_ciel_it_dfe_nfd.md`).
- **Placement:** orchestrator C3 da saída (`app/odoo/estoque/orchestrators/saida_retorno_industrializacao.py`) = **lado nosso** (planejar/validar/medir — READ + composição dos átomos R3); body da SA + provisionamento + monitor = **categoria infra-Odoo** (`app/odoo/estoque/provisioning/`), **FORA do sistema de skills** (§1.1 + `CLAUDE.md:410-412` — escrever `ir.actions.server`/`ir.cron` é provisionar config do ERP, não operar estoque). SEFAZ fica fora do orchestrator.
...
| **3.6** | **2026-06-15** | **ARQUITETURA TRAVADA da automação (§6.3) — Opção B (Rafael).** Veículo = `ir.cron`-Odoo + server action PERSISTENTE server-side (G1 monta NF-2 / G2 transmite NF-2), padrão dos robôs. Genealogia 100% server-side; `DescobertaIndustrializacaoService` vira spec+oráculo. Q2 resolvido: montagem da saída mora no body da SA (infra-Odoo `provisioning/`), NÃO em átomo XML-RPC (recompute só persiste server-side = *lying parameter* proibido, `PROTECAO §N34`); `montar_invoice_entrada_direta` fica puro p/ entrada. G2/SEFAZ: atuação humana só na NF-1 (transmitir NF-1 = autorizar o par); NF-2 invisível, transmitida pelo cron G2. Dívida obrigatória: provisionamento idempotente versionado + monitor anti-upgrade leve (checagem D8 + hook SessionStart). |
