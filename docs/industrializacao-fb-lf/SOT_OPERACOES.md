...
- **GATE 1** — ⚠️ **EXECUTADO (2026-06-13)** (`s11_gate1_emissao.py`; tudo revertido, shoyu restaurado, PA em 31093). **3 camadas:** (1) picking de retorno = **pt66 src override 31093→dst 5 Clientes** (pt98/26489 falha a operação fiscal); (2) **🔑 `create_invoice` via XML-RPC exige `allowed_company_ids=[5]` LF-ONLY** (com `[1,5]` pega a conta income da FB → "empresas incompatíveis"); recompute=48s; (3) **🔑 a expansão automática dos 5902 é SERVER-SIDE, não do produto** — testado com o **azeite (que expande no robô)**: o nosso `create_invoice` via XML-RPC **também NÃO expandiu** (1×5124, 0×5902), igual ao shoyu. A VND real do azeite (robô/OdooBot) tem 9×5902. O robô faz `create_invoice` **+ onchange + post**; os onchange via XML-RPC **não persistem** (retornam None/in-memory). ⇒ **CONCLUSÃO REORIENTADA: a expansão nativa (caminho A) só funciona dentro do Odoo → CONFIRMA o veículo SERVER ACTION** (pipeline Skill 8/XML-RPC não reproduz; teria de criar as 16 linhas 5902 manualmente = Forma 2 pura). A investigação de cadastro (BoM subcontract/rota/compra_indcom) foi um beco — o problema era o **método de execução**, não o cadastro. **Próximo (precisa go do Rafael): criar+rodar uma `ir.actions.server` de teste** replicando o robô (bloqueado pelo classificador — escalada de permissão). Detalhe: `ACHADOS §"GATE 1b"`.
