...
| **S3** | ✅ **FÍSICO** (A5 pend.) | Implementação Odoo | **A1 reparent ✅** + **A4 migração ✅** (439 quants, 0 SVL, saldo conservado, `s83`=gates OK) + **A2/A3 put-away ✅** (rules 3,4). Mecânica resolvida (codificar move.line manual + anti-dup at_confirm + wizard expiry via `finalizar_picking`). Exceções **açúcar + 3 sem-lote RESOLVIDAS** (`s86`: açúcar→31092 saída re-reservada; PALLET/CORANTE sem lote, PIMENTAO lote A-16/06); 42 vazio (2 resíduos ~1e-6). **Falta só A5 repoint L1 (Contador).** | **A5 = go + Contador** |
