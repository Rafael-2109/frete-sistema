...
**Última atualização:** 2026-05-17 fim do dia (refator D004/D005 + checkpoint para teste piloto)
**Status global:** Foundation + F3 + F4 + F5 completas + **AUDITORIA F4/F5 INSTRUMENTADA** + **REFATOR D004/D005 (rename + diferenca + lote MIGRACAO)**. F6 CANCELADA. F7 4/10 (preparação completa: extrair/carregar/confrontar/propor com nova lógica). DB tem 23.639 PROPOSTO. **Proximo passo: teste piloto end-to-end produto `210030325` LF (4 RENAME + 2 PERDA + extração validação)** — ver `CHECKPOINT_2026_05_17_FIM_DIA.md` e `QUICK_START_NEXT_SESSION.md`. Bloqueado por: 1) script wrapper teste piloto (a construir), 2) script extrator pós-execução replicavel por filial (a construir), 3) aprovação onda 1 via hash. F7.6 canary fiscal recomendado antes de execução real (irreversível em SEFAZ).
...
|-----------|----------|----------|
| **`CHECKPOINT_2026_05_17_FIM_DIA.md`** | **Snapshot mais recente do estado (DB, código, próximos passos teste piloto)** | **PRIMEIRO em sessão nova** |
| **`QUICK_START_NEXT_SESSION.md`** | **Prompt pronto + tarefas para a sessão seguinte** | **PRIMEIRO em sessão nova** |
