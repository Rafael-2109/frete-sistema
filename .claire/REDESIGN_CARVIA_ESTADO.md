...
- **Frente A — campo `local_cd` em `carvia_pedidos`/`carvia_cotacoes` + propagacao da Coleta: IMPLEMENTADO + TESTADO.**
  - **PROD (2026-06-18) — COMPLETO**: migration de COLUNAS `2026_06_18_carvia_pedido_cotacao_local_cd` APLICADA (2 ALTER + 2 indices + backfill `UPDATE 36` pedidos / `UPDATE 36` cotacoes = TM). A VIEW v12 FALHOU na 1a tentativa por **DEADLOCK** (recriar view+MV na mesma transacao com app lendo ambas) -> ROLLBACK; v12 tornada DEADLOCK-RESILIENTE (2 transacoes separadas + `lock_timeout`) e **REAPLICADA com sucesso** (NULL=0, **TM=24** linhas na tela, REFRESH OK, sem deadlock). Codigo commitado+pushado (`e6c4b321d`, main) -> deploy Render.
