"""
Script STANDALONE para investigar CTe duplicado no ODOO
...
def investigar_cte_odoo():
    ...
    try:
        ...
        # Buscar TODOS os DFes com essa chave de acesso
        ...
        if len(dfes) > 1:
            ...
            campos_comp = [
                ...
                'invoice_ids', 'purchase_fiscal_id', 'refs_ids'
            ]
        ...
        if len(dfes) > 1:
            ...
            print()
            print(f"   IDs Odoo: {[d['id'] for d in dfes]}")
            ...
            print(f"🔧 SOLUÇÃO NECESSÁRIA:")
            print(f"   Alterar cte_service.py linha 414:")
            print()
            print(f"   # ANTES (busca só por dfe_id):")
            print(f"   cte_existente = ConhecimentoTransporte.query.filter_by(dfe_id=dfe_id).first()")
            print()
            print(f"   # DEPOIS (busca por chave_acesso PRIMEIRO):")
            print(f"   cte_existente = ConhecimentoTransporte.query.filter_by(")
            print(f"       chave_acesso=chave_acesso")
            print(f"   ).first()")
            print()
            print(f"   # Se não encontrou por chave, busca por dfe_id")
            print(f"   if not cte_existente:")
            print(f"       cte_existente = ConhecimentoTransporte.query.filter_by(")
            print(f"           dfe_id=dfe_id")
            print(f"       ).first()")
        else:
