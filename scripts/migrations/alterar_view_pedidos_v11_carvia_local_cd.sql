-- Migration: VIEW pedidos v11 + MV — v10 com local_cd da CarVia = default VICTORIO_MARCHEZINE
-- Data: 2026-06-17
-- Descricao:
--   v10 = v9 + coluna `local_cd` (Victorio Marchezine / Tenente Marques) em TODAS as 3
--   ramificacoes UNION (Parte 1 Nacom, 2A/2B CarVia), tanto na VIEW quanto na MV.
--   Parte 1 (Nacom): min(s.local_cd) — vem de separacao (backfill VM ja aplicado).
--   Partes 2A/2B (CarVia): default 'VICTORIO_MARCHEZINE' (4B — badge aparece p/ CarVia tb).
--   (refinamento futuro: derivar TM da Coleta vinculada quando houver.)
--   PRE-REQUISITO: 2026_06_17_local_cd_e_chegada_filial (coluna separacao.local_cd).
--   Idempotente (DROP + CREATE) e ATOMICO (BEGIN/COMMIT). Mantem todos os indices da v9.
--
...
SELECT
    ...
    'VICTORIO_MARCHEZINE'::varchar(20) AS local_cd
