-- Migration: VIEW pedidos v12 + MV — local_cd da CarVia DERIVADO da Coleta (Frente A)
-- Data: 2026-06-18
-- Descricao:
--   v12 = v11, mas as Partes 2A/2B (CarVia) deixam de usar o literal fixo
--   'VICTORIO_MARCHEZINE' e passam a LER a coluna real:
--     Parte 2A (cotacoes): COALESCE(cot.local_cd, 'VICTORIO_MARCHEZINE')
--     Parte 2B (pedidos):  COALESCE(ped.local_cd, 'VICTORIO_MARCHEZINE')
--   Essas colunas sao alimentadas por propagacao a partir da Coleta
--   (CarviaColeta.local_cd -> CarviaNf -> CarviaPedido/Cotacao via numero_nf,
--   ver app/carvia/services/documentos/coleta_service.py). So LEITURA aqui.
--   Parte 1 (Nacom): inalterada — min(s.local_cd) de separacao.
--   PRE-REQUISITO: 2026_06_18_carvia_pedido_cotacao_local_cd (colunas) + v11.
--   Idempotente (DROP + CREATE). DEADLOCK-RESILIENTE: a VIEW `pedidos` e a MV
--   `mv_pedidos` (independentes) sao recriadas em TRANSACOES SEPARADAS — recriar as
--   duas na MESMA transacao com o app lendo ambas formava ciclo de locks (deadlock
--   observado no PROD 2026-06-18). `lock_timeout` faz a espera por lock falhar rapido
--   (re-tentavel) em vez de pendurar. Mantem todos os indices da v9.
--
-- ---- (corpo herdado da v9 abaixo) ----

SET lock_timeout = '10s';

-- Transacao 1: VIEW pedidos
BEGIN;
...
SELECT
    ...
    min(s.local_cd::text) AS local_cd
FROM separacao s
...
SELECT
    ...
    COALESCE(cot.local_cd, 'VICTORIO_MARCHEZINE')::varchar(20) AS local_cd
...
SELECT
    ...
    NULL::text AS equipe_vendas,
    NULL::text AS tags_pedido,
    COALESCE(ped.local_cd, 'VICTORIO_MARCHEZINE')::varchar(20) AS local_cd
...
-- Transacao 2: Materialized View mv_pedidos (independente da VIEW acima)
BEGIN;
...
CREATE INDEX IF NOT EXISTS idx_mv_pedidos_local_cd
    ON mv_pedidos (local_cd) WHERE local_cd IS NOT NULL AND local_cd <> 'VICTORIO_MARCHEZINE';
