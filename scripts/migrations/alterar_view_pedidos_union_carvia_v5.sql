...
--     Problema: UPPER nao remove acento. 'SÃO PAULO' nao bate com 'SAO PAULO'.
--
--   DEPOIS (v5):
--     AND lower(f_unaccent(dest.fisico_cidade))
--           LIKE '%' || lower(f_unaccent(csr.nome_cidade)) || '%'
--     f_unaccent (funcao SQL pura criada em remover_extensao_unaccent.sql)
--     remove acentos via translate(); lower() trata caixa. Match agora alinhado
--     com a busca canonica `buscar_sub_rota_por_uf_cidade` em
--     app/carteira/utils/separacao_utils.py (que usa remover_acentos).
--
--   Parte 1 (Nacom) e INALTERADA — sub_rota la vem persistido em separacao.sub_rota,
--   nao calculado on-the-fly. A normalizacao do que e gravado fica em
--   `buscar_sub_rota_por_uf_cidade` (Python).
--
