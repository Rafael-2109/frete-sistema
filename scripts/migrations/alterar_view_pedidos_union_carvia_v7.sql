...
-- Data: 2026-05-21
-- Descricao:
--   Recria a VIEW pedidos identica a v6 (alterar_view_pedidos_union_carvia_v6.sql),
--   ADICIONANDO a coluna `horario_agendamento` (TIME) logo apos `agendamento`
--   nas 3 partes do UNION. Mantem tudo o que a v6 ja fazia (agendamento_confirmado,
--   f_unaccent, saldo).
--
--   NOVA COLUNA horario_agendamento:
--     Parte 1 (Nacom):  NULL::time            — fluxo Nacom NAO usa horario
--     Parte 2A (CarVia cotacao): cot.horario_agenda
--     Parte 2B (CarVia pedido):  cot.horario_agenda  (JOIN carvia_cotacoes cot)
--   Reflete carvia_cotacoes.horario_agenda em lista_pedidos
--   (Pedido.horario_agendamento). Fonte: app/carvia/models/cotacao.py.
--
--   v6 mantida: cot.agendamento_confirmado nas Partes 2A/2B (vs FALSE na v5).
--
