...
--   alterando APENAS a projecao de `agendamento_confirmado` nas Partes 2A e 2B
--   (CarVia). O JOIN com cadastro_sub_rota (f_unaccent), o saldo e a Parte 1
--   (Nacom) permanecem INALTERADOS.
--
--   ANTES (v5):
--     FALSE AS agendamento_confirmado    (Partes 2A e 2B)
--     Regressao: a migration add_agendamento_confirmado_carvia.sql (2026-03-30)
--     ja projetava cot.agendamento_confirmado, mas a projecao foi perdida quando
--     a VIEW foi recriada integralmente em v4/v5 a partir de um template com FALSE.
--
--   DEPOIS (v6):
--     cot.agendamento_confirmado AS agendamento_confirmado    (Partes 2A e 2B)
--     Reflete o checkbox "Confirmacao de Agendamento" da cotacao comercial CarVia
--     em lista_pedidos (Pedido.agendamento_confirmado), alinhado com a coluna
--     carvia_cotacoes.agendamento_confirmado (app/carvia/models/cotacao.py:96).
--     Parte 2B usa o mesmo cot.agendamento_confirmado (JOIN carvia_cotacoes cot).
--
--   Parte 1 (Nacom) e INALTERADA — agendamento_confirmado vem de
--   bool_or(separacao.agendamento_confirmado).
--
