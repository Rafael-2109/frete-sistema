...
Origem: planilha "AJUSTE SALDO CD.xlsx" (versao 3 enviada pelo usuario com
93 linhas FILTRADAS para filial=FB, todos diff_qtd > 0).

NOTA: nome do arquivo herdado mantem "CD" por historico, mas os dados sao FB.
Schema da planilha mudou vs versao anterior:
- colunas: filial, cod, lote, diff_qtd (era EMP, COD, LOTE, AJUSTE)

Diferencas vs script 12 (ajuste positivo CD):
- COMPANY_ID=1 (FB) em vez de 4 (CD)
- LOCATION_ID=8 em vez de 32
- Filtro filial='FB' em vez de EMP='CD'
- Le colunas lowercase do XLSX

Cria stock.lot e/ou stock.quant se nao existirem (padrao herdado script 12).
Gera 1 stock.move automatico por quant ajustado (origem "Physical Inventory"
em Inventory > Reporting > Stock Moves).

Tratamento de 6 casas decimais identico ao script 12.
...
logger = logging.getLogger('13_ajuste_pos_fb')
...
# Schema esperado do XLSX (versao 3 — lowercase, diff_qtd)
COL_FILIAL = 'filial'
COL_COD = 'cod'
COL_LOTE = 'lote'
COL_DIFF = 'diff_qtd'
...
def buscar_quant(
    ...
    """Retorna o quant em (product, FB, location=8, lot) ou None."""
...
def carregar_planilha(path: str) -> List[Dict]:
    """Le XLSX e retorna lista de dicts {idx, filial, cod, lote, diff_qtd}."""
    df = pd.read_excel(path, dtype={COL_COD: str, COL_LOTE: str})
    df.columns = [c.strip() for c in df.columns]
    # Valida schema esperado
    obrig = {COL_FILIAL, COL_COD, COL_LOTE, COL_DIFF}
...
def processar_linha(
    ...
    # 2. Filtro: diff_qtd deve ser positivo (escopo do script)
    if item['diff_qtd'] <= 0:
        r['status'] = 'SKIP_AJUSTE_NEGATIVO'
        r['erro'] = f'diff_qtd {item["diff_qtd"]} <= 0 (escopo: somente positivos)'
        return r
    ...
    ajuste = round(item['diff_qtd'], CASAS_DECIMAIS)
    if abs(ajuste - item['diff_qtd']) > 1e-9:
        r['ajuste_truncado_em_6_casas'] = True
        logger.warning(
            f'idx={item["idx"]} diff_qtd {item["diff_qtd"]} truncado para {ajuste}'
        )
    r['ajuste_aplicado'] = ajuste

    # 6. Buscar quant (so se lote ja existe; se vai ser criado, vamos
...
def main():
    ...
    banner(
        f'AJUSTE POSITIVO FB — {"DRY-RUN" if args.dry_run else "EXECUCAO REAL"} '
    ...
    if not log_path:
        ...
        log_path = str(
            _THIS.parent / 'auditoria' / f'log_13_ajuste_pos_fb_{ts}.json'
        )
